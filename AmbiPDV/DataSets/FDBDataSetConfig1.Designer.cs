...
END;";
            ...
            this._commandCollection[21].CommandText = "\r\n\t\t\t\t\t\tCREATE OR ALTER PROCEDURE SP_TRI_PROCEDURES7\r\n\t\t\t\t\t\tRETURNS\r\n\t\t\t\t\t\t(\r\n\t\t\t" +
                ...
                " INTEGER) RETURNS ( ID_ESTOQUE INTEGER, ID_GRUPO INTEGER, DESCRICAO VARCHAR(150)" +
                ", STATUS VARCHAR(1), DT_CADAST DATE, HR_CADAST TIME, FRACIONADO CHAR(1), PRC_VEN" +
                "DA NUMERIC(18,4), PRC_CUSTO NUMERIC(18,4), ULT_VENDA DATE, MARGEM_LB NUMERIC(18," +
                "4), POR_COMISSAO NUMERIC(18,4), ULT_FORNEC INTEGER, GRADE_SERIE CHAR(1), ID_TIPO" +
                "ITEM VARCHAR(2), ID_CTI CHAR(3), CST_PIS VARCHAR(3), CST_COFINS VARCHAR(3), PIS " +
                "NUMERIC(18,4), COFINS NUMERIC(18,4), UNI_MEDIDA VARCHAR(3), MARGEM_PV NUMERIC(18" +
                ...
                " NUMERIC(18,4), TRI_PDV_DT_UPD TIMESTAMP, ID_IDENTIFICADOR INTEGER, ID_CTI_CFE C" +
                "HAR(3), MARGEM_LB_ATAC NUMERIC(18,4), INFCOMPLEMENTAR VARCHAR(250), ID_MOTIVO_DE" +
                "SO INTEGER, ID_CTAPLA INTEGER, COD_ANVISA VARCHAR(13), PMC NUMERIC(18,4), IRRF N" +
                "UMERIC(18,4), MED_MOTIVO_ISENCAO VARCHAR(255), CONTROLEESP INTEGER, ID_SUBGRUPO " +
                "SMALLINT, COD_BENEF_CFOP INTEGER, TP_PRC_ATACADO SMALLINT, EMPCADASTRO VARCHAR(1" +
                "8), ALIQINTERNA NUMERIC(18,4) ) AS BEGIN FOR SELECT A.ID_ESTOQUE , A.ID_GRUPO , " +
                "A.DESCRICAO , A.STATUS , A.DT_CADAST , A.HR_CADAST , A.FRACIONADO , A.PRC_VENDA " +
                ", A.PRC_CUSTO , A.ULT_VENDA , A.MARGEM_LB , A.POR_COMISSAO , A.ULT_FORNEC , A.GR" +
                "ADE_SERIE , A.ID_TIPOITEM , A.ID_CTI , A.CST_PIS , A.CST_COFINS , A.PIS , A.COFI" +
                "NS , A.UNI_MEDIDA , A.MARGEM_PV , A.CFOP , A.OBSERVACAO , A.NAT_RECEITA , A.CFOP" +
                "_NF , A.PRC_ATACADO , A.ID_CTI_PART , A.ID_CTI_FCP , A.QTD_ATACADO , A.TRI_PDV_D" +
                "T_UPD , B.ID_IDENTIFICADOR , A.ID_CTI_CFE, A.MARGEM_LB_ATAC, A.INFCOMPLEMENTAR ," +
                " A.ID_MOTIVO_DESO , A.ID_CTAPLA , A.COD_ANVISA , A.PMC , A.IRRF , A.MED_MOTIVO_I" +
                "SENCAO , A.CONTROLEESP , A.ID_SUBGRUPO , A.COD_BENEF_CFOP , A.TP_PRC_ATACADO , A" +
                ".EMPCADASTRO , A.ALIQINTERNA FROM TB_ESTOQUE A JOIN TB_EST_IDENTIFICADOR B ON B." +
                "ID_ESTOQUE = A.ID_ESTOQUE WHERE A.ID_ESTOQUE = :pID_ESTOQUE INTO :ID_ESTOQUE , :" +
                "ID_GRUPO , :DESCRICAO , :STATUS , :DT_CADAST , :HR_CADAST , :FRACIONADO , :PRC_V" +
                "ENDA , :PRC_CUSTO , :ULT_VENDA , :MARGEM_LB , :POR_COMISSAO , :ULT_FORNEC , :GRA" +
                "DE_SERIE , :ID_TIPOITEM , :ID_CTI , :CST_PIS , :CST_COFINS , :PIS , :COFINS , :U" +
                "NI_MEDIDA , :MARGEM_PV , :CFOP , :OBSERVACAO , :NAT_RECEITA , :CFOP_NF , :PRC_AT" +
                "ACADO , :ID_CTI_PART , :ID_CTI_FCP , :QTD_ATACADO , :TRI_PDV_DT_UPD , :ID_IDENTI" +
                "FICADOR , :ID_CTI_CFE , :MARGEM_LB_ATAC, :INFCOMPLEMENTAR , :ID_MOTIVO_DESO , :I" +
                "D_CTAPLA , :COD_ANVISA , :PMC , :IRRF , :MED_MOTIVO_ISENCAO , :CONTROLEESP , :ID" +
                "_SUBGRUPO , :COD_BENEF_CFOP , :TP_PRC_ATACADO , :EMPCADASTRO , :ALIQINTERNA DO B" +
                ...
                " VARIABLE vMaxIdTermario TYPE OF COLUMN TRI_PDV_TERMINAL_USUARIO.ID_OPER; DECLAR" +
                "E VARIABLE vDiffNextSeqValue INTEGER; DECLARE VARIABLE vNewNextSeqValue INTEGER;" +
                ...
                "(:vMaxIdTermario > :vCurrentSeqValue) THEN BEGIN vDiffNextSeqValue = (:vMaxIdTer" +
                "mario - GEN_ID(GEN_TRI_PDV_TERM_USUARIO, 0)); vNewNextSeqValue = GEN_ID(GEN_TRI_" +
                "PDV_TERM_USUARIO, :vDiffNextSeqValue); END rNewNextSeqValue = :vNewNextSeqValue;" +
                " END;\';\r\n\r\n\t\t\t\t\t\terro = \'sproc SP_TRI_NATOPER_UPSERT\';\r\n\t\t\t\t\t\texecute statement " +
                "\'CREATE OR ALTER PROCEDURE SP_TRI_NATOPER_UPSERT ( pID_NATOPE TYPE OF COLUMN TB_" +
                "NAT_OPERACAO.ID_NATOPE , pDESCRICAO TYPE OF COLUMN TB_NAT_OPERACAO.DESCRICAO , p" +
                "RET_PIS_COF_CSLL TYPE OF COLUMN TB_NAT_OPERACAO.RET_PIS_COF_CSLL , pRET_INSS TYP" +
                "E OF COLUMN TB_NAT_OPERACAO.RET_INSS , pRET_IRRF TYPE OF COLUMN TB_NAT_OPERACAO." +
                "RET_IRRF , pPIS_COFINS TYPE OF COLUMN TB_NAT_OPERACAO.PIS_COFINS , pSTATUS TYPE " +
                "OF COLUMN TB_NAT_OPERACAO.STATUS , pCFOP TYPE OF COLUMN TB_NAT_OPERACAO.CFOP , p" +
                "ID_CTI TYPE OF COLUMN TB_NAT_OPERACAO.ID_CTI , pGFR TYPE OF COLUMN TB_NAT_OPERAC" +
                "AO.GFR , pOBSERVACAO TYPE OF COLUMN TB_NAT_OPERACAO.OBSERVACAO , pBASE_COMISSAO " +
                "TYPE OF COLUMN TB_NAT_OPERACAO.BASE_COMISSAO , pCALCULA_IPI TYPE OF COLUMN TB_NA" +
                "T_OPERACAO.CALCULA_IPI , pGRAVA_TOT_TRIBUTOS TYPE OF COLUMN TB_NAT_OPERACAO.GRAV" +
                "A_TOT_TRIBUTOS ) RETURNS ( rRowsAffected INTEGER ) AS BEGIN UPDATE OR INSERT INT" +
                "O TB_NAT_OPERACAO (ID_NATOPE , DESCRICAO , RET_PIS_COF_CSLL , RET_INSS , RET_IRR" +
                "F , PIS_COFINS , STATUS , CFOP , ID_CTI , GFR , OBSERVACAO , BASE_COMISSAO , CAL" +
                "CULA_IPI , GRAVA_TOT_TRIBUTOS ) VALUES ( :pID_NATOPE , :pDESCRICAO , :pRET_PIS_C" +
                "OF_CSLL , :pRET_INSS , :pRET_IRRF , :pPIS_COFINS , :pSTATUS , :pCFOP , :pID_CTI " +
                ", :pGFR , :pOBSERVACAO , :pBASE_COMISSAO , :pCALCULA_IPI , :pGRAVA_TOT_TRIBUTOS " +
                ") MATCHING (ID_NATOPE ) ; rRowsAffected = ROW_COUNT; END;\';\r\n\r\n\t\t\t\t\t\terro = \'spr" +
                "oc SP_TRI_FMAPGTONFCE_UPSERT\';\r\n\t\t\t\t\t\texecute statement \'CREATE OR ALTER PROCEDU" +
                "RE SP_TRI_FMAPGTONFCE_UPSERT ( pID_FMANFCE TYPE OF COLUMN TB_FORMA_PAGTO_NFCE.ID" +
                "_FMANFCE , pSTATUS TYPE OF COLUMN TB_FORMA_PAGTO_NFCE.STATUS , pID_NFCE TYPE OF " +
                "COLUMN TB_FORMA_PAGTO_NFCE.ID_NFCE , pDESCRICAO TYPE OF COLUMN TB_FORMA_PAGTO_NF" +
                "CE.DESCRICAO ) RETURNS ( rRowsAffected INTEGER ) AS BEGIN UPDATE OR INSERT INTO " +
                "TB_FORMA_PAGTO_NFCE (ID_FMANFCE , STATUS , ID_NFCE , DESCRICAO ) VALUES ( :pID_F" +
                "MANFCE , :pSTATUS , :pID_NFCE , :pDESCRICAO ) MATCHING (ID_FMANFCE ) ; rRowsAffe" +
                "cted = ROW_COUNT; END;\';\r\n\r\n\t\t\t\t\t\terro = \'sproc SP_TRI_NFVENDA_GETALL_SYNC\';\r\n\t\t" +
                "\t\t\t\texecute statement \'CREATE OR ALTER PROCEDURE SP_TRI_NFVENDA_GETALL_SYNC ( TI" +
                "P_QUERY SMALLINT ) RETURNS ( ID_NFVENDA TYPE OF COLUMN TB_NFVENDA.ID_NFVENDA , I" +
                "D_NATOPE TYPE OF COLUMN TB_NFVENDA.ID_NATOPE , ID_VENDEDOR TYPE OF COLUMN TB_NFV" +
                "ENDA.ID_VENDEDOR , ID_CLIENTE TYPE OF COLUMN TB_NFVENDA.ID_CLIENTE , NF_NUMERO T" +
                "YPE OF COLUMN TB_NFVENDA.NF_NUMERO , NF_SERIE TYPE OF COLUMN TB_NFVENDA.NF_SERIE" +
                " , NF_MODELO TYPE OF COLUMN TB_NFVENDA.NF_MODELO , DT_EMISSAO TYPE OF COLUMN TB_" +
                "NFVENDA.DT_EMISSAO , DT_SAIDA TYPE OF COLUMN TB_NFVENDA.DT_SAIDA , HR_SAIDA TYPE" +
                " OF COLUMN TB_NFVENDA.HR_SAIDA , ESPECIE TYPE OF COLUMN TB_NFVENDA.ESPECIE , TIP" +
                "O_FRETE TYPE OF COLUMN TB_NFVENDA.TIPO_FRETE , PES_LIQUID TYPE OF COLUMN TB_NFVE" +
                "NDA.PES_LIQUID , PES_BRUTO TYPE OF COLUMN TB_NFVENDA.PES_BRUTO , STATUS TYPE OF " +
                "COLUMN TB_NFVENDA.STATUS , ENT_SAI TYPE OF COLUMN TB_NFVENDA.ENT_SAI , ID_FMAPGT" +
                "O TYPE OF COLUMN TB_NFVENDA.ID_FMAPGTO , ID_PARCELA TYPE OF COLUMN TB_NFVENDA.ID" +
                "_PARCELA , MARCA TYPE OF COLUMN TB_NFVENDA.MARCA , QTD_VOLUM TYPE OF COLUMN TB_N" +
                "FVENDA.QTD_VOLUM , NUM_VOLUM TYPE OF COLUMN TB_NFVENDA.NUM_VOLUM , PROD_REV TYPE" +
                " OF COLUMN TB_NFVENDA.PROD_REV , SOMA_FRETE TYPE OF COLUMN TB_NFVENDA.SOMA_FRETE" +
                " , VLR_TROCO TYPE OF COLUMN TB_NFVENDA.VLR_TROCO , IND_PRES TYPE OF COLUMN TB_NF" +
                "VENDA.IND_PRES , IND_IE_DEST TYPE OF COLUMN TB_NFVENDA.IND_IE_DEST , DESCONTO_CO" +
                "NDICIONAL TYPE OF COLUMN TB_NFVENDA.DESCONTO_CONDICIONAL , INF_COMP_FIXA TYPE OF" +
                " COLUMN TB_NFVENDA.INF_COMP_FIXA , INF_COMP_EDIT TYPE OF COLUMN TB_NFVENDA.INF_C" +
                "OMP_EDIT , ENDERECO_ENTREGA TYPE OF COLUMN TB_NFVENDA.ENDERECO_ENTREGA , ENVIO_A" +
                "PI TYPE OF COLUMN TB_NFVENDA.ENVIO_API , SYNCED SMALLINT , QTD_CTAREC INTEGER ) " +
                "AS BEGIN FOR SELECT FIRST 200 COALESCE(a.ID_NFVENDA, 0) , a.ID_NATOPE , a.ID_VEN" +
                "DEDOR , a.ID_CLIENTE , a.NF_NUMERO , a.NF_SERIE , a.NF_MODELO , a.DT_EMISSAO , a" +
                ...
                "NUM_VOLUM , a.PROD_REV , a.SOMA_FRETE , a.VLR_TROCO , a.IND_PRES , a.IND_IE_DEST" +
                " , a.DESCONTO_CONDICIONAL , a.INF_COMP_FIXA , a.INF_COMP_EDIT , a.ENDERECO_ENTRE" +
                "GA , a.ENVIO_API , a.SYNCED , COUNT(b.ID_CTAREC) QTD_CTAREC FROM TB_NFVENDA a LE" +
                "FT JOIN TB_NFV_CTAREC b ON a.ID_NFVENDA = b.ID_NFVENDA WHERE ( :TIP_QUERY = 0 AN" +
                "D ( ( a.SYNCED IS NULL OR a.SYNCED = 0 ) AND (a.STATUS = \'\'E\'\' OR a.STATUS = \'\'I" +
                "\'\' OR a.STATUS = \'\'C\'\') ) ) OR ( :TIP_QUERY = 1 AND ( a.SYNCED = 1 AND (a.STATUS" +
                " = \'\'C\'\' OR a.STATUS = \'\'X\'\') ) ) OR (:TIP_QUERY = 3 AND (a.SYNCED = 3 AND (a.ST" +
                "ATUS = \'\'I\'\' OR a.STATUS = \'\'E\'\'))) GROUP BY a.ID_NFVENDA , a.ID_NATOPE , a.ID_V" +
                "ENDEDOR , a.ID_CLIENTE , a.NF_NUMERO , a.NF_SERIE , a.NF_MODELO , a.DT_EMISSAO ," +
                " a.DT_SAIDA , a.HR_SAIDA , a.ESPECIE , a.TIPO_FRETE , a.PES_LIQUID , a.PES_BRUTO" +
                " , a.STATUS , a.ENT_SAI , a.ID_FMAPGTO , a.ID_PARCELA , a.MARCA , a.QTD_VOLUM , " +
                "a.NUM_VOLUM , a.PROD_REV , a.SOMA_FRETE , a.VLR_TROCO , a.IND_PRES , a.IND_IE_DE" +
                "ST , a.DESCONTO_CONDICIONAL , a.INF_COMP_FIXA , a.INF_COMP_EDIT , a.ENDERECO_ENT" +
                "REGA , a.ENVIO_API , a.SYNCED ORDER BY a.DT_EMISSAO , a.DT_SAIDA , a.HR_SAIDA IN" +
                "TO :ID_NFVENDA , :ID_NATOPE , :ID_VENDEDOR , :ID_CLIENTE , :NF_NUMERO , :NF_SERI" +
                "E , :NF_MODELO , :DT_EMISSAO , :DT_SAIDA , :HR_SAIDA , :ESPECIE , :TIPO_FRETE , " +
                ":PES_LIQUID , :PES_BRUTO , :STATUS , :ENT_SAI , :ID_FMAPGTO , :ID_PARCELA , :MAR" +
                "CA , :QTD_VOLUM , :NUM_VOLUM , :PROD_REV , :SOMA_FRETE , :VLR_TROCO , :IND_PRES " +
                ", :IND_IE_DEST , :DESCONTO_CONDICIONAL , :INF_COMP_FIXA , :INF_COMP_EDIT , :ENDE" +
                "RECO_ENTREGA , :ENVIO_API , :SYNCED , :QTD_CTAREC DO BEGIN SUSPEND ; END END;\';\r" +
                "\n\r\n\t\t\t\t\t\terro = \'sproc SP_TRI_NFVENDA_SYNC_INSERT\';\r\n\t\t\t\t\t\texecute statement \'CR" +
                "EATE OR ALTER PROCEDURE SP_TRI_NFVENDA_SYNC_INSERT ( pID_NATOPE TYPE OF COLUMN T" +
                "B_NFVENDA.ID_NATOPE , pID_VENDEDOR TYPE OF COLUMN TB_NFVENDA.ID_VENDEDOR , pID_C" +
                "LIENTE TYPE OF COLUMN TB_NFVENDA.ID_CLIENTE , pNF_NUMERO TYPE OF COLUMN TB_NFVEN" +
                "DA.NF_NUMERO , pNF_SERIE TYPE OF COLUMN TB_NFVENDA.NF_SERIE , pNF_MODELO TYPE OF" +
                " COLUMN TB_NFVENDA.NF_MODELO , pDT_EMISSAO TYPE OF COLUMN TB_NFVENDA.DT_EMISSAO " +
                ", pDT_SAIDA TYPE OF COLUMN TB_NFVENDA.DT_SAIDA , pHR_SAIDA TYPE OF COLUMN TB_NFV" +
                "ENDA.HR_SAIDA , pESPECIE TYPE OF COLUMN TB_NFVENDA.ESPECIE , pTIPO_FRETE TYPE OF" +
                " COLUMN TB_NFVENDA.TIPO_FRETE , pPES_LIQUID TYPE OF COLUMN TB_NFVENDA.PES_LIQUID" +
                " , pPES_BRUTO TYPE OF COLUMN TB_NFVENDA.PES_BRUTO , pSTATUS TYPE OF COLUMN TB_NF" +
                "VENDA.STATUS , pENT_SAI TYPE OF COLUMN TB_NFVENDA.ENT_SAI , pID_FMAPGTO TYPE OF " +
                "COLUMN TB_NFVENDA.ID_FMAPGTO , pID_PARCELA TYPE OF COLUMN TB_NFVENDA.ID_PARCELA " +
                ", pMARCA TYPE OF COLUMN TB_NFVENDA.MARCA , pQTD_VOLUM TYPE OF COLUMN TB_NFVENDA." +
                "QTD_VOLUM , pNUM_VOLUM TYPE OF COLUMN TB_NFVENDA.NUM_VOLUM , pPROD_REV TYPE OF C" +
                "OLUMN TB_NFVENDA.PROD_REV , pSOMA_FRETE TYPE OF COLUMN TB_NFVENDA.SOMA_FRETE , p" +
                "VLR_TROCO TYPE OF COLUMN TB_NFVENDA.VLR_TROCO , pIND_PRES TYPE OF COLUMN TB_NFVE" +
                "NDA.IND_PRES , pIND_IE_DEST TYPE OF COLUMN TB_NFVENDA.IND_IE_DEST , pDESCONTO_CO" +
                "NDICIONAL TYPE OF COLUMN TB_NFVENDA.DESCONTO_CONDICIONAL , pINF_COMP_FIXA TYPE O" +
                "F COLUMN TB_NFVENDA.INF_COMP_FIXA , pINF_COMP_EDIT TYPE OF COLUMN TB_NFVENDA.INF" +
                "_COMP_EDIT , pENDERECO_ENTREGA TYPE OF COLUMN TB_NFVENDA.ENDERECO_ENTREGA , pENV" +
                "IO_API TYPE OF COLUMN TB_NFVENDA.ENVIO_API , pSYNCED SMALLINT ) RETURNS ( NFVEND" +
                "AUSAVEL INTEGER ) AS DECLARE VARIABLE ULTIMONFVENDA INTEGER; BEGIN SELECT NEXT V" +
                "ALUE FOR GEN_TB_NFVENDA_ID FROM RDB$DATABASE INTO :ULTIMONFVENDA ; IF( :ULTIMONF" +
                "VENDA IS NULL ) THEN ULTIMONFVENDA = 0; BEGIN INSERT INTO TB_NFVENDA ( ID_NFVEND" +
                "A , ID_NATOPE , ID_VENDEDOR , ID_CLIENTE , NF_NUMERO , NF_SERIE , NF_MODELO , DT" +
                "_EMISSAO , DT_SAIDA , HR_SAIDA , ESPECIE , TIPO_FRETE , PES_LIQUID , PES_BRUTO ," +
                " STATUS , ENT_SAI , ID_FMAPGTO , ID_PARCELA , MARCA , QTD_VOLUM , NUM_VOLUM , PR" +
                "OD_REV , SOMA_FRETE , VLR_TROCO , IND_PRES , IND_IE_DEST , DESCONTO_CONDICIONAL " +
                ", INF_COMP_FIXA , INF_COMP_EDIT , ENDERECO_ENTREGA , ENVIO_API , SYNCED ) VALUES" +
                " (:ULTIMONFVENDA , :pID_NATOPE , :pID_VENDEDOR , :pID_CLIENTE , :pNF_NUMERO , :p" +
                "NF_SERIE , :pNF_MODELO , :pDT_EMISSAO , :pDT_SAIDA , :pHR_SAIDA , :pESPECIE , :p" +
                "TIPO_FRETE , :pPES_LIQUID , :pPES_BRUTO , :pSTATUS , :pENT_SAI , :pID_FMAPGTO , " +
                ":pID_PARCELA , :pMARCA , :pQTD_VOLUM , :pNUM_VOLUM , :pPROD_REV , :pSOMA_FRETE ," +
                " :pVLR_TROCO , :pIND_PRES , :pIND_IE_DEST , :pDESCONTO_CONDICIONAL , :pINF_COMP_" +
                "FIXA , :pINF_COMP_EDIT , :pENDERECO_ENTREGA , :pENVIO_API , :pSYNCED ) ; NFVENDA" +
                "USAVEL = ULTIMONFVENDA; END END;\';\r\n\r\n\t\t\t\t\t\terro = \'sproc SP_TRI_NFV_FMAPAGTO_BY" +
                "_IDNFV\';\r\n\t\t\t\t\t\texecute statement \'CREATE OR ALTER PROCEDURE SP_TRI_NFV_FMAPAGTO" +
                "_BY_IDNFV ( pIdNfvenda TYPE OF COLUMN TB_NFVENDA_FMAPAGTO_NFCE.ID_NFVENDA ) RETU" +
                "RNS ( ID_NUMPAG TYPE OF COLUMN TB_NFVENDA_FMAPAGTO_NFCE.ID_NUMPAG , VLR_PAGTO TY" +
                "PE OF COLUMN TB_NFVENDA_FMAPAGTO_NFCE.VLR_PAGTO , ID_NFVENDA TYPE OF COLUMN TB_N" +
                "FVENDA_FMAPAGTO_NFCE.ID_NFVENDA , ID_FMANFCE TYPE OF COLUMN TB_NFVENDA_FMAPAGTO_" +
                "NFCE.ID_FMANFCE , ID_PARCELA TYPE OF COLUMN TB_NFVENDA_FMAPAGTO_NFCE.ID_PARCELA)" +
                ...
                " :ID_NUMPAG , :VLR_PAGTO , :ID_NFVENDA , :ID_FMANFCE , :ID_PARCELA DO SUSPEND ; " +
                "END;\';\r\n\r\n\t\t\t\t\t\terro = \'sproc SP_TRI_NFV_FMAPAGT_SYNC_INSERT\';\r\n\t\t\t\t\t\texecute st" +
                "atement \'CREATE OR ALTER PROCEDURE SP_TRI_NFV_FMAPAGT_SYNC_INSERT ( pVLR_PAGTO T" +
                "YPE OF COLUMN TB_NFVENDA_FMAPAGTO_NFCE.VLR_PAGTO , pID_NFVENDA TYPE OF COLUMN TB" +
                "_NFVENDA_FMAPAGTO_NFCE.ID_NFVENDA , pID_FMANFCE TYPE OF COLUMN TB_NFVENDA_FMAPAG" +
                "TO_NFCE.ID_FMANFCE , pID_PARCELA TYPE OF COLUMN TB_NFVENDA_FMAPAGTO_NFCE.ID_PARC" +
                "ELA, pID_ADMINISTRADORA TYPE OF COLUMN TB_NFCE_BANDEIRA.ID_ADMINISTRADORA) RETUR" +
                "NS ( rNewIdNumpag TYPE OF COLUMN TB_NFVENDA_FMAPAGTO_NFCE.ID_NUMPAG ) AS DECLARE" +
                " VARIABLE newIdNumpag TYPE OF COLUMN TB_NFVENDA_FMAPAGTO_NFCE.ID_NUMPAG; BEGIN S" +
                "ELECT NEXT VALUE FOR gen_tb_nfvenda_fmapagto_nfce_id FROM RDB$DATABASE INTO :new" +
                "IdNumpag ; IF( :newIdNumpag IS NULL ) THEN newIdNumpag = 0; BEGIN INSERT INTO TB" +
                "_NFVENDA_FMAPAGTO_NFCE (ID_NUMPAG , VLR_PAGTO , ID_NFVENDA , ID_FMANFCE , ID_PAR" +
                "CELA ) VALUES ( :newIdNumpag , :pVLR_PAGTO , :pID_NFVENDA , :pID_FMANFCE , :pID_" +
                "PARCELA ) ; INSERT INTO TB_NFCE_BANDEIRA (ID_NUMPAG , ID_BANDEIRA , ID_ADMINISTR" +
                "ADORA , NUM_AUT , FORMA_ENVIO , SICLOS_ID , STONE_ID , PRE_TRANSACAO_ID , POS_SE" +
                "RIAL) VALUES (:newIdNumpag , NULL, :pID_ADMINISTRADORA , NULL , NULL , NULL , NU" +
                "LL , NULL , NULL) ; rNewIdNumpag = newIdNumpag; END END;\';\r\n\r\n\t\t\t\t\t\terro = \'spro" +
                "c SP_TRI_NFVITEM_GETBY_IDNFVENDA\';\r\n\t\t\t\t\t\texecute statement \'CREATE OR ALTER PRO" +
                "CEDURE SP_TRI_NFVITEM_GETBY_IDNFVENDA (PIDNFVENDA INTEGER) RETURNS ( ID_NFVITEM " +
                "INTEGER, ID_NFVENDA INTEGER, ID_IDENTIFICADOR INTEGER, CFOP VARCHAR(4), NUM_ITEM" +
                " SMALLINT, QTD_ITEM NUMERIC(18,4), UNI_MEDIDA VARCHAR(3), VLR_TOTAL NUMERIC(18,4" +
                "), VLR_DESC NUMERIC(18,4), VLR_CUSTO NUMERIC(18,4), PRC_LISTA NUMERIC(18,4), CF " +
                "VARCHAR(13), VLR_FRETE NUMERIC(18,4), VLR_SEGURO NUMERIC(18,4), VLR_DESPESA NUME" +
                "RIC(18,4), RET_PIS_COF_CSLL NUMERIC(18,4), RET_IRRF NUMERIC(18,4), COD_ENQ VARCH" +
                "AR(3), COD_BASE VARCHAR(2), CSOSN VARCHAR(3), NPED_COMPRA VARCHAR(15), ITEM_COMP" +
                "RA INTEGER, VLR_TOTTRIB NUMERIC(18,4), FCI VARCHAR(36), VLR_ICM_DESO NUMERIC(18," +
                "4), ID_MOTIVO_DESO INTEGER, EST_BX CHAR(1), VLR_TRIB_FED NUMERIC(18,4), VLR_TRIB" +
                "_EST NUMERIC(18,4), VLR_TRIB_MUN NUMERIC(18,4), INCLUIR_FATURA CHAR(1), VLR_UNIT" +
                " NUMERIC(18,4), VLR_RETENCAO NUMERIC(18,4), REFERENCIA VARCHAR(18), ID_COMPOSICA" +
                "O INTEGER, CODPROMOSCANNTECH INTEGER ) AS BEGIN FOR SELECT COALESCE(a.ID_NFVITEM" +
                ", 0) ID_NFVITEM , a.ID_NFVENDA , a.ID_IDENTIFICADOR , a.CFOP , a.NUM_ITEM , a.QT" +
                "D_ITEM , a.UNI_MEDIDA , a.VLR_TOTAL , a.VLR_DESC , a.VLR_CUSTO , a.PRC_LISTA , a" +
                ".CF , a.VLR_FRETE , a.VLR_SEGURO , a.VLR_DESPESA , a.RET_PIS_COF_CSLL , a.RET_IR" +
                "RF , a.COD_ENQ , a.COD_BASE , a.CSOSN , a.NPED_COMPRA , a.ITEM_COMPRA , a.VLR_TO" +
                "TTRIB , a.FCI , a.VLR_ICM_DESO , a.ID_MOTIVO_DESO , a.EST_BX , a.VLR_TRIB_FED , " +
                "a.VLR_TRIB_EST , a.VLR_TRIB_MUN , a.INCLUIR_FATURA , a.VLR_UNIT , a.VLR_RETENCAO" +
                " , a.REFERENCIA , e.ID_COMPOSICAO, a.CODPROMOSCANNTECH FROM TB_NFV_ITEM a LEFT J" +
                "OIN TB_EST_COMPOSICAO e ON e.ID_IDENTIFICADOR = a.ID_IDENTIFICADOR WHERE a.ID_NF" +
                "VENDA =:pIdNfvenda INTO :ID_NFVITEM , :ID_NFVENDA , :ID_IDENTIFICADOR , :CFOP , " +
                ":NUM_ITEM , :QTD_ITEM , :UNI_MEDIDA , :VLR_TOTAL , :VLR_DESC , :VLR_CUSTO , :PRC" +
                "_LISTA , :CF , :VLR_FRETE , :VLR_SEGURO , :VLR_DESPESA , :RET_PIS_COF_CSLL , :RE" +
                "T_IRRF , :COD_ENQ , :COD_BASE , :CSOSN , :NPED_COMPRA , :ITEM_COMPRA , :VLR_TOTT" +
                "RIB , :FCI , :VLR_ICM_DESO , :ID_MOTIVO_DESO , :EST_BX , :VLR_TRIB_FED , :VLR_TR" +
                "IB_EST , :VLR_TRIB_MUN , :INCLUIR_FATURA , :VLR_UNIT , :VLR_RETENCAO , :REFERENC" +
                "IA , :ID_COMPOSICAO, :CODPROMOSCANNTECH DO SUSPEND; END;\';\r\n\r\n\t\t\t\t\t\terro = \'spro" +
                "c SP_TRI_NFVITEM_SYNC_INSERT\';\r\n\t\t\t\t\t\texecute statement \'CREATE OR ALTER PROCEDU" +
                "RE SP_TRI_NFVITEM_SYNC_INSERT (PID_NFVENDA INTEGER, PID_IDENTIFICADOR INTEGER, P" +
                "CFOP VARCHAR(4), PNUM_ITEM SMALLINT, PQTD_ITEM NUMERIC(18,4), PUNI_MEDIDA VARCHA" +
                "R(3), PVLR_TOTAL NUMERIC(18,4), PVLR_DESC NUMERIC(18,4), PVLR_CUSTO NUMERIC(18,4" +
                "), PPRC_LISTA NUMERIC(18,4), PCF VARCHAR(13), PVLR_FRETE NUMERIC(18,4), PVLR_SEG" +
                "URO NUMERIC(18,4), PVLR_DESPESA NUMERIC(18,4), PRET_PIS_COF_CSLL NUMERIC(18,4), " +
                "PRET_IRRF NUMERIC(18,4), PCOD_ENQ VARCHAR(3), PCOD_BASE VARCHAR(2), PCSOSN VARCH" +
                "AR(3), PNPED_COMPRA VARCHAR(15), PITEM_COMPRA INTEGER, PVLR_TOTTRIB NUMERIC(18,4" +
                "), PFCI VARCHAR(36), PVLR_ICM_DESO NUMERIC(18,4), PID_MOTIVO_DESO INTEGER, PEST_" +
                "BX CHAR(1), PVLR_TRIB_FED NUMERIC(18,4), PVLR_TRIB_EST NUMERIC(18,4), PVLR_TRIB_" +
                "MUN NUMERIC(18,4), PINCLUIR_FATURA CHAR(1), PVLR_UNIT NUMERIC(18,4), PVLR_RETENC" +
                "AO NUMERIC(18,4), PREFERENCIA VARCHAR(18), PCOD_SCANNTECH INTEGER) RETURNS (  RN" +
                "EWIDNFVITEM INTEGER ) AS DECLARE VARIABLE newIdNfvItem INTEGER; BEGIN  SELECT  N" +
                "EXT VALUE FOR GEN_TB_NFV_ITEM_ID FROM  RDB$DATABASE INTO  :newIdNfvItem ;  IF( :" +
                ...
                " UNI_MEDIDA ,  VLR_TOTAL ,  VLR_DESC ,  VLR_CUSTO ,  PRC_LISTA ,  CF ,  VLR_FRET" +
                "E ,  VLR_SEGURO ,  VLR_DESPESA ,  RET_PIS_COF_CSLL ,  RET_IRRF ,  COD_ENQ ,  COD" +
                "_BASE ,  CSOSN ,  NPED_COMPRA ,  ITEM_COMPRA ,  VLR_TOTTRIB ,  FCI ,  VLR_ICM_DE" +
                "SO ,  ID_MOTIVO_DESO ,  EST_BX ,  VLR_TRIB_FED ,  VLR_TRIB_EST ,  VLR_TRIB_MUN ," +
                "  INCLUIR_FATURA ,  VLR_UNIT ,  VLR_RETENCAO ,  REFERENCIA,   CODPROMOSCANNTECH)" +
                " VALUES ( :newIdNfvItem , :pID_NFVENDA , :pID_IDENTIFICADOR , :pCFOP , :pNUM_ITE" +
                "M , :pQTD_ITEM , :pUNI_MEDIDA , :pVLR_TOTAL , :pVLR_DESC , :pVLR_CUSTO , :pPRC_L" +
                "ISTA , :pCF , :pVLR_FRETE , :pVLR_SEGURO , :pVLR_DESPESA , :pRET_PIS_COF_CSLL , " +
                ":pRET_IRRF , :pCOD_ENQ , :pCOD_BASE , :pCSOSN , :pNPED_COMPRA , :pITEM_COMPRA , " +
                ":pVLR_TOTTRIB , :pFCI , :pVLR_ICM_DESO , :pID_MOTIVO_DESO , :pEST_BX , :pVLR_TRI" +
                "B_FED , :pVLR_TRIB_EST , :pVLR_TRIB_MUN , :pINCLUIR_FATURA , :pVLR_UNIT , :pVLR_" +
                "RETENCAO , :pREFERENCIA,  :pCOD_SCANNTECH) ;  rnewIdNfvItem = newIdNfvItem; END " +
                "END;\';\r\n\r\n\t\t\t\t\t\terro = \'sproc SP_TRI_CTAREC_GETBY_IDNFVENDA\';\r\n\t\t\t\t\t\texecute sta" +
                "tement \'CREATE OR ALTER PROCEDURE SP_TRI_CTAREC_GETBY_IDNFVENDA (PIDNFVENDA INTE" +
                "GER) RETURNS ( ID_CTAREC INTEGER, DOCUMENTO VARCHAR(12), HISTORICO VARCHAR(50), " +
                "DT_EMISSAO DATE, DT_VENCTO DATE, VLR_CTAREC NUMERIC(18,4), TIP_CTAREC CHAR(1), I" +
                "D_PORTADOR INTEGER, ID_CLIENTE INTEGER, INV_REFERENCIA VARCHAR(18), DT_VENCTO_OR" +
                "IG DATE, NSU_CARTAO VARCHAR(32), ID_NUMPAG INTEGER, ID_CONTA INTEGER ) AS BEGIN " +
                "FOR SELECT COALESCE(a.ID_CTAREC, 0) , a.DOCUMENTO , a.HISTORICO , a.DT_EMISSAO ," +
                " a.DT_VENCTO , a.VLR_CTAREC , a.TIP_CTAREC , a.ID_PORTADOR , a.ID_CLIENTE , a.IN" +
                "V_REFERENCIA , a.DT_VENCTO_ORIG , a.NSU_CARTAO , b.ID_NUMPAG , a.ID_CONTA FROM T" +
                "B_CONTA_RECEBER a JOIN TB_NFV_CTAREC b ON a.ID_CTAREC = b.ID_CTAREC WHERE b.ID_N" +
                "FVENDA =:pIdNfvenda INTO :ID_CTAREC , :DOCUMENTO , :HISTORICO , :DT_EMISSAO , :D" +
                "T_VENCTO , :VLR_CTAREC , :TIP_CTAREC , :ID_PORTADOR , :ID_CLIENTE , :INV_REFEREN" +
                "CIA , :DT_VENCTO_ORIG , :NSU_CARTAO , :ID_NUMPAG, :ID_CONTA DO SUSPEND ; END;\';\r" +
                "\n\r\n\t\t\t\t\t\terro = \'sproc SP_TRI_NFVENDA_SETSYNCED\';\r\n\t\t\t\t\t\texecute statement \'CREA" +
                "TE OR ALTER PROCEDURE SP_TRI_NFVENDA_SETSYNCED ( pIdNfvenda INTEGER , pSynced SM" +
                "ALLINT ) AS BEGIN UPDATE TB_NFVENDA SET SYNCED = :pSynced WHERE ID_NFVENDA = :pI" +
                "dNfvenda ; END;\';\r\n\r\n\t\t\t\t\t\terro = \'sproc SP_TRI_CTAREC_GETBY_NFNUMSERIE\';\r\n\t\t\t\t\t" +
                "\texecute statement \'CREATE OR ALTER PROCEDURE SP_TRI_CTAREC_GETBY_NFNUMSERIE ( p" +
                "NF_NUMERO TYPE OF COLUMN TB_NFVENDA.NF_NUMERO , pNF_SERIE TYPE OF COLUMN TB_NFVE" +
                "NDA.NF_SERIE ) RETURNS ( ID_CTAREC INTEGER , DOCUMENTO VARCHAR(12) , HISTORICO V" +
                "ARCHAR(50) , DT_EMISSAO DATE , DT_VENCTO DATE , VLR_CTAREC TYPE OF COLUMN TRI_PD" +
                "V_OPER.DIN , TIP_CTAREC CHAR(1) , ID_PORTADOR INTEGER , ID_CLIENTE INTEGER , INV" +
                "_REFERENCIA VARCHAR(18) , DT_VENCTO_ORIG DATE , NSU_CARTAO VARCHAR(32) ) AS BEGI" +
                "N FOR SELECT a.ID_CTAREC , a.DOCUMENTO , a.HISTORICO , a.DT_EMISSAO , a.DT_VENCT" +
                "O , a.VLR_CTAREC , a.TIP_CTAREC , a.ID_PORTADOR , a.ID_CLIENTE , a.INV_REFERENCI" +
                "A , a.DT_VENCTO_ORIG , a.NSU_CARTAO FROM TB_CONTA_RECEBER a JOIN TB_NFV_CTAREC b" +
                " ON a.ID_CTAREC = b.ID_CTAREC JOIN TB_NFVENDA c ON c.ID_NFVENDA = b.ID_NFVENDA W" +
                "HERE c.NF_NUMERO = :pNF_NUMERO AND c.NF_SERIE = :pNF_SERIE INTO :ID_CTAREC , :DO" +
                "CUMENTO , :HISTORICO , :DT_EMISSAO , :DT_VENCTO , :VLR_CTAREC , :TIP_CTAREC , :I" +
                "D_PORTADOR , :ID_CLIENTE , :INV_REFERENCIA , :DT_VENCTO_ORIG , :NSU_CARTAO DO SU" +
                "SPEND ; END;\';\r\n\r\n\t\t\t\t\t\terro = \'sproc SP_TRI_NFV_CTAREC_SYNC_DEL\';\r\n\t\t\t\t\t\texecut" +
                "e statement \'CREATE OR ALTER PROCEDURE SP_TRI_NFV_CTAREC_SYNC_DEL ( pNfNumero TY" +
                "PE OF COLUMN TB_NFVENDA.NF_NUMERO , pNfSerie TYPE OF COLUMN TB_NFVENDA.NF_SERIE " +
                ", pIdCtarec TYPE OF COLUMN TB_NFV_CTAREC.ID_CTAREC ) AS DECLARE VARIABLE idNfven" +
                "da TYPE OF COLUMN TB_NFVENDA.ID_NFVENDA; BEGIN SELECT ID_NFVENDA FROM TB_NFVENDA" +
                " WHERE NF_NUMERO = :pNfNumero AND NF_SERIE = :pNfSerie AND NF_MODELO = 59 INTO :" +
                "idNfvenda ; DELETE FROM TB_NFV_CTAREC WHERE ID_NFVENDA = :idNfvenda AND ID_CTARE" +
                "C= :pIdCtarec ; END;\';\r\n\r\n\t\t\t\t\t\terro = \'sproc SP_TRI_NFV_UPDT_BYNFNUMSERIE\';\r\n\t\t" +
                "\t\t\t\texecute statement \'CREATE OR ALTER PROCEDURE SP_TRI_NFV_UPDT_BYNFNUMSERIE (P" +
                "ID_NATOPE INTEGER, PID_VENDEDOR SMALLINT, PID_CLIENTE INTEGER, PNF_NUMERO INTEGE" +
                "R, PNF_SERIE VARCHAR(3), PNF_MODELO VARCHAR(2), PDT_EMISSAO DATE, PDT_SAIDA DATE" +
                ", PHR_SAIDA TIME, PESPECIE VARCHAR(15), PPES_LIQUID NUMERIC(18,4), PPES_BRUTO NU" +
                "MERIC(18,4), PID_FMAPGTO SMALLINT, PID_PARCELA SMALLINT, PMARCA VARCHAR(15), PQT" +
                "D_VOLUM NUMERIC(18,4), PNUM_VOLUM VARCHAR(15), PVLR_TROCO NUMERIC(18,4), PIND_PR" +
                "ES CHAR(1), PIND_IE_DEST CHAR(1), PDESCONTO_CONDICIONAL CHAR(1), PINF_COMP_FIXA " +
                "BLOB, PINF_COMP_EDIT BLOB, PENDERECO_ENTREGA CHAR(1), PENVIO_API TIMESTAMP, PSYN" +
                "CED SMALLINT, PSOMA_FRETE CHAR(1) DEFAULT \'\'N\'\', PPROD_REV CHAR(1) DEFAULT \'\'N\'\'" +
                ", PSTATUS CHAR(1) DEFAULT \'\'E\'\', PENT_SAI CHAR(1) DEFAULT \'\'1\'\', PTIPO_FRETE CHA" +
                "R(1) DEFAULT \'\'9\'\') AS BEGIN UPDATE TB_NFVENDA SET ID_NATOPE =:pID_NATOPE , ID_V" +
            ...
            this._commandCollection[22].CommandText = "CREATE OR ALTER PROCEDURE SP_TRI_PROCEDURES8\r\nRETURNS\r\n(\r\nerro VARCHAR(64)\r\n)\r\nAS" +
                ...
                "AG INTEGER) RETURNS ( RID_CTAREC INTEGER ) AS DECLARE VARIABLE vNF_SERIE TYPE OF" +
                " COLUMN TB_NFVENDA.NF_SERIE; DECLARE VARIABLE vNF_NUMERO TYPE OF COLUMN TB_NFVEN" +
                "DA.NF_NUMERO; DECLARE VARIABLE vVALOR TYPE OF COLUMN TB_NFVENDA_FMAPAGTO_NFCE.VL" +
                "R_PAGTO; BEGIN SELECT NEXT VALUE FOR GEN_TB_CTAREC_ID FROM RDB$DATABASE INTO :rI" +
                "D_CTAREC; SELECT NF_SERIE, NF_NUMERO FROM TB_NFVENDA WHERE ID_NFVENDA = :pID_NFV" +
                "ENDA INTO :vNF_SERIE, :vNF_NUMERO; SELECT VLR_PAGTO FROM TB_NFVENDA_FMAPAGTO_NFC" +
                "E WHERE ID_NUMPAG = :pID_NUMPAG INTO :vVALOR; INSERT INTO TB_CONTA_RECEBER (ID_C" +
                "TAREC, DOCUMENTO, HISTORICO, DT_EMISSAO, DT_VENCTO, VLR_CTAREC, TIP_CTAREC, ID_P" +
                "ORTADOR, ID_CLIENTE, INV_REFERENCIA) VALUES (:rID_CTAREC, (CAST(:vNF_SERIE AS VA" +
                "RCHAR(12)) || \'\'-\'\' || CAST(:vNF_NUMERO AS VARCHAR(12))), :pDESCRICAO, CURRENT_D" +
                "ATE, :pDT_VENCIMENTO, :vVALOR, \'\'C\'\', 1, :pID_CLIENTE, \'\'D\'\' || LPAD(:rID_CTAREC" +
                ", 7, \'\'0\'\') || LPAD(:vNF_SERIE, 2, \'\'0\'\') || LPAD(:vNF_NUMERO, 7, \'\'0\'\')); INSER" +
                "T INTO TB_NFV_CTAREC (ID_NFVENDA, ID_CTAREC, ID_NUMPAG) VALUES (:pID_NFVENDA, :r" +
                "ID_CTAREC, :pID_NUMPAG); SUSPEND; END\';\r\n\r\n\t\t\t\t\t\terro = \'sproc SP_TRI_LANCAMOVDI" +
                "ARIO\';\r\n\t\t\t\t\t\texecute statement \'CREATE OR ALTER PROCEDURE SP_TRI_LANCAMOVDIARIO" +
                " (NUMCAIXA VARCHAR(60), VALOR NUMERIC(18,4), DESCRICAO_MOV VARCHAR(60), CONTA_OR" +
                "IGEM SMALLINT, CONTA_DESTINO SMALLINT) RETURNS (ID_MOVTO INTEGER) AS DECLARE VAR" +
                "IABLE ultimo_mov INTEGER; BEGIN SELECT NEXT VALUE FOR GEN_TB_MOVDIARIO_ID FROM R" +
                "DB$DATABASE INTO :ID_MOVTO; INSERT INTO TB_MOVDIARIO (ID_MOVTO, DT_MOVTO, HR_MOV" +
                "TO, HISTORICO, TIP_MOVTO, VLR_MOVTO, ID_CTAPLA) VALUES ((SELECT NEXT VALUE FOR G" +
                "EN_TB_MOVDIARIO_ID FROM RDB$DATABASE), CURRENT_DATE, CURRENT_TIME, :DESCRICAO_MO" +
                "V, \'\'C\'\', :VALOR, :CONTA_ORIGEM); INSERT INTO TB_MOVDIARIO (ID_MOVTO, DT_MOVTO, " +
                "HR_MOVTO, HISTORICO, TIP_MOVTO, VLR_MOVTO, ID_CTAPLA) VALUES (:ID_MOVTO, CURRENT" +
                "_DATE, CURRENT_TIME, :DESCRICAO_MOV, \'\'D\'\', :VALOR, :CONTA_DESTINO); SUSPEND; EN" +
                "D\';\r\n\r\n\t\t\t\t\t\terro = \'sproc SP_TRI_NFVFMAPGTO_INSERT\';\r\n\t\t\t\t\t\texecute statement \'" +
                "CREATE OR ALTER PROCEDURE SP_TRI_NFVFMAPGTO_INSERT (PVALOR TYPE OF COLUMN TB_NFV" +
                "ENDA_FMAPAGTO_NFCE.VLR_PAGTO, PID_NFVENDA INTEGER, PID_FMANFCE INTEGER, PID_PARC" +
                "ELA INTEGER, PID_ADMINISTRADORA INTEGER) RETURNS (RID_FMAPAGTO INTEGER) AS BEGIN" +
                " SELECT NEXT VALUE FOR gen_tb_nfvenda_fmapagto_nfce_id FROM RDB$DATABASE INTO :r" +
                "ID_FMAPAGTO; INSERT INTO TB_NFVENDA_FMAPAGTO_NFCE (ID_NUMPAG, VLR_PAGTO, ID_NFVE" +
                "NDA, ID_FMANFCE, ID_PARCELA) VALUES (:rID_FMAPAGTO, :pVALOR, :pID_NFVENDA, :pID_" +
                "FMANFCE, :pID_PARCELA); INSERT INTO TB_NFCE_BANDEIRA (ID_NUMPAG, ID_BANDEIRA, ID" +
                "_ADMINISTRADORA, NUM_AUT, FORMA_ENVIO, SICLOS_ID, STONE_ID, PRE_TRANSACAO_ID, PO" +
                "S_SERIAL) VALUES (:rID_FMAPAGTO, NULL, :pID_ADMINISTRADORA, NULL, NULL, NULL, NU" +
                "LL, NULL, NULL); END\';\r\n\r\n\t\t\t\t\t\terro = \'sproc SP_TRI_WHATSINSERE\';\r\n\t\t\t\t\t\texecut" +
                "e statement \'CREATE OR ALTER PROCEDURE SP_TRI_WHATSINSERE (PNUMERO VARCHAR(100)," +
                " PMENSAGEM BLOB SUB_TYPE TEXT, PDATA TIMESTAMP, PENVIADA VARCHAR(100), PCNPJ VAR" +
                "CHAR(50)) AS BEGIN INSERT INTO TRI_PDV_WHATS (NUMERO, MENSAGEM, DATAENVIO, ENVIA" +
                "DA,CNPJ) VALUES (:PNUMERO, :PMENSAGEM,:PDATA,:PENVIADA,:PCNPJ); END\';\r\n\r\n\t\t\t\t\t\te" +
                "rro = \'sproc SP_TRI_RENDIMENTO_SOMA\';\r\n\t\t\t\t\t\texecute statement \'CREATE OR ALTER " +
                "PROCEDURE SP_TRI_RENDIMENTO_SOMA (PDATA_INICIAL TIMESTAMP, PDATA_FINAL TIMESTAMP" +
                ", CAIXA VARCHAR(3)) RETURNS (RSOMA DECIMAL(18,4), RDESCRICAO VARCHAR(30)) AS BEG" +
                "IN FOR SELECT SUM(tni.VLR_TOTAL - tni.VLR_DESC),tetis.DESCRICAO FROM TB_NFV_ITEM" +
                " tni JOIN TB_NFVENDA tn ON tni.ID_NFVENDA = tn.ID_NFVENDA JOIN TB_EST_IDENTIFICA" +
                "DOR tei ON tni.ID_IDENTIFICADOR = tei.ID_IDENTIFICADOR JOIN TB_ESTOQUE te ON tei" +
                ".ID_ESTOQUE = te.ID_ESTOQUE JOIN TB_EST_TIPO_ITEM_SIS tetis ON te.ID_TIPOITEM = " +
                "tetis.ID_TIPOITEM WHERE (tn.NF_SERIE = \'\'N\'\'||:CAIXA OR tn.NF_SERIE = :CAIXA) AN" +
                "D tn.NF_MODELO = \'\'59\'\' AND tn.STATUS = \'\'I\'\' AND CAST ((tn.DT_SAIDA || \'\' \'\' ||" +
                " tn.HR_SAIDA) AS TIMESTAMP) BETWEEN :PDATA_INICIAL AND :PDATA_FINAL GROUP BY tet" +
                "is.DESCRICAO INTO :RSOMA, :RDESCRICAO DO BEGIN SUSPEND; END END\';\r\n\r\n\t\t\t\t\t\terro " +
                "= \'sproc SP_TRI_PROMOCOES_UPSERT\';\r\n\t\t\t\t\t\texecute statement \'CREATE OR ALTER PRO" +
                "CEDURE SP_TRI_PROMOCOES_UPSERT (PID INTEGER, PQTD NUMERIC(15,2), PTIPO VARCHAR(2" +
                "0), PDET NUMERIC(15,2), PLIMITE INTEGER, PINICIO DATE, PFIM DATE, P_REJEITADA IN" +
                "TEGER, PSCANNTECH INTEGER, P_EXECUTA CHAR(1)) RETURNS ( RROWSAFFECTED INTEGER ) " +
                "AS BEGIN IF(:P_EXECUTA = \'\'U\'\' OR :P_EXECUTA = \'\'I\'\') THEN  BEGIN UPDATE OR INSE" +
                "RT INTO TB_PROMOCOES (ID , QTD , TIPO , DET , LIMITE , INICIO , FIM , REJEITADA " +
                ", SCANNTECH) VALUES (:PID , :PQTD , :PTIPO , :PDET , :PLIMITE , :PINICIO , :PFIM" +
                " , :P_REJEITADA , :PSCANNTECH) ; END IF(:P_EXECUTA = \'\'D\'\') THEN BEGIN DELETE FR" +
                "OM TB_PROMOCOES WHERE ID = :PID; END rRowsAffected = ROW_COUNT; END\';\r\n\r\n\t\t\t\t\t\te" +
                "rro = \'sproc SP_TRI_PROMOCOES_ITENS_UPSERT\';\r\n\t\t\t\t\t\texecute statement \'CREATE OR" +
                " ALTER PROCEDURE SP_TRI_PROMOCOES_ITENS_UPSERT (PID INTEGER, PIDPROMOCAO INTEGER" +
                ", PPRODUTONOME VARCHAR(150), PCODIGOBARRAS VARCHAR(18), P_EXECUTA CHAR(1)) RETUR" +
                "NS ( RROWSAFFECTED INTEGER ) AS BEGIN IF(:P_EXECUTA = \'\'U\'\' OR :P_EXECUTA = \'\'I\'" +
                "\') THEN BEGIN UPDATE OR INSERT INTO TB_PROMOCOES_ITENS (ID , IDPROMOCAO , PRODUT" +
                "ONOME , CODIGOBARRAS) VALUES (:PID , :PIDPROMOCAO , :PPRODUTONOME , :PCODIGOBARR" +
                "AS) ; END IF(:P_EXECUTA = \'\'D\'\') THEN BEGIN DELETE FROM TB_PROMOCOES_ITENS WHERE" +
                " ID = :PID; END rRowsAffected = ROW_COUNT; END\';\r\n\r\n\t\t\t\t\t\terro = \'sproc SP_TRI_O" +
                "BTEMPROMOSCANNTECH\';\r\n\t\t\t\t\t\texecute statement \'CREATE OR ALTER PROCEDURE SP_TRI_" +
                "OBTEMPROMOSCANNTECH (ID_SCANNTECH INTEGER) RETURNS ( QTD NUMERIC(15,2), TIPO VAR" +
                "CHAR(20), DET NUMERIC(15,2), LIMITE INTEGER ) AS DECLARE VARIABLE VEXISTE_PROMO " +
                "INTEGER; BEGIN SELECT COUNT(1) FROM TB_PROMOCOES TP INNER JOIN TB_PROMOCOES_ITEN" +
                "S TPI ON TPI.IDPROMOCAO = TP.ID  WHERE CURRENT_DATE BETWEEN TP.INICIO AND TP.FIM" +
                "  AND TP.ID = :ID_SCANNTECH AND TP.REJEITADA = 0 INTO VEXISTE_PROMO; IF(VEXISTE_" +
                "PROMO > 0) THEN BEGIN FOR SELECT FIRST (1) TP.QTD, TP.TIPO, TP.DET, TP.LIMITE FR" +
                "OM TB_PROMOCOES TP INNER JOIN TB_PROMOCOES_ITENS TPI ON TPI.IDPROMOCAO = TP.ID  " +
                "WHERE (TP.ID = :ID_SCANNTECH) INTO :QTD, :TIPO, :DET, :LIMITE DO SUSPEND; END EN" +
                "D\';\r\n\r\n                                                                         " +
                "                                       erro = \'sproc SP_TRI_UPDATE_NF_TO_F\';\r\n\t\t" +
                "\t\t\t\texecute statement \'CREATE OR ALTER PROCEDURE SP_TRI_UPDATE_NF_TO_F (PNEW_NF_" +
                "NUMERO INTEGER, PNEW_NF_SERIE VARCHAR(3), PNEW_DT_SAIDA DATE, PNEW_HR_SAIDA TIME" +
                ", PSYNCED SMALLINT, POLD_NF_NUMERO INTEGER, POLD_NF_SERIE VARCHAR(3), POLD_NF_MO" +
                "DELO VARCHAR(3)) RETURNS ( RID_ALTERADO INTEGER ) AS BEGIN  UPDATE TB_NFVENDA SE" +
                "T NF_NUMERO = :PNEW_NF_NUMERO, NF_SERIE = :PNEW_NF_SERIE, DT_SAIDA = :PNEW_DT_SA" +
                "IDA, HR_SAIDA = :PNEW_HR_SAIDA, SYNCED = :PSYNCED WHERE NF_NUMERO = :POLD_NF_NUM" +
                "ERO AND NF_SERIE = :POLD_NF_SERIE AND NF_MODELO = :POLD_NF_MODELO; FOR SELECT ID" +
                "_NFVENDA FROM TB_NFVENDA WHERE NF_NUMERO = :PNEW_NF_NUMERO AND NF_SERIE = :PNEW_" +
                "NF_SERIE AND NF_MODELO = :POLD_NF_MODELO INTO :RID_ALTERADO DO  BEGIN  SUSPEND; " +
                "END END\';\r\n\r\n\t\t\t\t\t\terro = \'deu certo\';\r\n\r\n\t\t\t\t\t\tSUSPEND;\r\n\t\t\t\t\t\tWHEN ANY DO\r\n\t\t\t" +
                "\t\t\tBEGIN\r\n\t\t\t\t\t\tEND\r\n\t\t\t\t\t\tEND;";
