...
END;";
            ...
            this._commandCollection[21].CommandText = "\r\n\t\t\t\t\t\tCREATE OR ALTER PROCEDURE SP_TRI_PROCEDURES7\r\n\t\t\t\t\t\tRETURNS\r\n\t\t\t\t\t\t(\r\n\t\t\t" +
                ...
                "_BY_IDNFV (PIDNFVENDA TYPE OF COLUMN TB_NFVENDA.ID_NFVENDA) RETURNS ( ID_NUMPAG " +
                "TYPE OF COLUMN TB_NFVENDA_FMAPAGTO_NFCE.ID_NUMPAG, VLR_PAGTO TYPE OF COLUMN TB_N" +
                "FVENDA_FMAPAGTO_NFCE.VLR_PAGTO, ID_NFVENDA TYPE OF COLUMN TB_NFVENDA.ID_NFVENDA," +
                " ID_FMANFCE TYPE OF COLUMN TB_FORMA_PAGTO_NFCE.ID_FMANFCE, ID_PARCELA TYPE OF CO" +
                "LUMN TB_PARCELAMENTO.ID_PARCELA, TROCO TYPE OF COLUMN TB_NFVENDA_FMAPAGTO_NFCE.P" +
                "IX_COD_AUTORIZACAO ) AS BEGIN FOR SELECT a.ID_NUMPAG , a.VLR_PAGTO , a.ID_NFVEND" +
                "A , a.ID_FMANFCE , a.ID_PARCELA, a.PIX_COD_AUTORIZACAO FROM TB_NFVENDA_FMAPAGTO_" +
                "NFCE a WHERE a.ID_NFVENDA =:pIdNfvenda INTO :ID_NUMPAG , :VLR_PAGTO , :ID_NFVEND" +
                "A , :ID_FMANFCE , :ID_PARCELA , :TROCO DO SUSPEND; END;\';\r\n\r\n\t\t\t\t\t\terro = \'sproc" +
                " SP_TRI_NFV_FMAPAGT_SYNC_INSERT\';\r\n\t\t\t\t\t\texecute statement \'CREATE OR ALTER PROC" +
                "EDURE SP_TRI_NFV_FMAPAGT_SYNC_INSERT (PVLR_PAGTO TYPE OF COLUMN TB_NFVENDA_FMAPA" +
                "GTO_NFCE.VLR_PAGTO, PID_NFVENDA TYPE OF COLUMN TB_NFVENDA.ID_NFVENDA, PID_FMANFC" +
                "E TYPE OF COLUMN TB_FORMA_PAGTO_NFCE.ID_FMANFCE, PID_PARCELA TYPE OF COLUMN TB_P" +
                "ARCELAMENTO.ID_PARCELA, PTROCO TYPE OF COLUMN TB_NFVENDA_FMAPAGTO_NFCE.PIX_COD_A" +
                "UTORIZACAO, PID_ADMINISTRADORA TYPE OF COLUMN TB_CARTAO_ADMINISTRADORA.ID_ADMINI" +
                "STRADORA) RETURNS ( RNEWIDNUMPAG INTEGER ) AS DECLARE VARIABLE newIdNumpag TYPE " +
                "OF COLUMN TB_NFVENDA_FMAPAGTO_NFCE.ID_NUMPAG; BEGIN SELECT NEXT VALUE FOR gen_tb" +
                "_nfvenda_fmapagto_nfce_id FROM RDB$DATABASE INTO :newIdNumpag ; IF( :newIdNumpag" +
                " IS NULL ) THEN newIdNumpag = 0; BEGIN INSERT INTO TB_NFVENDA_FMAPAGTO_NFCE (ID_" +
                "NUMPAG , VLR_PAGTO , ID_NFVENDA , ID_FMANFCE , ID_PARCELA, PIX_COD_AUTORIZACAO) " +
                "VALUES ( :newIdNumpag , :pVLR_PAGTO , :pID_NFVENDA , :pID_FMANFCE , :pID_PARCELA" +
                ", :pTROCO) ; INSERT INTO TB_NFCE_BANDEIRA (ID_NUMPAG , ID_BANDEIRA , ID_ADMINIST" +
                "RADORA , NUM_AUT , FORMA_ENVIO , SICLOS_ID , STONE_ID , PRE_TRANSACAO_ID , POS_S" +
                "ERIAL) VALUES (:newIdNumpag , NULL, :pID_ADMINISTRADORA , NULL , NULL , NULL , N" +
                "ULL , NULL , NULL) ; rNewIdNumpag = newIdNumpag; END END;\';\r\n\r\n\t\t\t\t\t\terro = \'spr" +
                "oc SP_TRI_NFVITEM_GETBY_IDNFVENDA\';\r\n\t\t\t\t\t\texecute statement \'CREATE OR ALTER PR" +
                "OCEDURE SP_TRI_NFVITEM_GETBY_IDNFVENDA (PIDNFVENDA INTEGER) RETURNS ( ID_NFVITEM" +
                " INTEGER, ID_NFVENDA INTEGER, ID_IDENTIFICADOR INTEGER, CFOP VARCHAR(4), NUM_ITE" +
                "M SMALLINT, QTD_ITEM NUMERIC(18,4), UNI_MEDIDA VARCHAR(3), VLR_TOTAL NUMERIC(18," +
                "4), VLR_DESC NUMERIC(18,4), VLR_CUSTO NUMERIC(18,4), PRC_LISTA NUMERIC(18,4), CF" +
                " VARCHAR(13), VLR_FRETE NUMERIC(18,4), VLR_SEGURO NUMERIC(18,4), VLR_DESPESA NUM" +
                "ERIC(18,4), RET_PIS_COF_CSLL NUMERIC(18,4), RET_IRRF NUMERIC(18,4), COD_ENQ VARC" +
                "HAR(3), COD_BASE VARCHAR(2), CSOSN VARCHAR(3), NPED_COMPRA VARCHAR(15), ITEM_COM" +
                "PRA INTEGER, VLR_TOTTRIB NUMERIC(18,4), FCI VARCHAR(36), VLR_ICM_DESO NUMERIC(18" +
                ",4), ID_MOTIVO_DESO INTEGER, EST_BX CHAR(1), VLR_TRIB_FED NUMERIC(18,4), VLR_TRI" +
                "B_EST NUMERIC(18,4), VLR_TRIB_MUN NUMERIC(18,4), INCLUIR_FATURA CHAR(1), VLR_UNI" +
                "T NUMERIC(18,4), VLR_RETENCAO NUMERIC(18,4), REFERENCIA VARCHAR(18), ID_COMPOSIC" +
                "AO INTEGER, CODPROMOSCANNTECH INTEGER ) AS BEGIN FOR SELECT COALESCE(a.ID_NFVITE" +
                "M, 0) ID_NFVITEM , a.ID_NFVENDA , a.ID_IDENTIFICADOR , a.CFOP , a.NUM_ITEM , a.Q" +
                "TD_ITEM , a.UNI_MEDIDA , a.VLR_TOTAL , a.VLR_DESC , a.VLR_CUSTO , a.PRC_LISTA , " +
                "a.CF , a.VLR_FRETE , a.VLR_SEGURO , a.VLR_DESPESA , a.RET_PIS_COF_CSLL , a.RET_I" +
                "RRF , a.COD_ENQ , a.COD_BASE , a.CSOSN , a.NPED_COMPRA , a.ITEM_COMPRA , a.VLR_T" +
                "OTTRIB , a.FCI , a.VLR_ICM_DESO , a.ID_MOTIVO_DESO , a.EST_BX , a.VLR_TRIB_FED ," +
                " a.VLR_TRIB_EST , a.VLR_TRIB_MUN , a.INCLUIR_FATURA , a.VLR_UNIT , a.VLR_RETENCA" +
                "O , a.REFERENCIA , e.ID_COMPOSICAO, a.CODPROMOSCANNTECH FROM TB_NFV_ITEM a LEFT " +
                "JOIN TB_EST_COMPOSICAO e ON e.ID_IDENTIFICADOR = a.ID_IDENTIFICADOR WHERE a.ID_N" +
                "FVENDA =:pIdNfvenda INTO :ID_NFVITEM , :ID_NFVENDA , :ID_IDENTIFICADOR , :CFOP ," +
                " :NUM_ITEM , :QTD_ITEM , :UNI_MEDIDA , :VLR_TOTAL , :VLR_DESC , :VLR_CUSTO , :PR" +
                "C_LISTA , :CF , :VLR_FRETE , :VLR_SEGURO , :VLR_DESPESA , :RET_PIS_COF_CSLL , :R" +
                "ET_IRRF , :COD_ENQ , :COD_BASE , :CSOSN , :NPED_COMPRA , :ITEM_COMPRA , :VLR_TOT" +
                "TRIB , :FCI , :VLR_ICM_DESO , :ID_MOTIVO_DESO , :EST_BX , :VLR_TRIB_FED , :VLR_T" +
                "RIB_EST , :VLR_TRIB_MUN , :INCLUIR_FATURA , :VLR_UNIT , :VLR_RETENCAO , :REFEREN" +
                "CIA , :ID_COMPOSICAO, :CODPROMOSCANNTECH DO SUSPEND; END;\';\r\n\r\n\t\t\t\t\t\terro = \'spr" +
                "oc SP_TRI_NFVITEM_SYNC_INSERT\';\r\n\t\t\t\t\t\texecute statement \'CREATE OR ALTER PROCED" +
                "URE SP_TRI_NFVITEM_SYNC_INSERT (PID_NFVENDA INTEGER, PID_IDENTIFICADOR INTEGER, " +
                "PCFOP VARCHAR(4), PNUM_ITEM SMALLINT, PQTD_ITEM NUMERIC(18,4), PUNI_MEDIDA VARCH" +
                "AR(3), PVLR_TOTAL NUMERIC(18,4), PVLR_DESC NUMERIC(18,4), PVLR_CUSTO NUMERIC(18," +
                "4), PPRC_LISTA NUMERIC(18,4), PCF VARCHAR(13), PVLR_FRETE NUMERIC(18,4), PVLR_SE" +
                "GURO NUMERIC(18,4), PVLR_DESPESA NUMERIC(18,4), PRET_PIS_COF_CSLL NUMERIC(18,4)," +
                " PRET_IRRF NUMERIC(18,4), PCOD_ENQ VARCHAR(3), PCOD_BASE VARCHAR(2), PCSOSN VARC" +
                "HAR(3), PNPED_COMPRA VARCHAR(15), PITEM_COMPRA INTEGER, PVLR_TOTTRIB NUMERIC(18," +
                "4), PFCI VARCHAR(36), PVLR_ICM_DESO NUMERIC(18,4), PID_MOTIVO_DESO INTEGER, PEST" +
                "_BX CHAR(1), PVLR_TRIB_FED NUMERIC(18,4), PVLR_TRIB_EST NUMERIC(18,4), PVLR_TRIB" +
                "_MUN NUMERIC(18,4), PINCLUIR_FATURA CHAR(1), PVLR_UNIT NUMERIC(18,4), PVLR_RETEN" +
                "CAO NUMERIC(18,4), PREFERENCIA VARCHAR(18), PCOD_SCANNTECH INTEGER) RETURNS (  R" +
                "NEWIDNFVITEM INTEGER ) AS DECLARE VARIABLE newIdNfvItem INTEGER; BEGIN  SELECT  " +
                "NEXT VALUE FOR GEN_TB_NFV_ITEM_ID FROM  RDB$DATABASE INTO  :newIdNfvItem ;  IF( " +
                ...
                "  UNI_MEDIDA ,  VLR_TOTAL ,  VLR_DESC ,  VLR_CUSTO ,  PRC_LISTA ,  CF ,  VLR_FRE" +
                "TE ,  VLR_SEGURO ,  VLR_DESPESA ,  RET_PIS_COF_CSLL ,  RET_IRRF ,  COD_ENQ ,  CO" +
                "D_BASE ,  CSOSN ,  NPED_COMPRA ,  ITEM_COMPRA ,  VLR_TOTTRIB ,  FCI ,  VLR_ICM_D" +
                "ESO ,  ID_MOTIVO_DESO ,  EST_BX ,  VLR_TRIB_FED ,  VLR_TRIB_EST ,  VLR_TRIB_MUN " +
                ",  INCLUIR_FATURA ,  VLR_UNIT ,  VLR_RETENCAO ,  REFERENCIA,   CODPROMOSCANNTECH" +
                ") VALUES ( :newIdNfvItem , :pID_NFVENDA , :pID_IDENTIFICADOR , :pCFOP , :pNUM_IT" +
                "EM , :pQTD_ITEM , :pUNI_MEDIDA , :pVLR_TOTAL , :pVLR_DESC , :pVLR_CUSTO , :pPRC_" +
                "LISTA , :pCF , :pVLR_FRETE , :pVLR_SEGURO , :pVLR_DESPESA , :pRET_PIS_COF_CSLL ," +
                " :pRET_IRRF , :pCOD_ENQ , :pCOD_BASE , :pCSOSN , :pNPED_COMPRA , :pITEM_COMPRA ," +
                " :pVLR_TOTTRIB , :pFCI , :pVLR_ICM_DESO , :pID_MOTIVO_DESO , :pEST_BX , :pVLR_TR" +
                "IB_FED , :pVLR_TRIB_EST , :pVLR_TRIB_MUN , :pINCLUIR_FATURA , :pVLR_UNIT , :pVLR" +
                "_RETENCAO , :pREFERENCIA,  :pCOD_SCANNTECH) ;  rnewIdNfvItem = newIdNfvItem; END" +
                " END;\';\r\n\r\n\t\t\t\t\t\terro = \'sproc SP_TRI_CTAREC_GETBY_IDNFVENDA\';\r\n\t\t\t\t\t\texecute st" +
                "atement \'CREATE OR ALTER PROCEDURE SP_TRI_CTAREC_GETBY_IDNFVENDA (PIDNFVENDA INT" +
                "EGER) RETURNS ( ID_CTAREC INTEGER, DOCUMENTO VARCHAR(12), HISTORICO VARCHAR(50)," +
                " DT_EMISSAO DATE, DT_VENCTO DATE, VLR_CTAREC NUMERIC(18,4), TIP_CTAREC CHAR(1), " +
                "ID_PORTADOR INTEGER, ID_CLIENTE INTEGER, INV_REFERENCIA VARCHAR(18), DT_VENCTO_O" +
                "RIG DATE, NSU_CARTAO VARCHAR(32), ID_NUMPAG INTEGER, ID_CONTA INTEGER ) AS BEGIN" +
                " FOR SELECT COALESCE(a.ID_CTAREC, 0) , a.DOCUMENTO , a.HISTORICO , a.DT_EMISSAO " +
                ", a.DT_VENCTO , a.VLR_CTAREC , a.TIP_CTAREC , a.ID_PORTADOR , a.ID_CLIENTE , a.I" +
                "NV_REFERENCIA , a.DT_VENCTO_ORIG , a.NSU_CARTAO , b.ID_NUMPAG , a.ID_CONTA FROM " +
                "TB_CONTA_RECEBER a JOIN TB_NFV_CTAREC b ON a.ID_CTAREC = b.ID_CTAREC WHERE b.ID_" +
                "NFVENDA =:pIdNfvenda INTO :ID_CTAREC , :DOCUMENTO , :HISTORICO , :DT_EMISSAO , :" +
                "DT_VENCTO , :VLR_CTAREC , :TIP_CTAREC , :ID_PORTADOR , :ID_CLIENTE , :INV_REFERE" +
                "NCIA , :DT_VENCTO_ORIG , :NSU_CARTAO , :ID_NUMPAG, :ID_CONTA DO SUSPEND ; END;\';" +
                "\r\n\r\n\t\t\t\t\t\terro = \'sproc SP_TRI_NFVENDA_SETSYNCED\';\r\n\t\t\t\t\t\texecute statement \'CRE" +
                "ATE OR ALTER PROCEDURE SP_TRI_NFVENDA_SETSYNCED ( pIdNfvenda INTEGER , pSynced S" +
                "MALLINT ) AS BEGIN UPDATE TB_NFVENDA SET SYNCED = :pSynced WHERE ID_NFVENDA = :p" +
                "IdNfvenda ; END;\';\r\n\r\n\t\t\t\t\t\terro = \'sproc SP_TRI_CTAREC_GETBY_NFNUMSERIE\';\r\n\t\t\t\t" +
                "\t\texecute statement \'CREATE OR ALTER PROCEDURE SP_TRI_CTAREC_GETBY_NFNUMSERIE ( " +
                "pNF_NUMERO TYPE OF COLUMN TB_NFVENDA.NF_NUMERO , pNF_SERIE TYPE OF COLUMN TB_NFV" +
                "ENDA.NF_SERIE ) RETURNS ( ID_CTAREC INTEGER , DOCUMENTO VARCHAR(12) , HISTORICO " +
                "VARCHAR(50) , DT_EMISSAO DATE , DT_VENCTO DATE , VLR_CTAREC TYPE OF COLUMN TRI_P" +
                "DV_OPER.DIN , TIP_CTAREC CHAR(1) , ID_PORTADOR INTEGER , ID_CLIENTE INTEGER , IN" +
                "V_REFERENCIA VARCHAR(18) , DT_VENCTO_ORIG DATE , NSU_CARTAO VARCHAR(32) ) AS BEG" +
                "IN FOR SELECT a.ID_CTAREC , a.DOCUMENTO , a.HISTORICO , a.DT_EMISSAO , a.DT_VENC" +
                "TO , a.VLR_CTAREC , a.TIP_CTAREC , a.ID_PORTADOR , a.ID_CLIENTE , a.INV_REFERENC" +
                "IA , a.DT_VENCTO_ORIG , a.NSU_CARTAO FROM TB_CONTA_RECEBER a JOIN TB_NFV_CTAREC " +
                "b ON a.ID_CTAREC = b.ID_CTAREC JOIN TB_NFVENDA c ON c.ID_NFVENDA = b.ID_NFVENDA " +
                "WHERE c.NF_NUMERO = :pNF_NUMERO AND c.NF_SERIE = :pNF_SERIE INTO :ID_CTAREC , :D" +
                "OCUMENTO , :HISTORICO , :DT_EMISSAO , :DT_VENCTO , :VLR_CTAREC , :TIP_CTAREC , :" +
                "ID_PORTADOR , :ID_CLIENTE , :INV_REFERENCIA , :DT_VENCTO_ORIG , :NSU_CARTAO DO S" +
                "USPEND ; END;\';\r\n\r\n\t\t\t\t\t\terro = \'sproc SP_TRI_NFV_CTAREC_SYNC_DEL\';\r\n\t\t\t\t\t\texecu" +
                "te statement \'CREATE OR ALTER PROCEDURE SP_TRI_NFV_CTAREC_SYNC_DEL ( pNfNumero T" +
                "YPE OF COLUMN TB_NFVENDA.NF_NUMERO , pNfSerie TYPE OF COLUMN TB_NFVENDA.NF_SERIE" +
                " , pIdCtarec TYPE OF COLUMN TB_NFV_CTAREC.ID_CTAREC ) AS DECLARE VARIABLE idNfve" +
                "nda TYPE OF COLUMN TB_NFVENDA.ID_NFVENDA; BEGIN SELECT ID_NFVENDA FROM TB_NFVEND" +
                "A WHERE NF_NUMERO = :pNfNumero AND NF_SERIE = :pNfSerie AND NF_MODELO = 59 INTO " +
                ":idNfvenda ; DELETE FROM TB_NFV_CTAREC WHERE ID_NFVENDA = :idNfvenda AND ID_CTAR" +
                "EC= :pIdCtarec ; END;\';\r\n\r\n\t\t\t\t\t\terro = \'sproc SP_TRI_NFV_UPDT_BYNFNUMSERIE\';\r\n\t" +
                "\t\t\t\t\texecute statement \'CREATE OR ALTER PROCEDURE SP_TRI_NFV_UPDT_BYNFNUMSERIE (" +
                "PID_NATOPE INTEGER, PID_VENDEDOR SMALLINT, PID_CLIENTE INTEGER, PNF_NUMERO INTEG" +
                "ER, PNF_SERIE VARCHAR(3), PNF_MODELO VARCHAR(2), PDT_EMISSAO DATE, PDT_SAIDA DAT" +
                "E, PHR_SAIDA TIME, PESPECIE VARCHAR(15), PPES_LIQUID NUMERIC(18,4), PPES_BRUTO N" +
                "UMERIC(18,4), PID_FMAPGTO SMALLINT, PID_PARCELA SMALLINT, PMARCA VARCHAR(15), PQ" +
                "TD_VOLUM NUMERIC(18,4), PNUM_VOLUM VARCHAR(15), PVLR_TROCO NUMERIC(18,4), PIND_P" +
                "RES CHAR(1), PIND_IE_DEST CHAR(1), PDESCONTO_CONDICIONAL CHAR(1), PINF_COMP_FIXA" +
                " BLOB, PINF_COMP_EDIT BLOB, PENDERECO_ENTREGA CHAR(1), PENVIO_API TIMESTAMP, PSY" +
                "NCED SMALLINT, PSOMA_FRETE CHAR(1) DEFAULT \'\'N\'\', PPROD_REV CHAR(1) DEFAULT \'\'N\'" +
                "\', PSTATUS CHAR(1) DEFAULT \'\'E\'\', PENT_SAI CHAR(1) DEFAULT \'\'1\'\', PTIPO_FRETE CH" +
                "AR(1) DEFAULT \'\'9\'\') AS BEGIN UPDATE TB_NFVENDA SET ID_NATOPE =:pID_NATOPE , ID_" +
                "VENDEDOR =:pID_VENDEDOR , ID_CLIENTE =:pID_CLIENTE , NF_MODELO =:pNF_MODELO , DT" +
                "_EMISSAO =:pDT_EMISSAO , DT_SAIDA =:pDT_SAIDA , HR_SAIDA =:pHR_SAIDA , ESPECIE =" +
                ":pESPECIE , TIPO_FRETE =:pTIPO_FRETE , PES_LIQUID =:pPES_LIQUID , PES_BRUTO =:pP" +
                "ES_BRUTO , STATUS =:pSTATUS , ENT_SAI =:pENT_SAI , ID_FMAPGTO =:pID_FMAPGTO , ID" +
                "_PARCELA =:pID_PARCELA , MARCA =:pMARCA , QTD_VOLUM =:pQTD_VOLUM , NUM_VOLUM =:p" +
                "NUM_VOLUM , PROD_REV =:pPROD_REV , SOMA_FRETE =:pSOMA_FRETE , VLR_TROCO =:pVLR_T" +
                "ROCO , IND_PRES =:pIND_PRES , IND_IE_DEST =:pIND_IE_DEST , DESCONTO_CONDICIONAL=" +
                ":pDESCONTO_CONDICIONAL , INF_COMP_FIXA =:pINF_COMP_FIXA , INF_COMP_EDIT =:pINF_C" +
                "OMP_EDIT , ENDERECO_ENTREGA =:pENDERECO_ENTREGA , ENVIO_API =:pENVIO_API , SYNCE" +
                "D =:pSYNCED WHERE NF_NUMERO = :pNF_NUMERO and NF_SERIE = :pNF_SERIE ; END;\';\r\n\r\n" +
                "\t\t\t\t\t\terro = \'sproc SP_TRI_SAT_GET_BY_IDNFV\';\r\n\t\t\t\t\t\texecute statement \'CREATE O" +
                "R ALTER PROCEDURE SP_TRI_SAT_GET_BY_IDNFV ( pIdNfvenda TYPE OF COLUMN TB_SAT.ID_" +
                "NFVENDA ) RETURNS ( ID_REGISTRO TYPE OF COLUMN TB_SAT.ID_REGISTRO , ID_NFVENDA T" +
                "YPE OF COLUMN TB_SAT.ID_NFVENDA , CHAVE TYPE OF COLUMN TB_SAT.CHAVE , DT_EMISSAO" +
                " TYPE OF COLUMN TB_SAT.DT_EMISSAO , HR_EMISSAO TYPE OF COLUMN TB_SAT.HR_EMISSAO " +
                ", STATUS TYPE OF COLUMN TB_SAT.STATUS , STATUS_DES TYPE OF COLUMN TB_SAT.STATUS_" +
                "DES , NUMERO_CFE TYPE OF COLUMN TB_SAT.NUMERO_CFE , NUM_SERIE_SAT TYPE OF COLUMN" +
                " TB_SAT.NUM_SERIE_SAT) AS BEGIN FOR SELECT a.ID_REGISTRO , a.ID_NFVENDA , a.CHAV" +
                "E , a.DT_EMISSAO , a.HR_EMISSAO , a.STATUS , a.STATUS_DES , a.NUMERO_CFE , a.NUM" +
                "_SERIE_SAT FROM TB_SAT a WHERE a.ID_NFVENDA =:pIdNfvenda INTO :ID_REGISTRO , :ID" +
                "_NFVENDA , :CHAVE , :DT_EMISSAO , :HR_EMISSAO , :STATUS , :STATUS_DES , :NUMERO_" +
                "CFE , :NUM_SERIE_SAT DO SUSPEND ; END;\';\r\n\r\n\t\t\t\t\t\terro = \'deu certo\';\r\n\r\n\t\t\t\t\t\tS" +
                "USPEND;\r\n\t\t\t\t\t\tWHEN ANY DO\r\n\t\t\t\t\t\tBEGIN\r\n\t\t\t\t\t\tEND\r\n\t\t\t\t\t\tEND;";
            ...
            this._commandCollection[22].CommandText = "CREATE OR ALTER PROCEDURE SP_TRI_PROCEDURES8\r\nRETURNS\r\n(\r\nerro VARCHAR(64)\r\n)\r\nAS" +
                ...
                "B_NFVENDA_FMAPAGTO_NFCE.VLR_PAGTO, PID_NFVENDA TYPE OF COLUMN TB_NFVENDA.ID_NFVE" +
                "NDA, PID_FMANFCE TYPE OF COLUMN TB_FORMA_PAGTO_NFCE.ID_FMANFCE, PID_PARCELA TYPE" +
                " OF COLUMN TB_PARCELAMENTO.ID_PARCELA, PTROCO TYPE OF COLUMN TB_NFVENDA_FMAPAGTO" +
                "_NFCE.PIX_COD_AUTORIZACAO, PID_ADMINISTRADORA TYPE OF COLUMN TB_CARTAO_ADMINISTR" +
                "ADORA.ID_ADMINISTRADORA) RETURNS ( RID_FMAPAGTO INTEGER ) AS BEGIN SELECT NEXT V" +
                "ALUE FOR gen_tb_nfvenda_fmapagto_nfce_id FROM RDB$DATABASE INTO :rID_FMAPAGTO; I" +
                "NSERT INTO TB_NFVENDA_FMAPAGTO_NFCE (ID_NUMPAG, VLR_PAGTO, ID_NFVENDA, ID_FMANFC" +
                "E, ID_PARCELA, PIX_COD_AUTORIZACAO) VALUES (:rID_FMAPAGTO, :pVALOR, :pID_NFVENDA" +
                ", :pID_FMANFCE, :pID_PARCELA, :pTROCO); INSERT INTO TB_NFCE_BANDEIRA (ID_NUMPAG," +
                " ID_BANDEIRA, ID_ADMINISTRADORA, NUM_AUT, FORMA_ENVIO, SICLOS_ID, STONE_ID, PRE_" +
                "TRANSACAO_ID, POS_SERIAL) VALUES (:rID_FMAPAGTO, NULL, :pID_ADMINISTRADORA, NULL" +
                ", NULL, NULL, NULL, NULL, NULL); END;\';\r\n\r\n\t\t\t\t\t\terro = \'sproc SP_TRI_WHATSINSER" +
                "E\';\r\n\t\t\t\t\t\texecute statement \'CREATE OR ALTER PROCEDURE SP_TRI_WHATSINSERE (PNUM" +
                "ERO VARCHAR(100), PMENSAGEM BLOB SUB_TYPE TEXT, PDATA TIMESTAMP, PENVIADA VARCHA" +
                "R(100), PCNPJ VARCHAR(50)) AS BEGIN INSERT INTO TRI_PDV_WHATS (NUMERO, MENSAGEM," +
                " DATAENVIO, ENVIADA,CNPJ) VALUES (:PNUMERO, :PMENSAGEM,:PDATA,:PENVIADA,:PCNPJ);" +
                " END\';\r\n\r\n\t\t\t\t\t\terro = \'sproc SP_TRI_RENDIMENTO_SOMA\';\r\n\t\t\t\t\t\texecute statement " +
                "\'CREATE OR ALTER PROCEDURE SP_TRI_RENDIMENTO_SOMA (PDATA_INICIAL TIMESTAMP, PDAT" +
                "A_FINAL TIMESTAMP, CAIXA VARCHAR(3)) RETURNS (RSOMA DECIMAL(18,4), RDESCRICAO VA" +
                "RCHAR(30)) AS BEGIN FOR SELECT SUM(tni.VLR_TOTAL - tni.VLR_DESC),tetis.DESCRICAO" +
                " FROM TB_NFV_ITEM tni JOIN TB_NFVENDA tn ON tni.ID_NFVENDA = tn.ID_NFVENDA JOIN " +
                "TB_EST_IDENTIFICADOR tei ON tni.ID_IDENTIFICADOR = tei.ID_IDENTIFICADOR JOIN TB_" +
                "ESTOQUE te ON tei.ID_ESTOQUE = te.ID_ESTOQUE JOIN TB_EST_TIPO_ITEM_SIS tetis ON " +
                "te.ID_TIPOITEM = tetis.ID_TIPOITEM WHERE (tn.NF_SERIE = \'\'N\'\'||:CAIXA OR tn.NF_S" +
                "ERIE = :CAIXA) AND tn.NF_MODELO = \'\'59\'\' AND tn.STATUS = \'\'I\'\' AND CAST ((tn.DT_" +
                "SAIDA || \'\' \'\' || tn.HR_SAIDA) AS TIMESTAMP) BETWEEN :PDATA_INICIAL AND :PDATA_F" +
                "INAL GROUP BY tetis.DESCRICAO INTO :RSOMA, :RDESCRICAO DO BEGIN SUSPEND; END END" +
                "\';\r\n\r\n\t\t\t\t\t\terro = \'sproc SP_TRI_PROMOCOES_UPSERT\';\r\n\t\t\t\t\t\texecute statement \'CR" +
                "EATE OR ALTER PROCEDURE SP_TRI_PROMOCOES_UPSERT (PID INTEGER, PQTD NUMERIC(15,2)" +
                ", PTIPO VARCHAR(20), PDET NUMERIC(15,2), PLIMITE INTEGER, PINICIO DATE, PFIM DAT" +
                "E, P_REJEITADA INTEGER, PSCANNTECH INTEGER, P_EXECUTA CHAR(1)) RETURNS ( RROWSAF" +
                "FECTED INTEGER ) AS BEGIN IF(:P_EXECUTA = \'\'U\'\' OR :P_EXECUTA = \'\'I\'\') THEN  BEG" +
                "IN UPDATE OR INSERT INTO TB_PROMOCOES (ID , QTD , TIPO , DET , LIMITE , INICIO ," +
                " FIM , REJEITADA , SCANNTECH) VALUES (:PID , :PQTD , :PTIPO , :PDET , :PLIMITE ," +
                " :PINICIO , :PFIM , :P_REJEITADA , :PSCANNTECH) ; END IF(:P_EXECUTA = \'\'D\'\') THE" +
                "N BEGIN DELETE FROM TB_PROMOCOES WHERE ID = :PID; END rRowsAffected = ROW_COUNT;" +
                " END\';\r\n\r\n\t\t\t\t\t\terro = \'sproc SP_TRI_PROMOCOES_ITENS_UPSERT\';\r\n\t\t\t\t\t\texecute sta" +
                "tement \'CREATE OR ALTER PROCEDURE SP_TRI_PROMOCOES_ITENS_UPSERT (PID INTEGER, PI" +
                "DPROMOCAO INTEGER, PPRODUTONOME VARCHAR(150), PCODIGOBARRAS VARCHAR(18), P_EXECU" +
                "TA CHAR(1)) RETURNS ( RROWSAFFECTED INTEGER ) AS BEGIN IF(:P_EXECUTA = \'\'U\'\' OR " +
                ":P_EXECUTA = \'\'I\'\') THEN BEGIN UPDATE OR INSERT INTO TB_PROMOCOES_ITENS (ID , ID" +
                "PROMOCAO , PRODUTONOME , CODIGOBARRAS) VALUES (:PID , :PIDPROMOCAO , :PPRODUTONO" +
                "ME , :PCODIGOBARRAS) ; END IF(:P_EXECUTA = \'\'D\'\') THEN BEGIN DELETE FROM TB_PROM" +
                "OCOES_ITENS WHERE ID = :PID; END rRowsAffected = ROW_COUNT; END\';\r\n\r\n\t\t\t\t\t\terro " +
                "= \'sproc SP_TRI_OBTEMPROMOSCANNTECH\';\r\n\t\t\t\t\t\texecute statement \'CREATE OR ALTER " +
                "PROCEDURE SP_TRI_OBTEMPROMOSCANNTECH (ID_SCANNTECH INTEGER) RETURNS ( QTD NUMERI" +
                "C(15,2), TIPO VARCHAR(20), DET NUMERIC(15,2), LIMITE INTEGER ) AS DECLARE VARIAB" +
                "LE VEXISTE_PROMO INTEGER; BEGIN SELECT COUNT(1) FROM TB_PROMOCOES TP INNER JOIN " +
                "TB_PROMOCOES_ITENS TPI ON TPI.IDPROMOCAO = TP.ID  WHERE CURRENT_DATE BETWEEN TP." +
                "INICIO AND TP.FIM  AND TP.ID = :ID_SCANNTECH AND TP.REJEITADA = 0 INTO VEXISTE_P" +
                "ROMO; IF(VEXISTE_PROMO > 0) THEN BEGIN FOR SELECT FIRST (1) TP.QTD, TP.TIPO, TP." +
                "DET, TP.LIMITE FROM TB_PROMOCOES TP INNER JOIN TB_PROMOCOES_ITENS TPI ON TPI.IDP" +
                "ROMOCAO = TP.ID  WHERE (TP.ID = :ID_SCANNTECH) INTO :QTD, :TIPO, :DET, :LIMITE D" +
                "O SUSPEND; END END\';\r\n\r\n                                                        " +
                "                                                        erro = \'sproc SP_TRI_UPD" +
                "ATE_NF_TO_F\';\r\n\t\t\t\t\t\texecute statement \'CREATE OR ALTER PROCEDURE SP_TRI_UPDATE_" +
                "NF_TO_F (PNEW_NF_NUMERO INTEGER, PNEW_NF_SERIE VARCHAR(3), PNEW_DT_SAIDA DATE, P" +
                "NEW_HR_SAIDA TIME, PSYNCED SMALLINT, POLD_NF_NUMERO INTEGER, POLD_NF_SERIE VARCH" +
                "AR(3), POLD_NF_MODELO VARCHAR(3)) RETURNS ( RID_ALTERADO INTEGER ) AS BEGIN  UPD" +
                "ATE TB_NFVENDA SET NF_NUMERO = :PNEW_NF_NUMERO, NF_SERIE = :PNEW_NF_SERIE, DT_SA" +
                "IDA = :PNEW_DT_SAIDA, HR_SAIDA = :PNEW_HR_SAIDA, SYNCED = :PSYNCED WHERE NF_NUME" +
                "RO = :POLD_NF_NUMERO AND NF_SERIE = :POLD_NF_SERIE AND NF_MODELO = :POLD_NF_MODE" +
                "LO; FOR SELECT ID_NFVENDA FROM TB_NFVENDA WHERE NF_NUMERO = :PNEW_NF_NUMERO AND " +
                "NF_SERIE = :PNEW_NF_SERIE AND NF_MODELO = :POLD_NF_MODELO INTO :RID_ALTERADO DO " +
