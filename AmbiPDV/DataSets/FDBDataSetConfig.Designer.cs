...
END;";
            ...
            this._commandCollection[21].CommandText = "CREATE OR ALTER PROCEDURE SP_TRI_PROCEDURES8\r\nRETURNS\r\n(\r\nerro VARCHAR(64)\r\n)\r\nAS" +
                ...
                " PDT_SAIDA DATE, PHR_SAIDA TIME, PID_FMAPGTO SMALLINT, PVLR_TROCO TYPE OF COLUMN" +
                " TB_NFVENDA.VLR_TROCO, PNCFE INTEGER) RETURNS (RNF_NUMERO TYPE OF COLUMN TB_NFVE" +
                "NDA.NF_NUMERO, rID_NFVENDA INTEGER) AS BEGIN SELECT NEXT VALUE FOR GEN_TB_NFVEND" +
                "A_ID FROM RDB$DATABASE INTO :rID_NFVENDA; IF (:pNCFE = -1) THEN SELECT MAX(NF_NU" +
                "MERO)+1 FROM TB_NFVENDA WHERE NF_MODELO = \'\'59\'\' AND NF_SERIE = :PNF_SERIE INTO " +
                ":rNF_NUMERO; ELSE rNF_NUMERO = :pNCFE; IF (:rNF_NUMERO IS NULL) THEN rNF_NUMERO " +
                "= 0; INSERT INTO TB_NFVENDA (ID_NFVENDA, ID_NATOPE, ID_VENDEDOR, ID_CLIENTE, NF_" +
                "NUMERO, NF_SERIE, NF_MODELO, DT_EMISSAO, DT_SAIDA, HR_SAIDA, ESPECIE, TIPO_FRETE" +
                ", PES_LIQUID, PES_BRUTO, STATUS, ENT_SAI, ID_FMAPGTO, ID_PARCELA, MARCA, QTD_VOL" +
                "UM, NUM_VOLUM, PROD_REV, SOMA_FRETE, VLR_TROCO, IND_PRES, IND_IE_DEST, DESCONTO_" +
                "CONDICIONAL, INF_COMP_FIXA, INF_COMP_EDIT, ENDERECO_ENTREGA, ENVIO_API) VALUES (" +
                ":rID_NFVENDA, 2, :pID_VENDEDOR, 0, :rNF_NUMERO, :pNF_SERIE, \'\'59\'\', CURRENT_DATE" +
                ", :pDT_SAIDA, :pHR_SAIDA, NULL, \'\'9\'\', 0, 0, \'\'E\'\', \'\'1\'\', :pID_FMAPGTO, 2, NULL" +
                ", 0, NULL, \'\'N\'\', \'\'N\'\', :pVLR_TROCO, NULL, NULL, \'\'S\'\', NULL, NULL, \'\'N\'\', NULL" +
                "); SUSPEND; END\';\r\n\r\nerro = \'sproc SP_TRI_GRAVANFVITEM\';\r\nexecute statement \'CRE" +
                "ATE OR ALTER PROCEDURE SP_TRI_GRAVANFVITEM (PID_NFVENDA INTEGER, PID_IDENTIFICAD" +
                "OR INTEGER, PNUM_ITEM SMALLINT, PQTD_ITEM TYPE OF COLUMN TB_NFV_ITEM.QTD_ITEM, P" +
                "VLR_DESC TYPE OF COLUMN TB_NFV_ITEM.VLR_DESC, PCSOSN VARCHAR(3), PVLR_TRIB_FED T" +
                "YPE OF COLUMN TB_NFV_ITEM.VLR_TRIB_FED, PVLR_TRIB_EST TYPE OF COLUMN TB_NFV_ITEM" +
                ".VLR_TRIB_EST, PVLR_UNIT TYPE OF COLUMN TB_NFV_ITEM.VLR_UNIT) RETURNS (RID_NFVIT" +
                "EM INTEGER) AS DECLARE VARIABLE vCFOP TYPE OF COLUMN TB_NFV_ITEM.CFOP; DECLARE V" +
                "ARIABLE vUNI_MEDIDA TYPE OF COLUMN TB_NFV_ITEM.UNI_MEDIDA; DECLARE VARIABLE vPRC" +
                "_CUSTO TYPE OF COLUMN TB_NFV_ITEM.VLR_CUSTO; BEGIN SELECT NEXT VALUE FOR GEN_TB_" +
                "NFV_ITEM_ID FROM RDB$DATABASE INTO :rID_NFVITEM; SELECT COALESCE (A.CFOP, \'\'5102" +
                "\'\'), A.UNI_MEDIDA, A.PRC_CUSTO FROM TB_ESTOQUE A JOIN TB_EST_IDENTIFICADOR B ON " +
                "A.ID_ESTOQUE = B.ID_ESTOQUE WHERE B.ID_IDENTIFICADOR = :pID_IDENTIFICADOR INTO :" +
                "vCFOP, :vUNI_MEDIDA, :vPRC_CUSTO; INSERT INTO TB_NFV_ITEM (ID_NFVITEM, ID_NFVEND" +
                "A, ID_IDENTIFICADOR, CFOP, NUM_ITEM, QTD_ITEM, UNI_MEDIDA, VLR_TOTAL, VLR_DESC, " +
                "VLR_CUSTO, PRC_LISTA, CF, VLR_FRETE, VLR_SEGURO, VLR_DESPESA, RET_PIS_COF_CSLL, " +
                "RET_IRRF, COD_ENQ, COD_BASE, CSOSN, NPED_COMPRA, ITEM_COMPRA, VLR_TOTTRIB, FCI, " +
                "VLR_ICM_DESO, ID_MOTIVO_DESO, EST_BX, VLR_TRIB_FED, VLR_TRIB_EST, VLR_TRIB_MUN, " +
                "INCLUIR_FATURA, VLR_UNIT, VLR_RETENCAO) VALUES (:rID_NFVITEM, :pID_NFVENDA, :pID" +
                "_IDENTIFICADOR, :vCFOP, :pNUM_ITEM, :pQTD_ITEM, :vUNI_MEDIDA, (:pVLR_UNIT * :pQT" +
                "D_ITEM), :pVLR_DESC, :vPRC_CUSTO, 0, NULL, 0, 0, 0, 0, 0, NULL, NULL, :pCSOSN, N" +
                "ULL, 0, (:pVLR_TRIB_FED + :pVLR_TRIB_EST), NULL, NULL, NULL, \'\'S\'\', :pVLR_TRIB_F" +
                "ED, :pVLR_TRIB_EST, 0, \'\'S\'\', :pVLR_UNIT, 0); SUSPEND; END\';\r\n\r\nerro = \'sproc SP" +
                "_TRI_GRAVASAT\';\r\nexecute statement \'CREATE OR ALTER PROCEDURE SP_TRI_GRAVASAT (P" +
                "ID_NFVENDA INTEGER, PCHAVE VARCHAR(44), PNUMERO_CFE INTEGER, PNUM_SERIE_SAT VARC" +
                "HAR(9)) RETURNS (RID_REGISTRO INTEGER) AS DECLARE VARIABLE vDT_EMISSAO TYPE OF C" +
                "OLUMN TB_SAT.DT_EMISSAO; DECLARE VARIABLE vHR_EMISSAO TYPE OF COLUMN TB_SAT.HR_E" +
                "MISSAO; BEGIN SELECT DT_SAIDA, HR_SAIDA FROM TB_NFVENDA WHERE ID_NFVENDA = :pID_" +
                "NFVENDA INTO :vDT_EMISSAO, :vHR_EMISSAO; SELECT NEXT VALUE FOR GEN_TB_SAT_ID FRO" +
                "M RDB$DATABASE INTO :rID_REGISTRO; INSERT INTO TB_SAT (ID_REGISTRO, ID_NFVENDA, " +
                "CHAVE, DT_EMISSAO, HR_EMISSAO, STATUS, STATUS_DES, NUMERO_CFE, NUM_SERIE_SAT) VA" +
                "LUES (:rID_REGISTRO, :pID_NFVENDA, :pCHAVE, :vDT_EMISSAO, :vHR_EMISSAO, \'\'06000\'" +
                "\', \'\'Emitido com sucesso + conteúdo notas\'\', :pNUMERO_CFE, :pNUM_SERIE_SAT); SUS" +
                "PEND; END\';\r\n\r\nerro = \'sproc SP_TRI_ATUALIZANFVENDA\';\r\nexecute statement \'CREATE" +
                " OR ALTER PROCEDURE SP_TRI_ATUALIZANFVENDA (pID_NFVENDA INTEGER, PID_CLIENTE INT" +
                "EGER) AS BEGIN UPDATE TB_NFVENDA SET STATUS = \'\'I\'\', ID_CLIENTE = :PID_CLIENTE W" +
                "HERE ID_NFVENDA = :pID_NFVENDA; END\';\r\n\r\nerro = \'sproc V_TRI_CUPONSCANCELAVEIS\';" +
                "\r\nexecute statement \'RECREATE VIEW V_TRI_CUPONSCANCELAVEIS (VALOR_TOTAL, TS_VEND" +
                "A, CHAVE, NF_SERIE, ID_NFVENDA, ID_REGISTRO) AS SELECT B.TOT_NF, CAST(A.DT_SAIDA" +
                " || \'\' \'\' || A.HR_SAIDA AS TIMESTAMP), COALESCE(C.CHAVE, \'\'BETERRABA\'\'), A.NF_SE" +
                "RIE, A.ID_NFVENDA, COALESCE(C.ID_REGISTRO, -1) FROM TB_NFVENDA A JOIN TB_NFVENDA" +
                "_TOT B ON A.ID_NFVENDA = B.ID_NFVENDA LEFT JOIN TB_SAT C ON A.ID_NFVENDA = C.ID_" +
                "NFVENDA WHERE (DATEDIFF(MINUTE FROM CAST(A.DT_SAIDA || \'\' \'\' || A.HR_SAIDA AS TI" +
                "MESTAMP) TO CURRENT_TIMESTAMP) < 30) AND A.STATUS = \'\'I\'\'\';\r\n\r\n                 " +
                "     erro = \'sproc SP_TRI_CONTANFVPAGTO\';\r\n                      execute stateme" +
                "nt \'CREATE OR ALTER PROCEDURE SP_TRI_CONTANFVPAGTO (PNF_SERIE VARCHAR(3), PTS_AB" +
                "ERTURA TIMESTAMP, PTS_FECHAMENTO TIMESTAMP, PSTATUS CHAR(1) = \'\'I\'\') RETURNS (RC" +
                "OUNT_FMANCE INTEGER,\tRID_FMANCFE INTEGER) AS BEGIN FOR SELECT COUNT(A.ID_FMANFCE" +
                "), A.ID_FMANFCE FROM TB_NFVENDA_FMAPAGTO_NFCE A JOIN TB_NFVENDA B ON A.ID_NFVEND" +
                "A = B.ID_NFVENDA WHERE B.NF_SERIE = :pNF_SERIE AND CAST ((B.DT_SAIDA || \'\' \'\' ||" +
                " B.HR_SAIDA) AS TIMESTAMP) BETWEEN :pTS_ABERTURA AND :pTS_FECHAMENTO AND B.STATU" +
                "S = :PSTATUS GROUP BY A.ID_FMANFCE INTO :rCOUNT_FMANCE, :rID_FMANCFE DO SUSPEND;" +
                " END\';\r\n\r\n                      erro = \'sproc SP_TRI_NFVCTAREC\';\r\n              " +
                "        execute statement \'CREATE OR ALTER PROCEDURE SP_TRI_NFVCTAREC (pID_NFVEN" +
                "DA INTEGER, pDT_VENCIMENTO DATE, pDESCRICAO VARCHAR(50), pID_CLIENTE INTEGER, pI" +
                "D_NUMPAG INTEGER) RETURNS (rID_CTAREC INTEGER) AS DECLARE VARIABLE vNF_SERIE TYP" +
                "E OF COLUMN TB_NFVENDA.NF_SERIE; DECLARE VARIABLE vNF_NUMERO TYPE OF COLUMN TB_N" +
                "FVENDA.NF_NUMERO; DECLARE VARIABLE vVALOR TYPE OF COLUMN TB_NFVENDA_FMAPAGTO_NFC" +
                "E.VLR_PAGTO; BEGIN SELECT NEXT VALUE FOR GEN_TB_CTAREC_ID FROM RDB$DATABASE INTO" +
                " :rID_CTAREC; SELECT NF_SERIE, NF_NUMERO FROM TB_NFVENDA WHERE ID_NFVENDA = :pID" +
                "_NFVENDA INTO :vNF_SERIE, :vNF_NUMERO; SELECT VLR_PAGTO FROM TB_NFVENDA_FMAPAGTO" +
                "_NFCE WHERE ID_NUMPAG = :pID_NUMPAG INTO :vVALOR; INSERT INTO TB_CONTA_RECEBER (" +
                "ID_CTAREC, DOCUMENTO, HISTORICO, DT_EMISSAO, DT_VENCTO, VLR_CTAREC, TIP_CTAREC, " +
                "ID_PORTADOR, ID_CLIENTE, INV_REFERENCIA) VALUES (:rID_CTAREC, (CAST(:vNF_SERIE A" +
                "S VARCHAR(12)) || \'\'-\'\' || CAST(:vNF_NUMERO AS VARCHAR(12))), :pDESCRICAO, CURRE" +
                "NT_DATE, :pDT_VENCIMENTO, :vVALOR, \'\'C\'\', 1, :pID_CLIENTE, \'\'D\'\' || LPAD(:rID_CT" +
                "AREC,7,\'\'0\'\') || LPAD(:vNF_SERIE, 2, \'\'0\'\') || \'\'X\'\' || LPAD(:vNF_NUMERO, 7, \'\'0" +
                "\'\')); INSERT INTO TB_NFV_CTAREC (ID_NFVENDA, ID_CTAREC, ID_NUMPAG) VALUES (:pID_" +
                "NFVENDA, :rID_CTAREC, :pID_NUMPAG); SUSPEND; END\';\r\n\r\n                      erro" +
                " = \'sproc SP_TRI_LANCAMOVDIARIO\';\r\n                      execute statement \'CREA" +
                "TE OR ALTER PROCEDURE SP_TRI_LANCAMOVDIARIO (NUMCAIXA VARCHAR(60), VALOR NUMERIC" +
                "(18,4), DESCRICAO_MOV VARCHAR(60), CONTA_ORIGEM SMALLINT, CONTA_DESTINO SMALLINT" +
                ") RETURNS (ID_MOVTO INTEGER) AS DECLARE VARIABLE ultimo_mov INTEGER; BEGIN SELEC" +
                "T NEXT VALUE FOR GEN_TB_MOVDIARIO_ID FROM RDB$DATABASE INTO :ID_MOVTO; INSERT IN" +
                "TO TB_MOVDIARIO (ID_MOVTO, DT_MOVTO, HR_MOVTO, HISTORICO, TIP_MOVTO, VLR_MOVTO, " +
                "ID_CTAPLA) VALUES ((SELECT NEXT VALUE FOR GEN_TB_MOVDIARIO_ID FROM RDB$DATABASE)" +
                ", CURRENT_DATE, CURRENT_TIME, :DESCRICAO_MOV, \'\'C\'\', :VALOR, :CONTA_ORIGEM); INS" +
                "ERT INTO TB_MOVDIARIO (ID_MOVTO, DT_MOVTO, HR_MOVTO, HISTORICO, TIP_MOVTO, VLR_M" +
                "OVTO, ID_CTAPLA) VALUES (:ID_MOVTO, CURRENT_DATE, CURRENT_TIME, :DESCRICAO_MOV, " +
                "\'\'D\'\', :VALOR, :CONTA_DESTINO); SUSPEND; END\';\r\n\r\n                      erro = \'" +
                "sproc SP_TRI_NFVFMAPGTO_INSERT\';\r\n                      execute statement \'CREAT" +
                "E OR ALTER PROCEDURE SP_TRI_NFVFMAPGTO_INSERT (PVALOR TYPE OF COLUMN TB_NFVENDA_" +
                "FMAPAGTO_NFCE.VLR_PAGTO, PID_NFVENDA INTEGER, PID_FMANFCE INTEGER, PID_PARCELA I" +
                "NTEGER) RETURNS (RID_FMAPAGTO INTEGER) AS BEGIN SELECT NEXT VALUE FOR gen_tb_nfv" +
                "enda_fmapagto_nfce_id FROM RDB$DATABASE INTO :rID_FMAPAGTO; INSERT INTO TB_NFVEN" +
                "DA_FMAPAGTO_NFCE (ID_NUMPAG, VLR_PAGTO, ID_NFVENDA, ID_FMANFCE, ID_PARCELA) VALU" +
                "ES (:rID_FMAPAGTO, :pVALOR, :pID_NFVENDA, :pID_FMANFCE, :pID_PARCELA); END\';\r\n\r\n" +
                "                      erro = \'sproc SP_TRI_WHATSINSERE\';\r\n                      " +
                "execute statement \'CREATE OR ALTER PROCEDURE SP_TRI_WHATSINSERE (PNUMERO VARCHAR" +
                "(100), PMENSAGEM BLOB SUB_TYPE TEXT, PDATA TIMESTAMP, PENVIADA VARCHAR(100), PCN" +
                "PJ VARCHAR(50)) AS BEGIN INSERT INTO TRI_PDV_WHATS (NUMERO, MENSAGEM, DATAENVIO," +
                " ENVIADA,CNPJ) VALUES (:PNUMERO, :PMENSAGEM,:PDATA,:PENVIADA,:PCNPJ); END\';\r\n\r\n " +
                "                     erro = \'sproc SP_TRI_RENDIMENTO_SOMA\';\r\n                   " +
                "   execute statement \'CREATE OR ALTER PROCEDURE SP_TRI_RENDIMENTO_SOMA (PDATA_IN" +
                "ICIAL TIMESTAMP, PDATA_FINAL TIMESTAMP, CAIXA VARCHAR(3)) RETURNS (RSOMA DECIMAL" +
                "(18,4), RDESCRICAO VARCHAR(30)) AS BEGIN FOR SELECT SUM(tni.VLR_TOTAL - tni.VLR_" +
                "DESC),tetis.DESCRICAO FROM TB_NFV_ITEM tni JOIN TB_NFVENDA tn ON tni.ID_NFVENDA " +
                "= tn.ID_NFVENDA JOIN TB_EST_IDENTIFICADOR tei ON tni.ID_IDENTIFICADOR = tei.ID_I" +
                "DENTIFICADOR JOIN TB_ESTOQUE te ON tei.ID_ESTOQUE = te.ID_ESTOQUE JOIN TB_EST_TI" +
                "PO_ITEM_SIS tetis ON te.ID_TIPOITEM = tetis.ID_TIPOITEM WHERE (tn.NF_SERIE = \'\'N" +
                "\'\'||:CAIXA OR tn.NF_SERIE = :CAIXA) AND tn.NF_MODELO = \'\'59\'\' AND tn.STATUS = \'\'" +
                "I\'\' AND CAST ((tn.DT_SAIDA || \'\' \'\' || tn.HR_SAIDA) AS TIMESTAMP) BETWEEN :PDATA" +
                "_INICIAL AND :PDATA_FINAL GROUP BY tetis.DESCRICAO INTO :RSOMA, :RDESCRICAO DO B" +
                "EGIN SUSPEND; END END\';\r\n\r\n                      erro = \'deu certo\';\r\n\r\n        " +
                "              SUSPEND;\r\n                      WHEN ANY DO\r\n                     " +
                " BEGIN\r\n                      END\r\n                      END;";
