...
END;";
            ...
            this._commandCollection[20].CommandText = "\r\n\t\t\t\t\t\tCREATE OR ALTER PROCEDURE SP_TRI_PROCEDURES7\r\n\t\t\t\t\t\tRETURNS\r\n\t\t\t\t\t\t(\r\n\t\t\t" +
                ...
                "rNewNextSeqValue = :vNewNextSeqValue; END;\';\r\n\r\n\t\t\t\t\t\terro = \'sproc SP_TRI_NATOP" +
                "ER_UPSERT\';\r\n\t\t\t\t\t\texecute statement \'CREATE OR ALTER PROCEDURE SP_TRI_NATOPER_U" +
                "PSERT ( pID_NATOPE TYPE OF COLUMN TB_NAT_OPERACAO.ID_NATOPE , pDESCRICAO TYPE OF" +
                " COLUMN TB_NAT_OPERACAO.DESCRICAO , pRET_PIS_COF_CSLL TYPE OF COLUMN TB_NAT_OPER" +
                "ACAO.RET_PIS_COF_CSLL , pRET_INSS TYPE OF COLUMN TB_NAT_OPERACAO.RET_INSS , pRET" +
                "_IRRF TYPE OF COLUMN TB_NAT_OPERACAO.RET_IRRF , pPIS_COFINS TYPE OF COLUMN TB_NA" +
                "T_OPERACAO.PIS_COFINS , pSTATUS TYPE OF COLUMN TB_NAT_OPERACAO.STATUS , pCFOP TY" +
                "PE OF COLUMN TB_NAT_OPERACAO.CFOP , pID_CTI TYPE OF COLUMN TB_NAT_OPERACAO.ID_CT" +
                "I , pGFR TYPE OF COLUMN TB_NAT_OPERACAO.GFR , pOBSERVACAO TYPE OF COLUMN TB_NAT_" +
                "OPERACAO.OBSERVACAO , pBASE_COMISSAO TYPE OF COLUMN TB_NAT_OPERACAO.BASE_COMISSA" +
                "O , pCALCULA_IPI TYPE OF COLUMN TB_NAT_OPERACAO.CALCULA_IPI , pGRAVA_TOT_TRIBUTO" +
                "S TYPE OF COLUMN TB_NAT_OPERACAO.GRAVA_TOT_TRIBUTOS ) RETURNS ( rRowsAffected IN" +
                "TEGER ) AS BEGIN UPDATE OR INSERT INTO TB_NAT_OPERACAO (ID_NATOPE , DESCRICAO , " +
                "RET_PIS_COF_CSLL , RET_INSS , RET_IRRF , PIS_COFINS , STATUS , CFOP , ID_CTI , G" +
                "FR , OBSERVACAO , BASE_COMISSAO , CALCULA_IPI , GRAVA_TOT_TRIBUTOS ) VALUES ( :p" +
                "ID_NATOPE , :pDESCRICAO , :pRET_PIS_COF_CSLL , :pRET_INSS , :pRET_IRRF , :pPIS_C" +
                "OFINS , :pSTATUS , :pCFOP , :pID_CTI , :pGFR , :pOBSERVACAO , :pBASE_COMISSAO , " +
                ":pCALCULA_IPI , :pGRAVA_TOT_TRIBUTOS ) MATCHING (ID_NATOPE ) ; rRowsAffected = R" +
                "OW_COUNT; END;\';\r\n\r\n\t\t\t\t\t\terro = \'sproc SP_TRI_FMAPGTONFCE_UPSERT\';\r\n\t\t\t\t\t\texecu" +
                "te statement \'CREATE OR ALTER PROCEDURE SP_TRI_FMAPGTONFCE_UPSERT ( pID_FMANFCE " +
                "TYPE OF COLUMN TB_FORMA_PAGTO_NFCE.ID_FMANFCE , pSTATUS TYPE OF COLUMN TB_FORMA_" +
                "PAGTO_NFCE.STATUS , pID_NFCE TYPE OF COLUMN TB_FORMA_PAGTO_NFCE.ID_NFCE , pDESCR" +
                "ICAO TYPE OF COLUMN TB_FORMA_PAGTO_NFCE.DESCRICAO ) RETURNS ( rRowsAffected INTE" +
                "GER ) AS BEGIN UPDATE OR INSERT INTO TB_FORMA_PAGTO_NFCE (ID_FMANFCE , STATUS , " +
                "ID_NFCE , DESCRICAO ) VALUES ( :pID_FMANFCE , :pSTATUS , :pID_NFCE , :pDESCRICAO" +
                " ) MATCHING (ID_FMANFCE ) ; rRowsAffected = ROW_COUNT; END;\';\r\n\r\n\t\t\t\t\t\terro = \'s" +
                "proc SP_TRI_NFVENDA_GETALL_SYNC\';\r\n\t\t\t\t\t\texecute statement \'CREATE OR ALTER PROC" +
                "EDURE SP_TRI_NFVENDA_GETALL_SYNC ( TIP_QUERY SMALLINT ) RETURNS ( ID_NFVENDA TYP" +
                "E OF COLUMN TB_NFVENDA.ID_NFVENDA , ID_NATOPE TYPE OF COLUMN TB_NFVENDA.ID_NATOP" +
                "E , ID_VENDEDOR TYPE OF COLUMN TB_NFVENDA.ID_VENDEDOR , ID_CLIENTE TYPE OF COLUM" +
                "N TB_NFVENDA.ID_CLIENTE , NF_NUMERO TYPE OF COLUMN TB_NFVENDA.NF_NUMERO , NF_SER" +
                "IE TYPE OF COLUMN TB_NFVENDA.NF_SERIE , NF_MODELO TYPE OF COLUMN TB_NFVENDA.NF_M" +
                "ODELO , DT_EMISSAO TYPE OF COLUMN TB_NFVENDA.DT_EMISSAO , DT_SAIDA TYPE OF COLUM" +
                "N TB_NFVENDA.DT_SAIDA , HR_SAIDA TYPE OF COLUMN TB_NFVENDA.HR_SAIDA , ESPECIE TY" +
                "PE OF COLUMN TB_NFVENDA.ESPECIE , TIPO_FRETE TYPE OF COLUMN TB_NFVENDA.TIPO_FRET" +
                "E , PES_LIQUID TYPE OF COLUMN TB_NFVENDA.PES_LIQUID , PES_BRUTO TYPE OF COLUMN T" +
                "B_NFVENDA.PES_BRUTO , STATUS TYPE OF COLUMN TB_NFVENDA.STATUS , ENT_SAI TYPE OF " +
                "COLUMN TB_NFVENDA.ENT_SAI , ID_FMAPGTO TYPE OF COLUMN TB_NFVENDA.ID_FMAPGTO , ID" +
                "_PARCELA TYPE OF COLUMN TB_NFVENDA.ID_PARCELA , MARCA TYPE OF COLUMN TB_NFVENDA." +
                "MARCA , QTD_VOLUM TYPE OF COLUMN TB_NFVENDA.QTD_VOLUM , NUM_VOLUM TYPE OF COLUMN" +
                " TB_NFVENDA.NUM_VOLUM , PROD_REV TYPE OF COLUMN TB_NFVENDA.PROD_REV , SOMA_FRETE" +
                " TYPE OF COLUMN TB_NFVENDA.SOMA_FRETE , VLR_TROCO TYPE OF COLUMN TB_NFVENDA.VLR_" +
                "TROCO , IND_PRES TYPE OF COLUMN TB_NFVENDA.IND_PRES , IND_IE_DEST TYPE OF COLUMN" +
                " TB_NFVENDA.IND_IE_DEST , DESCONTO_CONDICIONAL TYPE OF COLUMN TB_NFVENDA.DESCONT" +
                "O_CONDICIONAL , INF_COMP_FIXA TYPE OF COLUMN TB_NFVENDA.INF_COMP_FIXA , INF_COMP" +
                "_EDIT TYPE OF COLUMN TB_NFVENDA.INF_COMP_EDIT , ENDERECO_ENTREGA TYPE OF COLUMN " +
                "TB_NFVENDA.ENDERECO_ENTREGA , ENVIO_API TYPE OF COLUMN TB_NFVENDA.ENVIO_API , SY" +
                "NCED SMALLINT , QTD_CTAREC INTEGER ) AS BEGIN FOR SELECT FIRST 200 COALESCE(a.ID" +
                "_NFVENDA, 0) , a.ID_NATOPE , a.ID_VENDEDOR , a.ID_CLIENTE , a.NF_NUMERO , a.NF_S" +
                "ERIE , a.NF_MODELO , a.DT_EMISSAO , a.DT_SAIDA , a.HR_SAIDA , a.ESPECIE , a.TIPO" +
                "_FRETE , a.PES_LIQUID , a.PES_BRUTO , a.STATUS , a.ENT_SAI , a.ID_FMAPGTO , a.ID" +
                "_PARCELA , a.MARCA , a.QTD_VOLUM , a.NUM_VOLUM , a.PROD_REV , a.SOMA_FRETE , a.V" +
                "LR_TROCO , a.IND_PRES , a.IND_IE_DEST , a.DESCONTO_CONDICIONAL , a.INF_COMP_FIXA" +
                " , a.INF_COMP_EDIT , a.ENDERECO_ENTREGA , a.ENVIO_API , a.SYNCED , COUNT(b.ID_CT" +
                "AREC) QTD_CTAREC FROM TB_NFVENDA a LEFT JOIN TB_NFV_CTAREC b ON a.ID_NFVENDA = b" +
                ".ID_NFVENDA WHERE ( :TIP_QUERY = 0 AND ( ( a.SYNCED IS NULL OR a.SYNCED = 0 ) AN" +
                "D (a.STATUS = \'\'E\'\' OR a.STATUS = \'\'I\'\' OR a.STATUS = \'\'C\'\') ) ) OR ( :TIP_QUERY" +
                " = 1 AND ( a.SYNCED = 1 AND (a.STATUS = \'\'C\'\' OR a.STATUS = \'\'X\'\') ) ) OR (:TIP_" +
                "QUERY = 3 AND (a.SYNCED = 3 AND (a.STATUS = \'\'I\'\' OR a.STATUS = \'\'E\'\'))) GROUP B" +
                ...
                "CTAREC DO BEGIN SUSPEND ; END END;\';\r\n\r\n\t\t\t\t\t\terro = \'sproc SP_TRI_NFVENDA_SYNC_" +
                "INSERT\';\r\n\t\t\t\t\t\texecute statement \'CREATE OR ALTER PROCEDURE SP_TRI_NFVENDA_SYNC" +
                "_INSERT ( pID_NATOPE TYPE OF COLUMN TB_NFVENDA.ID_NATOPE , pID_VENDEDOR TYPE OF " +
                "COLUMN TB_NFVENDA.ID_VENDEDOR , pID_CLIENTE TYPE OF COLUMN TB_NFVENDA.ID_CLIENTE" +
                " , pNF_NUMERO TYPE OF COLUMN TB_NFVENDA.NF_NUMERO , pNF_SERIE TYPE OF COLUMN TB_" +
                "NFVENDA.NF_SERIE , pNF_MODELO TYPE OF COLUMN TB_NFVENDA.NF_MODELO , pDT_EMISSAO " +
                "TYPE OF COLUMN TB_NFVENDA.DT_EMISSAO , pDT_SAIDA TYPE OF COLUMN TB_NFVENDA.DT_SA" +
                "IDA , pHR_SAIDA TYPE OF COLUMN TB_NFVENDA.HR_SAIDA , pESPECIE TYPE OF COLUMN TB_" +
                "NFVENDA.ESPECIE , pTIPO_FRETE TYPE OF COLUMN TB_NFVENDA.TIPO_FRETE , pPES_LIQUID" +
                " TYPE OF COLUMN TB_NFVENDA.PES_LIQUID , pPES_BRUTO TYPE OF COLUMN TB_NFVENDA.PES" +
                "_BRUTO , pSTATUS TYPE OF COLUMN TB_NFVENDA.STATUS , pENT_SAI TYPE OF COLUMN TB_N" +
                "FVENDA.ENT_SAI , pID_FMAPGTO TYPE OF COLUMN TB_NFVENDA.ID_FMAPGTO , pID_PARCELA " +
                "TYPE OF COLUMN TB_NFVENDA.ID_PARCELA , pMARCA TYPE OF COLUMN TB_NFVENDA.MARCA , " +
                "pQTD_VOLUM TYPE OF COLUMN TB_NFVENDA.QTD_VOLUM , pNUM_VOLUM TYPE OF COLUMN TB_NF" +
                "VENDA.NUM_VOLUM , pPROD_REV TYPE OF COLUMN TB_NFVENDA.PROD_REV , pSOMA_FRETE TYP" +
                "E OF COLUMN TB_NFVENDA.SOMA_FRETE , pVLR_TROCO TYPE OF COLUMN TB_NFVENDA.VLR_TRO" +
                "CO , pIND_PRES TYPE OF COLUMN TB_NFVENDA.IND_PRES , pIND_IE_DEST TYPE OF COLUMN " +
                "TB_NFVENDA.IND_IE_DEST , pDESCONTO_CONDICIONAL TYPE OF COLUMN TB_NFVENDA.DESCONT" +
                "O_CONDICIONAL , pINF_COMP_FIXA TYPE OF COLUMN TB_NFVENDA.INF_COMP_FIXA , pINF_CO" +
                "MP_EDIT TYPE OF COLUMN TB_NFVENDA.INF_COMP_EDIT , pENDERECO_ENTREGA TYPE OF COLU" +
                "MN TB_NFVENDA.ENDERECO_ENTREGA , pENVIO_API TYPE OF COLUMN TB_NFVENDA.ENVIO_API " +
                ", pSYNCED SMALLINT ) RETURNS ( NFVENDAUSAVEL INTEGER ) AS DECLARE VARIABLE ULTIM" +
                "ONFVENDA INTEGER; BEGIN SELECT NEXT VALUE FOR GEN_TB_NFVENDA_ID FROM RDB$DATABAS" +
                "E INTO :ULTIMONFVENDA ; IF( :ULTIMONFVENDA IS NULL ) THEN ULTIMONFVENDA = 0; BEG" +
                "IN INSERT INTO TB_NFVENDA ( ID_NFVENDA , ID_NATOPE , ID_VENDEDOR , ID_CLIENTE , " +
                "NF_NUMERO , NF_SERIE , NF_MODELO , DT_EMISSAO , DT_SAIDA , HR_SAIDA , ESPECIE , " +
                "TIPO_FRETE , PES_LIQUID , PES_BRUTO , STATUS , ENT_SAI , ID_FMAPGTO , ID_PARCELA" +
                " , MARCA , QTD_VOLUM , NUM_VOLUM , PROD_REV , SOMA_FRETE , VLR_TROCO , IND_PRES " +
                ", IND_IE_DEST , DESCONTO_CONDICIONAL , INF_COMP_FIXA , INF_COMP_EDIT , ENDERECO_" +
                "ENTREGA , ENVIO_API , SYNCED ) VALUES (:ULTIMONFVENDA , :pID_NATOPE , :pID_VENDE" +
                "DOR , :pID_CLIENTE , :pNF_NUMERO , :pNF_SERIE , :pNF_MODELO , :pDT_EMISSAO , :pD" +
                "T_SAIDA , :pHR_SAIDA , :pESPECIE , :pTIPO_FRETE , :pPES_LIQUID , :pPES_BRUTO , :" +
                "pSTATUS , :pENT_SAI , :pID_FMAPGTO , :pID_PARCELA , :pMARCA , :pQTD_VOLUM , :pNU" +
                "M_VOLUM , :pPROD_REV , :pSOMA_FRETE , :pVLR_TROCO , :pIND_PRES , :pIND_IE_DEST ," +
                " :pDESCONTO_CONDICIONAL , :pINF_COMP_FIXA , :pINF_COMP_EDIT , :pENDERECO_ENTREGA" +
                " , :pENVIO_API , :pSYNCED ) ; NFVENDAUSAVEL = ULTIMONFVENDA; END END;\';\r\n\r\n\t\t\t\t\t" +
                "\terro = \'sproc SP_TRI_NFV_FMAPAGTO_BY_IDNFV\';\r\n\t\t\t\t\t\texecute statement \'CREATE O" +
                "R ALTER PROCEDURE SP_TRI_NFV_FMAPAGTO_BY_IDNFV ( pIdNfvenda TYPE OF COLUMN TB_NF" +
                "VENDA_FMAPAGTO_NFCE.ID_NFVENDA ) RETURNS ( ID_NUMPAG TYPE OF COLUMN TB_NFVENDA_F" +
                "MAPAGTO_NFCE.ID_NUMPAG , VLR_PAGTO TYPE OF COLUMN TB_NFVENDA_FMAPAGTO_NFCE.VLR_P" +
                "AGTO , ID_NFVENDA TYPE OF COLUMN TB_NFVENDA_FMAPAGTO_NFCE.ID_NFVENDA , ID_FMANFC" +
                "E TYPE OF COLUMN TB_NFVENDA_FMAPAGTO_NFCE.ID_FMANFCE , ID_PARCELA TYPE OF COLUMN" +
                " TB_NFVENDA_FMAPAGTO_NFCE.ID_PARCELA) AS BEGIN FOR SELECT a.ID_NUMPAG , a.VLR_PA" +
                "GTO , a.ID_NFVENDA , a.ID_FMANFCE , a.ID_PARCELA FROM TB_NFVENDA_FMAPAGTO_NFCE a" +
                " WHERE a.ID_NFVENDA =:pIdNfvenda INTO :ID_NUMPAG , :VLR_PAGTO , :ID_NFVENDA , :I" +
                "D_FMANFCE , :ID_PARCELA DO SUSPEND ; END;\';\r\n\r\n\t\t\t\t\t\terro = \'sproc SP_TRI_NFV_FM" +
                "APAGT_SYNC_INSERT\';\r\n\t\t\t\t\t\texecute statement \'CREATE OR ALTER PROCEDURE SP_TRI_N" +
                "FV_FMAPAGT_SYNC_INSERT ( pVLR_PAGTO TYPE OF COLUMN TB_NFVENDA_FMAPAGTO_NFCE.VLR_" +
                "PAGTO , pID_NFVENDA TYPE OF COLUMN TB_NFVENDA_FMAPAGTO_NFCE.ID_NFVENDA , pID_FMA" +
                "NFCE TYPE OF COLUMN TB_NFVENDA_FMAPAGTO_NFCE.ID_FMANFCE , pID_PARCELA TYPE OF CO" +
                "LUMN TB_NFVENDA_FMAPAGTO_NFCE.ID_PARCELA, pID_ADMINISTRADORA TYPE OF COLUMN TB_N" +
                "FCE_BANDEIRA.ID_ADMINISTRADORA) RETURNS ( rNewIdNumpag TYPE OF COLUMN TB_NFVENDA" +
                "_FMAPAGTO_NFCE.ID_NUMPAG ) AS DECLARE VARIABLE newIdNumpag TYPE OF COLUMN TB_NFV" +
                "ENDA_FMAPAGTO_NFCE.ID_NUMPAG; BEGIN SELECT NEXT VALUE FOR gen_tb_nfvenda_fmapagt" +
                "o_nfce_id FROM RDB$DATABASE INTO :newIdNumpag ; IF( :newIdNumpag IS NULL ) THEN " +
                "newIdNumpag = 0; BEGIN INSERT INTO TB_NFVENDA_FMAPAGTO_NFCE (ID_NUMPAG , VLR_PAG" +
                "TO , ID_NFVENDA , ID_FMANFCE , ID_PARCELA ) VALUES ( :newIdNumpag , :pVLR_PAGTO " +
                ", :pID_NFVENDA , :pID_FMANFCE , :pID_PARCELA ) ; INSERT INTO TB_NFCE_BANDEIRA (I" +
                "D_NUMPAG , ID_BANDEIRA , ID_ADMINISTRADORA , NUM_AUT , FORMA_ENVIO , SICLOS_ID ," +
                " STONE_ID , PRE_TRANSACAO_ID , POS_SERIAL) VALUES (:newIdNumpag , NULL, :pID_ADM" +
                "INISTRADORA , NULL , NULL , NULL , NULL , NULL , NULL) ; rNewIdNumpag = newIdNum" +
                "pag; END END;\';\r\n\r\n\t\t\t\t\t\terro = \'sproc SP_TRI_NFVITEM_GETBY_IDNFVENDA\';\r\n\t\t\t\t\t\te" +
                "xecute statement \'CREATE OR ALTER PROCEDURE SP_TRI_NFVITEM_GETBY_IDNFVENDA ( pId" +
                "Nfvenda INTEGER ) RETURNS ( ID_NFVITEM TYPE OF COLUMN TB_NFV_ITEM.ID_NFVITEM , I" +
                "D_NFVENDA TYPE OF COLUMN TB_NFV_ITEM.ID_NFVENDA , ID_IDENTIFICADOR TYPE OF COLUM" +
                "N TB_NFV_ITEM.ID_IDENTIFICADOR , CFOP TYPE OF COLUMN TB_NFV_ITEM.CFOP , NUM_ITEM" +
                " TYPE OF COLUMN TB_NFV_ITEM.NUM_ITEM , QTD_ITEM TYPE OF COLUMN TB_NFV_ITEM.QTD_I" +
                "TEM , UNI_MEDIDA TYPE OF COLUMN TB_NFV_ITEM.UNI_MEDIDA , VLR_TOTAL TYPE OF COLUM" +
                "N TB_NFV_ITEM.VLR_TOTAL , VLR_DESC TYPE OF COLUMN TB_NFV_ITEM.VLR_DESC , VLR_CUS" +
                "TO TYPE OF COLUMN TB_NFV_ITEM.VLR_CUSTO , PRC_LISTA TYPE OF COLUMN TB_NFV_ITEM.P" +
                "RC_LISTA , CF TYPE OF COLUMN TB_NFV_ITEM.CF , VLR_FRETE TYPE OF COLUMN TB_NFV_IT" +
                "EM.VLR_FRETE , VLR_SEGURO TYPE OF COLUMN TB_NFV_ITEM.VLR_SEGURO , VLR_DESPESA TY" +
                "PE OF COLUMN TB_NFV_ITEM.VLR_DESPESA , RET_PIS_COF_CSLL TYPE OF COLUMN TB_NFV_IT" +
                "EM.RET_PIS_COF_CSLL , RET_IRRF TYPE OF COLUMN TB_NFV_ITEM.RET_IRRF , COD_ENQ TYP" +
                "E OF COLUMN TB_NFV_ITEM.COD_ENQ , COD_BASE TYPE OF COLUMN TB_NFV_ITEM.COD_BASE ," +
                " CSOSN TYPE OF COLUMN TB_NFV_ITEM.CSOSN , NPED_COMPRA TYPE OF COLUMN TB_NFV_ITEM" +
                ".NPED_COMPRA , ITEM_COMPRA TYPE OF COLUMN TB_NFV_ITEM.ITEM_COMPRA , VLR_TOTTRIB " +
                "TYPE OF COLUMN TB_NFV_ITEM.VLR_TOTTRIB , FCI TYPE OF COLUMN TB_NFV_ITEM.FCI , VL" +
                "R_ICM_DESO TYPE OF COLUMN TB_NFV_ITEM.VLR_ICM_DESO , ID_MOTIVO_DESO TYPE OF COLU" +
                "MN TB_NFV_ITEM.ID_MOTIVO_DESO , EST_BX TYPE OF COLUMN TB_NFV_ITEM.EST_BX , VLR_T" +
                "RIB_FED TYPE OF COLUMN TB_NFV_ITEM.VLR_TRIB_FED , VLR_TRIB_EST TYPE OF COLUMN TB" +
                "_NFV_ITEM.VLR_TRIB_EST , VLR_TRIB_MUN TYPE OF COLUMN TB_NFV_ITEM.VLR_TRIB_MUN , " +
                "INCLUIR_FATURA TYPE OF COLUMN TB_NFV_ITEM.INCLUIR_FATURA , VLR_UNIT TYPE OF COLU" +
                "MN TB_NFV_ITEM.VLR_UNIT , VLR_RETENCAO TYPE OF COLUMN TB_NFV_ITEM.VLR_RETENCAO ," +
                " REFERENCIA TYPE OF COLUMN TB_NFV_ITEM.REFERENCIA , ID_COMPOSICAO TYPE OF COLUMN" +
                " TB_EST_COMPOSICAO.ID_COMPOSICAO ) AS BEGIN FOR SELECT COALESCE(a.ID_NFVITEM, 0)" +
                " ID_NFVITEM , a.ID_NFVENDA , a.ID_IDENTIFICADOR , a.CFOP , a.NUM_ITEM , a.QTD_IT" +
                "EM , a.UNI_MEDIDA , a.VLR_TOTAL , a.VLR_DESC , a.VLR_CUSTO , a.PRC_LISTA , a.CF " +
                ", a.VLR_FRETE , a.VLR_SEGURO , a.VLR_DESPESA , a.RET_PIS_COF_CSLL , a.RET_IRRF ," +
                " a.COD_ENQ , a.COD_BASE , a.CSOSN , a.NPED_COMPRA , a.ITEM_COMPRA , a.VLR_TOTTRI" +
                "B , a.FCI , a.VLR_ICM_DESO , a.ID_MOTIVO_DESO , a.EST_BX , a.VLR_TRIB_FED , a.VL" +
                "R_TRIB_EST , a.VLR_TRIB_MUN , a.INCLUIR_FATURA , a.VLR_UNIT , a.VLR_RETENCAO , a" +
                ".REFERENCIA , e.ID_COMPOSICAO FROM TB_NFV_ITEM a LEFT JOIN TB_EST_COMPOSICAO e O" +
                "N e.ID_IDENTIFICADOR = a.ID_IDENTIFICADOR WHERE a.ID_NFVENDA =:pIdNfvenda INTO :" +
                ...
                ":INCLUIR_FATURA , :VLR_UNIT , :VLR_RETENCAO , :REFERENCIA , :ID_COMPOSICAO DO SU" +
                "SPEND ; END;\';\r\n\r\n\t\t\t\t\t\terro = \'sproc SP_TRI_NFVITEM_SYNC_INSERT\';\r\n\t\t\t\t\t\texecut" +
                "e statement \'CREATE OR ALTER PROCEDURE SP_TRI_NFVITEM_SYNC_INSERT ( pID_NFVENDA " +
                "TYPE OF COLUMN TB_NFV_ITEM.ID_NFVENDA , pID_IDENTIFICADOR TYPE OF COLUMN TB_NFV_" +
                "ITEM.ID_IDENTIFICADOR , pCFOP TYPE OF COLUMN TB_NFV_ITEM.CFOP , pNUM_ITEM TYPE O" +
                "F COLUMN TB_NFV_ITEM.NUM_ITEM , pQTD_ITEM TYPE OF COLUMN TB_NFV_ITEM.QTD_ITEM , " +
                "pUNI_MEDIDA TYPE OF COLUMN TB_NFV_ITEM.UNI_MEDIDA , pVLR_TOTAL TYPE OF COLUMN TB" +
                "_NFV_ITEM.VLR_TOTAL , pVLR_DESC TYPE OF COLUMN TB_NFV_ITEM.VLR_DESC , pVLR_CUSTO" +
                " TYPE OF COLUMN TB_NFV_ITEM.VLR_CUSTO , pPRC_LISTA TYPE OF COLUMN TB_NFV_ITEM.PR" +
                "C_LISTA , pCF TYPE OF COLUMN TB_NFV_ITEM.CF , pVLR_FRETE TYPE OF COLUMN TB_NFV_I" +
                "TEM.VLR_FRETE , pVLR_SEGURO TYPE OF COLUMN TB_NFV_ITEM.VLR_SEGURO , pVLR_DESPESA" +
                " TYPE OF COLUMN TB_NFV_ITEM.VLR_DESPESA , pRET_PIS_COF_CSLL TYPE OF COLUMN TB_NF" +
                "V_ITEM.RET_PIS_COF_CSLL , pRET_IRRF TYPE OF COLUMN TB_NFV_ITEM.RET_IRRF , pCOD_E" +
                "NQ TYPE OF COLUMN TB_NFV_ITEM.COD_ENQ , pCOD_BASE TYPE OF COLUMN TB_NFV_ITEM.COD" +
                "_BASE , pCSOSN TYPE OF COLUMN TB_NFV_ITEM.CSOSN , pNPED_COMPRA TYPE OF COLUMN TB" +
                "_NFV_ITEM.NPED_COMPRA , pITEM_COMPRA TYPE OF COLUMN TB_NFV_ITEM.ITEM_COMPRA , pV" +
                "LR_TOTTRIB TYPE OF COLUMN TB_NFV_ITEM.VLR_TOTTRIB , pFCI TYPE OF COLUMN TB_NFV_I" +
                "TEM.FCI , pVLR_ICM_DESO TYPE OF COLUMN TB_NFV_ITEM.VLR_ICM_DESO , pID_MOTIVO_DES" +
                "O TYPE OF COLUMN TB_NFV_ITEM.ID_MOTIVO_DESO , pEST_BX TYPE OF COLUMN TB_NFV_ITEM" +
                ".EST_BX , pVLR_TRIB_FED TYPE OF COLUMN TB_NFV_ITEM.VLR_TRIB_FED , pVLR_TRIB_EST " +
                "TYPE OF COLUMN TB_NFV_ITEM.VLR_TRIB_EST , pVLR_TRIB_MUN TYPE OF COLUMN TB_NFV_IT" +
                "EM.VLR_TRIB_MUN , pINCLUIR_FATURA TYPE OF COLUMN TB_NFV_ITEM.INCLUIR_FATURA , pV" +
                "LR_UNIT TYPE OF COLUMN TB_NFV_ITEM.VLR_UNIT , pVLR_RETENCAO TYPE OF COLUMN TB_NF" +
                "V_ITEM.VLR_RETENCAO , pREFERENCIA TYPE OF COLUMN TB_NFV_ITEM.REFERENCIA ) RETURN" +
                "S ( rnewIdNfvItem INTEGER ) AS DECLARE VARIABLE newIdNfvItem INTEGER; BEGIN SELE" +
                "CT NEXT VALUE FOR GEN_TB_NFV_ITEM_ID FROM RDB$DATABASE INTO :newIdNfvItem ;  IF(" +
                " :newIdNfvItem IS NULL ) THEN newIdNfvItem = 0; BEGIN INSERT INTO TB_NFV_ITEM ( " +
                "ID_NFVITEM , ID_NFVENDA , ID_IDENTIFICADOR , CFOP , NUM_ITEM , QTD_ITEM , UNI_ME" +
                "DIDA , VLR_TOTAL , VLR_DESC , VLR_CUSTO , PRC_LISTA , CF , VLR_FRETE , VLR_SEGUR" +
                "O , VLR_DESPESA , RET_PIS_COF_CSLL , RET_IRRF , COD_ENQ , COD_BASE , CSOSN , NPE" +
                "D_COMPRA , ITEM_COMPRA , VLR_TOTTRIB , FCI , VLR_ICM_DESO , ID_MOTIVO_DESO , EST" +
                "_BX , VLR_TRIB_FED , VLR_TRIB_EST , VLR_TRIB_MUN , INCLUIR_FATURA , VLR_UNIT , V" +
                "LR_RETENCAO , REFERENCIA  ) VALUES ( :newIdNfvItem , :pID_NFVENDA , :pID_IDENTIF" +
                "ICADOR , :pCFOP , :pNUM_ITEM , :pQTD_ITEM , :pUNI_MEDIDA , :pVLR_TOTAL , :pVLR_D" +
                "ESC , :pVLR_CUSTO , :pPRC_LISTA , :pCF , :pVLR_FRETE , :pVLR_SEGURO , :pVLR_DESP" +
                "ESA , :pRET_PIS_COF_CSLL , :pRET_IRRF , :pCOD_ENQ , :pCOD_BASE , :pCSOSN , :pNPE" +
                "D_COMPRA , :pITEM_COMPRA , :pVLR_TOTTRIB , :pFCI , :pVLR_ICM_DESO , :pID_MOTIVO_" +
                "DESO , :pEST_BX , :pVLR_TRIB_FED , :pVLR_TRIB_EST , :pVLR_TRIB_MUN , :pINCLUIR_F" +
                "ATURA , :pVLR_UNIT , :pVLR_RETENCAO , :pREFERENCIA  ) ;  rnewIdNfvItem = newIdNf" +
                "vItem; END END;\';\r\n\r\n\t\t\t\t\t\terro = \'sproc SP_TRI_CTAREC_GETBY_IDNFVENDA\';\r\n\t\t\t\t\t\t" +
                "execute statement \'CREATE OR ALTER PROCEDURE SP_TRI_CTAREC_GETBY_IDNFVENDA ( pId" +
                "Nfvenda INTEGER ) RETURNS ( ID_CTAREC INTEGER , DOCUMENTO VARCHAR(12) , HISTORIC" +
                "O VARCHAR(50) , DT_EMISSAO DATE , DT_VENCTO DATE , VLR_CTAREC TYPE OF COLUMN TRI" +
                "_PDV_OPER.DIN , TIP_CTAREC CHAR(1) , ID_PORTADOR INTEGER , ID_CLIENTE INTEGER , " +
                "INV_REFERENCIA VARCHAR(18) , DT_VENCTO_ORIG DATE , NSU_CARTAO VARCHAR(32) , ID_N" +
                "UMPAG TYPE OF COLUMN TB_NFV_CTAREC.ID_NUMPAG ) AS BEGIN FOR SELECT COALESCE(a.ID" +
                "_CTAREC, 0) , a.DOCUMENTO , a.HISTORICO , a.DT_EMISSAO , a.DT_VENCTO , a.VLR_CTA" +
                "REC , a.TIP_CTAREC , a.ID_PORTADOR , a.ID_CLIENTE , a.INV_REFERENCIA , a.DT_VENC" +
                "TO_ORIG , a.NSU_CARTAO , b.ID_NUMPAG FROM TB_CONTA_RECEBER a JOIN TB_NFV_CTAREC " +
                "b ON a.ID_CTAREC = b.ID_CTAREC WHERE b.ID_NFVENDA =:pIdNfvenda INTO :ID_CTAREC ," +
                " :DOCUMENTO , :HISTORICO , :DT_EMISSAO , :DT_VENCTO , :VLR_CTAREC , :TIP_CTAREC " +
                ", :ID_PORTADOR , :ID_CLIENTE , :INV_REFERENCIA , :DT_VENCTO_ORIG , :NSU_CARTAO ," +
                " :ID_NUMPAG DO SUSPEND ; END;\';\r\n\r\n\t\t\t\t\t\terro = \'sproc SP_TRI_NFVENDA_SETSYNCED\'" +
                ...
                "CTAREC_GETBY_NFNUMSERIE\';\r\n\t\t\t\t\t\texecute statement \'CREATE OR ALTER PROCEDURE SP" +
                "_TRI_CTAREC_GETBY_NFNUMSERIE ( pNF_NUMERO TYPE OF COLUMN TB_NFVENDA.NF_NUMERO , " +
                "pNF_SERIE TYPE OF COLUMN TB_NFVENDA.NF_SERIE ) RETURNS ( ID_CTAREC INTEGER , DOC" +
                "UMENTO VARCHAR(12) , HISTORICO VARCHAR(50) , DT_EMISSAO DATE , DT_VENCTO DATE , " +
                "VLR_CTAREC TYPE OF COLUMN TRI_PDV_OPER.DIN , TIP_CTAREC CHAR(1) , ID_PORTADOR IN" +
                "TEGER , ID_CLIENTE INTEGER , INV_REFERENCIA VARCHAR(18) , DT_VENCTO_ORIG DATE , " +
                "NSU_CARTAO VARCHAR(32) ) AS BEGIN FOR SELECT a.ID_CTAREC , a.DOCUMENTO , a.HISTO" +
                "RICO , a.DT_EMISSAO , a.DT_VENCTO , a.VLR_CTAREC , a.TIP_CTAREC , a.ID_PORTADOR " +
                ", a.ID_CLIENTE , a.INV_REFERENCIA , a.DT_VENCTO_ORIG , a.NSU_CARTAO FROM TB_CONT" +
                "A_RECEBER a JOIN TB_NFV_CTAREC b ON a.ID_CTAREC = b.ID_CTAREC JOIN TB_NFVENDA c " +
                "ON c.ID_NFVENDA = b.ID_NFVENDA WHERE c.NF_NUMERO = :pNF_NUMERO AND c.NF_SERIE = " +
                ":pNF_SERIE INTO :ID_CTAREC , :DOCUMENTO , :HISTORICO , :DT_EMISSAO , :DT_VENCTO " +
                ", :VLR_CTAREC , :TIP_CTAREC , :ID_PORTADOR , :ID_CLIENTE , :INV_REFERENCIA , :DT" +
                "_VENCTO_ORIG , :NSU_CARTAO DO SUSPEND ; END;\';\r\n\r\n\t\t\t\t\t\terro = \'sproc SP_TRI_NFV" +
                "_CTAREC_SYNC_DEL\';\r\n\t\t\t\t\t\texecute statement \'CREATE OR ALTER PROCEDURE SP_TRI_NF" +
                "V_CTAREC_SYNC_DEL ( pNfNumero TYPE OF COLUMN TB_NFVENDA.NF_NUMERO , pNfSerie TYP" +
                "E OF COLUMN TB_NFVENDA.NF_SERIE , pIdCtarec TYPE OF COLUMN TB_NFV_CTAREC.ID_CTAR" +
                "EC ) AS DECLARE VARIABLE idNfvenda TYPE OF COLUMN TB_NFVENDA.ID_NFVENDA; BEGIN S" +
                "ELECT ID_NFVENDA FROM TB_NFVENDA WHERE NF_NUMERO = :pNfNumero AND NF_SERIE = :pN" +
                "fSerie AND NF_MODELO = 59 INTO :idNfvenda ; DELETE FROM TB_NFV_CTAREC WHERE ID_N" +
                "FVENDA = :idNfvenda AND ID_CTAREC= :pIdCtarec ; END;\';\r\n\r\n\t\t\t\t\t\terro = \'sproc SP" +
                "_TRI_NFV_UPDT_BYNFNUMSERIE\';\r\n\t\t\t\t\t\texecute statement \'CREATE OR ALTER PROCEDURE" +
                " SP_TRI_NFV_UPDT_BYNFNUMSERIE ( pID_NATOPE TYPE OF COLUMN TB_NFVENDA.ID_NATOPE ," +
                " pID_VENDEDOR TYPE OF COLUMN TB_NFVENDA.ID_VENDEDOR , pID_CLIENTE TYPE OF COLUMN" +
                " TB_NFVENDA.ID_CLIENTE , pNF_NUMERO TYPE OF COLUMN TB_NFVENDA.NF_NUMERO , pNF_SE" +
                "RIE TYPE OF COLUMN TB_NFVENDA.NF_SERIE , pNF_MODELO TYPE OF COLUMN TB_NFVENDA.NF" +
                "_MODELO , pDT_EMISSAO TYPE OF COLUMN TB_NFVENDA.DT_EMISSAO , pDT_SAIDA TYPE OF C" +
                "OLUMN TB_NFVENDA.DT_SAIDA , pHR_SAIDA TYPE OF COLUMN TB_NFVENDA.HR_SAIDA , pESPE" +
                "CIE TYPE OF COLUMN TB_NFVENDA.ESPECIE , pTIPO_FRETE TYPE OF COLUMN TB_NFVENDA.TI" +
                "PO_FRETE , pPES_LIQUID TYPE OF COLUMN TB_NFVENDA.PES_LIQUID , pPES_BRUTO TYPE OF" +
                " COLUMN TB_NFVENDA.PES_BRUTO , pSTATUS TYPE OF COLUMN TB_NFVENDA.STATUS , pENT_S" +
                "AI TYPE OF COLUMN TB_NFVENDA.ENT_SAI , pID_FMAPGTO TYPE OF COLUMN TB_NFVENDA.ID_" +
                "FMAPGTO , pID_PARCELA TYPE OF COLUMN TB_NFVENDA.ID_PARCELA , pMARCA TYPE OF COLU" +
                "MN TB_NFVENDA.MARCA , pQTD_VOLUM TYPE OF COLUMN TB_NFVENDA.QTD_VOLUM , pNUM_VOLU" +
                "M TYPE OF COLUMN TB_NFVENDA.NUM_VOLUM , pPROD_REV TYPE OF COLUMN TB_NFVENDA.PROD" +
                "_REV , pSOMA_FRETE TYPE OF COLUMN TB_NFVENDA.SOMA_FRETE , pVLR_TROCO TYPE OF COL" +
                "UMN TB_NFVENDA.VLR_TROCO , pIND_PRES TYPE OF COLUMN TB_NFVENDA.IND_PRES , pIND_I" +
                "E_DEST TYPE OF COLUMN TB_NFVENDA.IND_IE_DEST , pDESCONTO_CONDICIONAL TYPE OF COL" +
                "UMN TB_NFVENDA.DESCONTO_CONDICIONAL , pINF_COMP_FIXA TYPE OF COLUMN TB_NFVENDA.I" +
                "NF_COMP_FIXA , pINF_COMP_EDIT TYPE OF COLUMN TB_NFVENDA.INF_COMP_EDIT , pENDEREC" +
                "O_ENTREGA TYPE OF COLUMN TB_NFVENDA.ENDERECO_ENTREGA , pENVIO_API TYPE OF COLUMN" +
                " TB_NFVENDA.ENVIO_API , pSYNCED TYPE OF COLUMN TB_NFVENDA.SYNCED ) AS BEGIN UPDA" +
                "TE TB_NFVENDA SET ID_NATOPE =:pID_NATOPE , ID_VENDEDOR =:pID_VENDEDOR , ID_CLIEN" +
                "TE =:pID_CLIENTE , NF_MODELO =:pNF_MODELO , DT_EMISSAO =:pDT_EMISSAO , DT_SAIDA " +
                "=:pDT_SAIDA , HR_SAIDA =:pHR_SAIDA , ESPECIE =:pESPECIE , TIPO_FRETE =:pTIPO_FRE" +
                "TE , PES_LIQUID =:pPES_LIQUID , PES_BRUTO =:pPES_BRUTO , STATUS =:pSTATUS , ENT_" +
                "SAI =:pENT_SAI , ID_FMAPGTO =:pID_FMAPGTO , ID_PARCELA =:pID_PARCELA , MARCA =:p" +
                "MARCA , QTD_VOLUM =:pQTD_VOLUM , NUM_VOLUM =:pNUM_VOLUM , PROD_REV =:pPROD_REV ," +
                " SOMA_FRETE =:pSOMA_FRETE , VLR_TROCO =:pVLR_TROCO , IND_PRES =:pIND_PRES , IND_" +
                "IE_DEST =:pIND_IE_DEST , DESCONTO_CONDICIONAL=:pDESCONTO_CONDICIONAL , INF_COMP_" +
                "FIXA =:pINF_COMP_FIXA , INF_COMP_EDIT =:pINF_COMP_EDIT , ENDERECO_ENTREGA =:pEND" +
                "ERECO_ENTREGA , ENVIO_API =:pENVIO_API , SYNCED =:pSYNCED WHERE NF_NUMERO = :pNF" +
                "_NUMERO and NF_SERIE = :pNF_SERIE ; END;\';\r\n\r\n\t\t\t\t\t\terro = \'sproc SP_TRI_SAT_GET" +
                "_BY_IDNFV\';\r\n\t\t\t\t\t\texecute statement \'CREATE OR ALTER PROCEDURE SP_TRI_SAT_GET_B" +
                "Y_IDNFV ( pIdNfvenda TYPE OF COLUMN TB_SAT.ID_NFVENDA ) RETURNS ( ID_REGISTRO TY" +
                "PE OF COLUMN TB_SAT.ID_REGISTRO , ID_NFVENDA TYPE OF COLUMN TB_SAT.ID_NFVENDA , " +
                "CHAVE TYPE OF COLUMN TB_SAT.CHAVE , DT_EMISSAO TYPE OF COLUMN TB_SAT.DT_EMISSAO " +
                ", HR_EMISSAO TYPE OF COLUMN TB_SAT.HR_EMISSAO , STATUS TYPE OF COLUMN TB_SAT.STA" +
                "TUS , STATUS_DES TYPE OF COLUMN TB_SAT.STATUS_DES , NUMERO_CFE TYPE OF COLUMN TB" +
                "_SAT.NUMERO_CFE , NUM_SERIE_SAT TYPE OF COLUMN TB_SAT.NUM_SERIE_SAT) AS BEGIN FO" +
                "R SELECT a.ID_REGISTRO , a.ID_NFVENDA , a.CHAVE , a.DT_EMISSAO , a.HR_EMISSAO , " +
                "a.STATUS , a.STATUS_DES , a.NUMERO_CFE , a.NUM_SERIE_SAT FROM TB_SAT a WHERE a.I" +
                "D_NFVENDA =:pIdNfvenda INTO :ID_REGISTRO , :ID_NFVENDA , :CHAVE , :DT_EMISSAO , " +
                ":HR_EMISSAO , :STATUS , :STATUS_DES , :NUMERO_CFE , :NUM_SERIE_SAT DO SUSPEND ; " +
                "END;\';\r\n\r\n\t\t\t\t\t\terro = \'deu certo\';\r\n\r\n\t\t\t\t\t\tSUSPEND;\r\n\t\t\t\t\t\tWHEN ANY DO\r\n\t\t\t\t\t\t" +
                "BEGIN\r\n\t\t\t\t\t\tEND\r\n\t\t\t\t\t\tEND;";
            ...
            this._commandCollection[21].CommandText = "CREATE OR ALTER PROCEDURE SP_TRI_PROCEDURES8\r\nRETURNS\r\n(\r\nerro VARCHAR(64)\r\n)\r\nAS" +
                ...
                "oc SP_TRI_CONTANFVPAGTO\';\r\n\t\t\t\t\t\texecute statement \'CREATE OR ALTER PROCEDURE SP" +
                "_TRI_CONTANFVPAGTO (PNF_SERIE VARCHAR(3), PTS_ABERTURA TIMESTAMP, PTS_FECHAMENTO" +
                " TIMESTAMP, PSTATUS CHAR(1) = \'\'I\'\') RETURNS (RCOUNT_FMANCE INTEGER,\tRID_FMANCFE" +
                " INTEGER) AS BEGIN FOR SELECT COUNT(A.ID_FMANFCE), A.ID_FMANFCE FROM TB_NFVENDA_" +
                "FMAPAGTO_NFCE A JOIN TB_NFVENDA B ON A.ID_NFVENDA = B.ID_NFVENDA WHERE B.NF_SERI" +
                "E = :pNF_SERIE AND CAST ((B.DT_SAIDA || \'\' \'\' || B.HR_SAIDA) AS TIMESTAMP) BETWE" +
                "EN :pTS_ABERTURA AND :pTS_FECHAMENTO AND B.STATUS = :PSTATUS GROUP BY A.ID_FMANF" +
                "CE INTO :rCOUNT_FMANCE, :rID_FMANCFE DO SUSPEND; END\';\r\n\r\n\t\t\t\t\t\terro = \'sproc SP" +
                "_TRI_NFVCTAREC\';\r\n\t\t\t\t\t\texecute statement \'CREATE OR ALTER PROCEDURE SP_TRI_NFVC" +
                "TAREC (pID_NFVENDA INTEGER, pDT_VENCIMENTO DATE, pDESCRICAO VARCHAR(50), pID_CLI" +
                "ENTE INTEGER, pID_NUMPAG INTEGER) RETURNS (rID_CTAREC INTEGER) AS DECLARE VARIAB" +
                ...
                "DB$DATABASE INTO :rID_CTAREC; SELECT NF_SERIE, NF_NUMERO FROM TB_NFVENDA WHERE I" +
                "D_NFVENDA = :pID_NFVENDA INTO :vNF_SERIE, :vNF_NUMERO; SELECT VLR_PAGTO FROM TB_" +
                "NFVENDA_FMAPAGTO_NFCE WHERE ID_NUMPAG = :pID_NUMPAG INTO :vVALOR; INSERT INTO TB" +
                "_CONTA_RECEBER (ID_CTAREC, DOCUMENTO, HISTORICO, DT_EMISSAO, DT_VENCTO, VLR_CTAR" +
                "EC, TIP_CTAREC, ID_PORTADOR, ID_CLIENTE, INV_REFERENCIA) VALUES (:rID_CTAREC, (C" +
                "AST(:vNF_SERIE AS VARCHAR(12)) || \'\'-\'\' || CAST(:vNF_NUMERO AS VARCHAR(12))), :p" +
                "DESCRICAO, CURRENT_DATE, :pDT_VENCIMENTO, :vVALOR, \'\'C\'\', 1, :pID_CLIENTE, \'\'D\'\'" +
                " || LPAD(:rID_CTAREC,7,\'\'0\'\') || LPAD(:vNF_SERIE, 2, \'\'0\'\') || \'\'X\'\' || LPAD(:vN" +
                "F_NUMERO, 7, \'\'0\'\')); INSERT INTO TB_NFV_CTAREC (ID_NFVENDA, ID_CTAREC, ID_NUMPA" +
                "G) VALUES (:pID_NFVENDA, :rID_CTAREC, :pID_NUMPAG); SUSPEND; END\';\r\n\r\n\t\t\t\t\t\terro" +
                " = \'sproc SP_TRI_LANCAMOVDIARIO\';\r\n\t\t\t\t\t\texecute statement \'CREATE OR ALTER PROC" +
                "EDURE SP_TRI_LANCAMOVDIARIO (NUMCAIXA VARCHAR(60), VALOR NUMERIC(18,4), DESCRICA" +
                "O_MOV VARCHAR(60), CONTA_ORIGEM SMALLINT, CONTA_DESTINO SMALLINT) RETURNS (ID_MO" +
                "VTO INTEGER) AS DECLARE VARIABLE ultimo_mov INTEGER; BEGIN SELECT NEXT VALUE FOR" +
                " GEN_TB_MOVDIARIO_ID FROM RDB$DATABASE INTO :ID_MOVTO; INSERT INTO TB_MOVDIARIO " +
                "(ID_MOVTO, DT_MOVTO, HR_MOVTO, HISTORICO, TIP_MOVTO, VLR_MOVTO, ID_CTAPLA) VALUE" +
                "S ((SELECT NEXT VALUE FOR GEN_TB_MOVDIARIO_ID FROM RDB$DATABASE), CURRENT_DATE, " +
                "CURRENT_TIME, :DESCRICAO_MOV, \'\'C\'\', :VALOR, :CONTA_ORIGEM); INSERT INTO TB_MOVD" +
                "IARIO (ID_MOVTO, DT_MOVTO, HR_MOVTO, HISTORICO, TIP_MOVTO, VLR_MOVTO, ID_CTAPLA)" +
                " VALUES (:ID_MOVTO, CURRENT_DATE, CURRENT_TIME, :DESCRICAO_MOV, \'\'D\'\', :VALOR, :" +
                "CONTA_DESTINO); SUSPEND; END\';\r\n\r\n\t\t\t\t\t\terro = \'sproc SP_TRI_NFVFMAPGTO_INSERT\';" +
                "\r\n\t\t\t\t\t\texecute statement \'CREATE OR ALTER PROCEDURE SP_TRI_NFVFMAPGTO_INSERT (P" +
                "VALOR TYPE OF COLUMN TB_NFVENDA_FMAPAGTO_NFCE.VLR_PAGTO, PID_NFVENDA INTEGER, PI" +
                "D_FMANFCE INTEGER, PID_PARCELA INTEGER, PID_ADMINISTRADORA INTEGER) RETURNS (RID" +
                "_FMAPAGTO INTEGER) AS BEGIN SELECT NEXT VALUE FOR gen_tb_nfvenda_fmapagto_nfce_i" +
                "d FROM RDB$DATABASE INTO :rID_FMAPAGTO; INSERT INTO TB_NFVENDA_FMAPAGTO_NFCE (ID" +
                "_NUMPAG, VLR_PAGTO, ID_NFVENDA, ID_FMANFCE, ID_PARCELA) VALUES (:rID_FMAPAGTO, :" +
                "pVALOR, :pID_NFVENDA, :pID_FMANFCE, :pID_PARCELA); INSERT INTO TB_NFCE_BANDEIRA " +
                "(ID_NUMPAG, ID_BANDEIRA, ID_ADMINISTRADORA, NUM_AUT, FORMA_ENVIO, SICLOS_ID, STO" +
                "NE_ID, PRE_TRANSACAO_ID, POS_SERIAL) VALUES (:rID_FMAPAGTO, NULL, :pID_ADMINISTR" +
                "ADORA, NULL, NULL, NULL, NULL, NULL, NULL); END\';\r\n\r\n\t\t\t\t\t\terro = \'sproc SP_TRI_" +
                "WHATSINSERE\';\r\n\t\t\t\t\t\texecute statement \'CREATE OR ALTER PROCEDURE SP_TRI_WHATSIN" +
                "SERE (PNUMERO VARCHAR(100), PMENSAGEM BLOB SUB_TYPE TEXT, PDATA TIMESTAMP, PENVI" +
                "ADA VARCHAR(100), PCNPJ VARCHAR(50)) AS BEGIN INSERT INTO TRI_PDV_WHATS (NUMERO," +
                " MENSAGEM, DATAENVIO, ENVIADA,CNPJ) VALUES (:PNUMERO, :PMENSAGEM,:PDATA,:PENVIAD" +
                "A,:PCNPJ); END\';\r\n\r\n\t\t\t\t\t\terro = \'sproc SP_TRI_RENDIMENTO_SOMA\';\r\n\t\t\t\t\t\texecute " +
                "statement \'CREATE OR ALTER PROCEDURE SP_TRI_RENDIMENTO_SOMA (PDATA_INICIAL TIMES" +
                "TAMP, PDATA_FINAL TIMESTAMP, CAIXA VARCHAR(3)) RETURNS (RSOMA DECIMAL(18,4), RDE" +
                "SCRICAO VARCHAR(30)) AS BEGIN FOR SELECT SUM(tni.VLR_TOTAL - tni.VLR_DESC),tetis" +
                ".DESCRICAO FROM TB_NFV_ITEM tni JOIN TB_NFVENDA tn ON tni.ID_NFVENDA = tn.ID_NFV" +
                "ENDA JOIN TB_EST_IDENTIFICADOR tei ON tni.ID_IDENTIFICADOR = tei.ID_IDENTIFICADO" +
                "R JOIN TB_ESTOQUE te ON tei.ID_ESTOQUE = te.ID_ESTOQUE JOIN TB_EST_TIPO_ITEM_SIS" +
                " tetis ON te.ID_TIPOITEM = tetis.ID_TIPOITEM WHERE (tn.NF_SERIE = \'\'N\'\'||:CAIXA " +
                ...
                "                                                                   erro = \'sproc" +
                " SP_TRI_UPDATE_NF_TO_F\';\r\n\t\t\t\t\t\texecute statement \'CREATE OR ALTER PROCEDURE SP_" +
                "TRI_UPDATE_NF_TO_F (PNEW_NF_NUMERO INTEGER, PNEW_NF_SERIE VARCHAR(3), PNEW_DT_SA" +
                "IDA DATE, PNEW_HR_SAIDA TIME, PSYNCED SMALLINT, POLD_NF_NUMERO INTEGER, POLD_NF_" +
                "SERIE VARCHAR(3), POLD_NF_MODELO VARCHAR(3)) RETURNS ( RID_ALTERADO INTEGER ) AS" +
                " BEGIN  UPDATE TB_NFVENDA SET NF_NUMERO = :PNEW_NF_NUMERO, NF_SERIE = :PNEW_NF_S" +
                "ERIE, DT_SAIDA = :PNEW_DT_SAIDA, HR_SAIDA = :PNEW_HR_SAIDA, SYNCED = :PSYNCED WH" +
                "ERE NF_NUMERO = :POLD_NF_NUMERO AND NF_SERIE = :POLD_NF_SERIE AND NF_MODELO = :P" +
                "OLD_NF_MODELO; FOR SELECT ID_NFVENDA FROM TB_NFVENDA WHERE NF_NUMERO = :PNEW_NF_" +
                "NUMERO AND NF_SERIE = :PNEW_NF_SERIE AND NF_MODELO = :POLD_NF_MODELO INTO :RID_A" +
                "LTERADO DO  BEGIN  SUSPEND; END END\';\r\n\r\n\t\t\t\t\t\terro = \'deu certo\';\r\n\r\n\t\t\t\t\t\tSUSP" +
                "END;\r\n\t\t\t\t\t\tWHEN ANY DO\r\n\t\t\t\t\t\tBEGIN\r\n\t\t\t\t\t\tEND\r\n\t\t\t\t\t\tEND;";
