...
END;";
            ...
            this._commandCollection[17].CommandText = "CREATE OR ALTER PROCEDURE SP_TRI_PROCEDURES4\r\nRETURNS\r\n(\r\nerro VARCHAR(64)\r\n)\r\nAS" +
                ...
                " SP_TRI_ESTOQUE_UPSERT ( pID_ESTOQUE INTEGER , pID_GRUPO INTEGER , pDESCRICAO VA" +
                "RCHAR(50) , pSTATUS VARCHAR(1) , pDT_CADAST DATE , pHR_CADAST TIME , pFRACIONADO" +
                " CHAR(1) , pPRC_VENDA TYPE OF COLUMN TRI_PDV_OPER.DIN , pPRC_CUSTO TYPE OF COLUM" +
                "N TRI_PDV_OPER.DIN , pULT_VENDA DATE , pMARGEM_LB TYPE OF COLUMN TRI_PDV_OPER.DI" +
                "N , pPOR_COMISSAO TYPE OF COLUMN TRI_PDV_OPER.DIN , pULT_FORNEC INTEGER , pGRADE" +
                "_SERIE CHAR(1) , pID_TIPOITEM VARCHAR(2) , pID_CTI CHAR(3) , pCST_PIS VARCHAR(3)" +
                " , pCST_COFINS VARCHAR(3) , pPIS TYPE OF COLUMN TRI_PDV_OPER.DIN , pCOFINS TYPE " +
                "OF COLUMN TRI_PDV_OPER.DIN , pUNI_MEDIDA VARCHAR(3) , pMARGEM_PV TYPE OF COLUMN " +
                "TRI_PDV_OPER.DIN , pCFOP VARCHAR(4) , pOBSERVACAO BLOB SUB_TYPE 1 , pNAT_RECEITA" +
                " SMALLINT , pCFOP_NF VARCHAR(4) , pPRC_ATACADO TYPE OF COLUMN TRI_PDV_OPER.DIN ," +
                " pID_CTI_PART CHAR(3) , pID_CTI_FCP CHAR(3) , pQTD_ATACADO TYPE OF COLUMN TRI_PD" +
                "V_OPER.DIN , pID_CTI_CFE CHAR(4) , pTRI_PDV_DT_UPD TIMESTAMP ) RETURNS (rRowsAff" +
                "ected INTEGER) AS BEGIN UPDATE OR INSERT INTO TB_ESTOQUE (ID_ESTOQUE , ID_GRUPO " +
                ", DESCRICAO , STATUS , DT_CADAST , HR_CADAST , FRACIONADO , PRC_VENDA , PRC_CUST" +
                "O , ULT_VENDA , MARGEM_LB , POR_COMISSAO , ULT_FORNEC , GRADE_SERIE , ID_TIPOITE" +
                "M , ID_CTI , CST_PIS , CST_COFINS , PIS , COFINS , UNI_MEDIDA , MARGEM_PV , CFOP" +
                " , OBSERVACAO , NAT_RECEITA , CFOP_NF , PRC_ATACADO , ID_CTI_PART , ID_CTI_FCP ," +
                " QTD_ATACADO , ID_CTI_CFE , TRI_PDV_DT_UPD ) VALUES (:pID_ESTOQUE , :pID_GRUPO ," +
                " :pDESCRICAO , :pSTATUS , :pDT_CADAST , :pHR_CADAST , :pFRACIONADO , :pPRC_VENDA" +
                " , :pPRC_CUSTO , :pULT_VENDA , :pMARGEM_LB , :pPOR_COMISSAO , :pULT_FORNEC , :pG" +
                "RADE_SERIE , :pID_TIPOITEM , :pID_CTI , :pCST_PIS , :pCST_COFINS , :pPIS , :pCOF" +
                "INS , :pUNI_MEDIDA , :pMARGEM_PV , :pCFOP , :pOBSERVACAO , :pNAT_RECEITA , :pCFO" +
                "P_NF , :pPRC_ATACADO , :pID_CTI_PART , :pID_CTI_FCP , :pQTD_ATACADO , :pID_CTI_C" +
                "FE , :pTRI_PDV_DT_UPD ) MATCHING (ID_ESTOQUE ) ; rRowsAffected = ROW_COUNT; END;" +
                "\';\r\n\r\n\t\t\t\t\t\terro = \'sproc SP_TRI_MOV_CTAREC\';\r\n\t\t\t\t\t\texecute statement \'CREATE O" +
                "R ALTER PROCEDURE SP_TRI_MOV_CTAREC ( NO_CUPOM VARCHAR(12) ) AS DECLARE CTAREC i" +
                "nteger; DECLARE MOVDIA integer; BEGIN SELECT FIRST 1 ID_CTAREC FROM TB_CONTA_REC" +
                "EBER WHERE DOCUMENTO CONTAINING :no_cupom ORDER BY ID_CTAREC DESC INTO CTAREC ; " +
                "SELECT FIRST 1 ID_MOVTO FROM TB_MOVDIARIO WHERE HISTORICO CONTAINING :no_cupom O" +
                "RDER BY ID_MOVTO DESC INTO MOVDIA ; INSERT INTO TB_CTAREC_MOVTO (ID_MOVTO , ID_C" +
                "TAREC ) VALUES (:MOVDIA , :CTAREC ) ; SUSPEND; WHEN ANY DO BEGIN SUSPEND; END EN" +
                "D\';\r\n\r\n\t\t\t\t\t\terro = \'sproc SP_TRI_LISTAFECHAMENTOS\';\r\n\t\t\t\t\t\texecute statement \'C" +
                "REATE OR ALTER PROCEDURE SP_TRI_LISTAFECHAMENTOS ( DATA_INICIAL TIMESTAMP , DATA" +
                "_FINAL TIMESTAMP ) RETURNS ( FECHADO TIMESTAMP , ID_CAIXA INTEGER , OPERADOR INT" +
                "EGER , TOTALVENDA TYPE OF COLUMN TRI_PDV_OPER.DIN ) AS BEGIN for SELECT FECHADO " +
                ", ID_CAIXA , OPERADOR , (DIN + CHEQUE + CREDITO + DEBITO + LOJA + ALIMENTACAO + " +
                "REFEICAO + PRESENTE + COMBUSTIVEL + OUTROS + EXTRA_1 + EXTRA_2 + EXTRA_3 + EXTRA" +
                "_4 + EXTRA_5 + EXTRA_6 + EXTRA_7 + EXTRA_8 + EXTRA_9 + EXTRA_10) AS TotalVendaS " +
                "FROM TRI_PDV_FECHAMENTOS WHERE FECHADO >= :DATA_INICIAL AND FECHADO <= :DATA_FIN" +
                "AL INTO :FECHADO , :ID_CAIXA , :OPERADOR , :TOTALVENDA do SUSPEND ; END;\';\r\nerro" +
                " = \'sproc fechamento get all unsynced\';\r\nexecute statement \'CREATE OR ALTER PROC" +
                "EDURE SP_TRI_FECH_GETALL_UNSYNCED RETURNS ( DIN TYPE OF COLUMN TRI_PDV_OPER.DIN " +
                ", CHEQUE TYPE OF COLUMN TRI_PDV_OPER.DIN , CREDITO TYPE OF COLUMN TRI_PDV_OPER.D" +
                "IN , DEBITO TYPE OF COLUMN TRI_PDV_OPER.DIN , LOJA TYPE OF COLUMN TRI_PDV_OPER.D" +
                "IN , ALIMENTACAO TYPE OF COLUMN TRI_PDV_OPER.DIN , REFEICAO TYPE OF COLUMN TRI_P" +
                "DV_OPER.DIN , PRESENTE TYPE OF COLUMN TRI_PDV_OPER.DIN , COMBUSTIVEL TYPE OF COL" +
                "UMN TRI_PDV_OPER.DIN , OUTROS TYPE OF COLUMN TRI_PDV_OPER.DIN , EXTRA_1 TYPE OF " +
                "COLUMN TRI_PDV_OPER.DIN , EXTRA_2 TYPE OF COLUMN TRI_PDV_OPER.DIN , EXTRA_3 TYPE" +
                " OF COLUMN TRI_PDV_OPER.DIN , EXTRA_4 TYPE OF COLUMN TRI_PDV_OPER.DIN , EXTRA_5 " +
                "TYPE OF COLUMN TRI_PDV_OPER.DIN , EXTRA_6 TYPE OF COLUMN TRI_PDV_OPER.DIN , EXTR" +
                "A_7 TYPE OF COLUMN TRI_PDV_OPER.DIN , EXTRA_8 TYPE OF COLUMN TRI_PDV_OPER.DIN , " +
                "EXTRA_9 TYPE OF COLUMN TRI_PDV_OPER.DIN , EXTRA_10 TYPE OF COLUMN TRI_PDV_OPER.D" +
                "IN , OPERADOR INTEGER , ID_CAIXA INTEGER , FECHADO TIMESTAMP , SANGRIAS TYPE OF " +
                "COLUMN TRI_PDV_OPER.DIN , SUPRIMENTOS TYPE OF COLUMN TRI_PDV_OPER.DIN , TROCAS T" +
                "YPE OF COLUMN TRI_PDV_OPER.DIN , SYNCED SMALLINT ) AS BEGIN FOR SELECT FIRST 200" +
                " DIN , CHEQUE , CREDITO , DEBITO , LOJA , ALIMENTACAO , REFEICAO , PRESENTE , CO" +
                "MBUSTIVEL , OUTROS , EXTRA_1 , EXTRA_2 , EXTRA_3 , EXTRA_4 , EXTRA_5 , EXTRA_6 ," +
                " EXTRA_7 , EXTRA_8 , EXTRA_9 , EXTRA_10 , OPERADOR , ID_CAIXA , FECHADO , SANGRI" +
                "AS , SUPRIMENTOS , TROCAS , SYNCED FROM TRI_PDV_FECHAMENTOS a WHERE ( a.SYNCED I" +
                "S NULL OR a.SYNCED = 0 ) ORDER BY a.FECHADO INTO :DIN , :CHEQUE , :CREDITO , :DE" +
                "BITO , :LOJA , :ALIMENTACAO , :REFEICAO , :PRESENTE , :COMBUSTIVEL , :OUTROS , :" +
                "EXTRA_1 , :EXTRA_2 , :EXTRA_3 , :EXTRA_4 , :EXTRA_5 , :EXTRA_6 , :EXTRA_7 , :EXT" +
                "RA_8 , :EXTRA_9 , :EXTRA_10 , :OPERADOR , :ID_CAIXA , :FECHADO , :SANGRIAS , :SU" +
                "PRIMENTOS , :TROCAS , :SYNCED DO BEGIN SUSPEND ; END END;\';\r\nerro = \'sproc fecha" +
                "mento sync insert\';\r\nexecute statement \'CREATE OR ALTER PROCEDURE SP_TRI_FECH_SY" +
                "NC_INSERT ( pDIN TYPE OF COLUMN TRI_PDV_OPER.DIN , pCHEQUE TYPE OF COLUMN TRI_PD" +
                "V_OPER.DIN , pCREDITO TYPE OF COLUMN TRI_PDV_OPER.DIN , pDEBITO TYPE OF COLUMN T" +
                "RI_PDV_OPER.DIN , pLOJA TYPE OF COLUMN TRI_PDV_OPER.DIN , pALIMENTACAO TYPE OF C" +
                "OLUMN TRI_PDV_OPER.DIN , pREFEICAO TYPE OF COLUMN TRI_PDV_OPER.DIN , pPRESENTE T" +
                "YPE OF COLUMN TRI_PDV_OPER.DIN , pCOMBUSTIVEL TYPE OF COLUMN TRI_PDV_OPER.DIN , " +
                "pOUTROS TYPE OF COLUMN TRI_PDV_OPER.DIN , pEXTRA_1 TYPE OF COLUMN TRI_PDV_OPER.D" +
                "IN , pEXTRA_2 TYPE OF COLUMN TRI_PDV_OPER.DIN , pEXTRA_3 TYPE OF COLUMN TRI_PDV_" +
                "OPER.DIN , pEXTRA_4 TYPE OF COLUMN TRI_PDV_OPER.DIN , pEXTRA_5 TYPE OF COLUMN TR" +
                "I_PDV_OPER.DIN , pEXTRA_6 TYPE OF COLUMN TRI_PDV_OPER.DIN , pEXTRA_7 TYPE OF COL" +
                "UMN TRI_PDV_OPER.DIN , pEXTRA_8 TYPE OF COLUMN TRI_PDV_OPER.DIN , pEXTRA_9 TYPE " +
                "OF COLUMN TRI_PDV_OPER.DIN , pEXTRA_10 TYPE OF COLUMN TRI_PDV_OPER.DIN , pOPERAD" +
                "OR INTEGER , pID_CAIXA INTEGER , pFECHADO TIMESTAMP , pSANGRIAS TYPE OF COLUMN T" +
                "RI_PDV_OPER.DIN , pSUPRIMENTOS TYPE OF COLUMN TRI_PDV_OPER.DIN , pTROCAS TYPE OF" +
                " COLUMN TRI_PDV_OPER.DIN , pSYNCED SMALLINT ) AS BEGIN INSERT INTO TRI_PDV_FECHA" +
                "MENTOS (DIN , CHEQUE , CREDITO , DEBITO , LOJA , ALIMENTACAO , REFEICAO , PRESEN" +
                "TE , COMBUSTIVEL , OUTROS , EXTRA_1 , EXTRA_2 , EXTRA_3 , EXTRA_4 , EXTRA_5 , EX" +
                "TRA_6 , EXTRA_7 , EXTRA_8 , EXTRA_9 , EXTRA_10 , OPERADOR , ID_CAIXA , FECHADO ," +
                " SANGRIAS , SUPRIMENTOS , TROCAS , SYNCED ) VALUES (:pDIN , :pCHEQUE , :pCREDITO" +
                " , :pDEBITO , :pLOJA , :pALIMENTACAO , :pREFEICAO , :pPRESENTE , :pCOMBUSTIVEL ," +
                " :pOUTROS , :pEXTRA_1 , :pEXTRA_2 , :pEXTRA_3 , :pEXTRA_4 , :pEXTRA_5 , :pEXTRA_" +
                "6 , :pEXTRA_7 , :pEXTRA_8 , :pEXTRA_9 , :pEXTRA_10 , :pOPERADOR , :pID_CAIXA , :" +
                ...
                "RI_FECH_SETSYNCED ( pID_CAIXA INTEGER , pFECHADO TIMESTAMP , pSYNCED SMALLINT ) " +
                "AS BEGIN UPDATE TRI_PDV_FECHAMENTOS SET SYNCED = :pSYNCED WHERE ID_CAIXA = :pID_" +
                "CAIXA AND FECHADO = :pFECHADO ; END;\';\r\n\r\nerro = \'deu certo\';\r\n\r\nSUSPEND;\r\nWHEN " +
                "ANY DO\r\nBEGIN\r\nEND\r\nEND;";
            ...
            this._commandCollection[20].CommandText = "\r\n\t\t\t\t\t\tCREATE OR ALTER PROCEDURE SP_TRI_PROCEDURES7\r\n\t\t\t\t\t\tRETURNS\r\n\t\t\t\t\t\t(\r\n\t\t\t" +
                ...
                "ute statement \'CREATE OR ALTER PROCEDURE SP_TRI_ESTOQUE_ID_GETBY_ID ( pID_ESTOQU" +
                "E TYPE OF COLUMN TB_ESTOQUE.ID_ESTOQUE ) RETURNS ( ID_ESTOQUE TYPE OF COLUMN TB_" +
                "ESTOQUE.ID_ESTOQUE , ID_GRUPO TYPE OF COLUMN TB_ESTOQUE.ID_GRUPO , DESCRICAO TYP" +
                "E OF COLUMN TB_ESTOQUE.DESCRICAO , STATUS TYPE OF COLUMN TB_ESTOQUE.STATUS , DT_" +
                "CADAST TYPE OF COLUMN TB_ESTOQUE.DT_CADAST , HR_CADAST TYPE OF COLUMN TB_ESTOQUE" +
                ".HR_CADAST , FRACIONADO TYPE OF COLUMN TB_ESTOQUE.FRACIONADO , PRC_VENDA TYPE OF" +
                " COLUMN TRI_PDV_OPER.DIN , PRC_CUSTO TYPE OF COLUMN TRI_PDV_OPER.DIN , ULT_VENDA" +
                " TYPE OF COLUMN TB_ESTOQUE.ULT_VENDA , MARGEM_LB TYPE OF COLUMN TRI_PDV_OPER.DIN" +
                " , POR_COMISSAO TYPE OF COLUMN TRI_PDV_OPER.DIN , ULT_FORNEC TYPE OF COLUMN TB_E" +
                "STOQUE.ULT_FORNEC , GRADE_SERIE TYPE OF COLUMN TB_ESTOQUE.GRADE_SERIE , ID_TIPOI" +
                "TEM TYPE OF COLUMN TB_ESTOQUE.ID_TIPOITEM , ID_CTI TYPE OF COLUMN TB_ESTOQUE.ID_" +
                "CTI , CST_PIS TYPE OF COLUMN TB_ESTOQUE.CST_PIS , CST_COFINS TYPE OF COLUMN TB_E" +
                "STOQUE.CST_COFINS , PIS TYPE OF COLUMN TRI_PDV_OPER.DIN , COFINS TYPE OF COLUMN " +
                "TRI_PDV_OPER.DIN , UNI_MEDIDA TYPE OF COLUMN TB_ESTOQUE.UNI_MEDIDA , MARGEM_PV T" +
                "YPE OF COLUMN TRI_PDV_OPER.DIN , CFOP TYPE OF COLUMN TB_ESTOQUE.CFOP , OBSERVACA" +
                "O TYPE OF COLUMN TB_ESTOQUE.OBSERVACAO , NAT_RECEITA TYPE OF COLUMN TB_ESTOQUE.N" +
                "AT_RECEITA , CFOP_NF TYPE OF COLUMN TB_ESTOQUE.CFOP_NF , PRC_ATACADO TYPE OF COL" +
                "UMN TRI_PDV_OPER.DIN , ID_CTI_PART TYPE OF COLUMN TB_ESTOQUE.ID_CTI_PART , ID_CT" +
                "I_FCP TYPE OF COLUMN TB_ESTOQUE.ID_CTI_FCP , QTD_ATACADO TYPE OF COLUMN TRI_PDV_" +
                "OPER.DIN , TRI_PDV_DT_UPD TYPE OF COLUMN TB_ESTOQUE.TRI_PDV_DT_UPD , ID_IDENTIFI" +
                "CADOR TYPE OF COLUMN TB_EST_IDENTIFICADOR.ID_IDENTIFICADOR , ID_CTI_CFE TYPE OF " +
                "COLUMN TB_ESTOQUE.ID_CTI_CFE) AS BEGIN FOR SELECT A.ID_ESTOQUE , A.ID_GRUPO , A." +
                "DESCRICAO , A.STATUS , A.DT_CADAST , A.HR_CADAST , A.FRACIONADO , A.PRC_VENDA , " +
                "A.PRC_CUSTO , A.ULT_VENDA , A.MARGEM_LB , A.POR_COMISSAO , A.ULT_FORNEC , A.GRAD" +
                "E_SERIE , A.ID_TIPOITEM , A.ID_CTI , A.CST_PIS , A.CST_COFINS , A.PIS , A.COFINS" +
                " , A.UNI_MEDIDA , A.MARGEM_PV , A.CFOP , A.OBSERVACAO , A.NAT_RECEITA , A.CFOP_N" +
                "F , A.PRC_ATACADO , A.ID_CTI_PART , A.ID_CTI_FCP , A.QTD_ATACADO , A.TRI_PDV_DT_" +
                "UPD , B.ID_IDENTIFICADOR , A.ID_CTI_CFE FROM TB_ESTOQUE A JOIN TB_EST_IDENTIFICA" +
                "DOR B on B.ID_ESTOQUE = A.ID_ESTOQUE WHERE A.ID_ESTOQUE = :pID_ESTOQUE INTO :ID_" +
                "ESTOQUE , :ID_GRUPO , :DESCRICAO , :STATUS , :DT_CADAST , :HR_CADAST , :FRACIONA" +
                "DO , :PRC_VENDA , :PRC_CUSTO , :ULT_VENDA , :MARGEM_LB , :POR_COMISSAO , :ULT_FO" +
                "RNEC , :GRADE_SERIE , :ID_TIPOITEM , :ID_CTI , :CST_PIS , :CST_COFINS , :PIS , :" +
                "COFINS , :UNI_MEDIDA , :MARGEM_PV , :CFOP , :OBSERVACAO , :NAT_RECEITA , :CFOP_N" +
                "F , :PRC_ATACADO , :ID_CTI_PART , :ID_CTI_FCP , :QTD_ATACADO , :TRI_PDV_DT_UPD ," +
                " :ID_IDENTIFICADOR , :ID_CTI_CFE DO BEGIN SUSPEND ; END END;\';\r\n\r\n\t\t\t\t\t\terro = \'" +
                "sproc SP_TRI_TERMARIO_CHECKSEQ\';\r\n\t\t\t\t\t\texecute statement \'CREATE OR ALTER PROCE" +
                "DURE SP_TRI_TERMARIO_CHECKSEQ RETURNS(rNewNextSeqValue INTEGER) AS DECLARE VARIA" +
                "BLE vCurrentSeqValue INTEGER; DECLARE VARIABLE vMaxIdTermario TYPE OF COLUMN TRI" +
                "_PDV_TERMINAL_USUARIO.ID_OPER; DECLARE VARIABLE vDiffNextSeqValue INTEGER; DECLA" +
                "RE VARIABLE vNewNextSeqValue INTEGER; BEGIN vCurrentSeqValue = GEN_ID(GEN_TRI_PD" +
                "V_TERM_USUARIO, 0 ); vMaxIdTermario = (SELECT MAX(ID_OPER) FROM TRI_PDV_TERMINAL" +
                "_USUARIO); vNewNextSeqValue = -1; IF (:vMaxIdTermario > :vCurrentSeqValue) THEN " +
                "BEGIN vDiffNextSeqValue = (:vMaxIdTermario - GEN_ID(GEN_TRI_PDV_TERM_USUARIO, 0)" +
                "); vNewNextSeqValue = GEN_ID(GEN_TRI_PDV_TERM_USUARIO, :vDiffNextSeqValue); END " +
                "rNewNextSeqValue = :vNewNextSeqValue; END;\';\r\n\r\nerro = \'sproc SP_TRI_NATOPER_UPS" +
                "ERT\';\r\nexecute statement \'CREATE OR ALTER PROCEDURE SP_TRI_NATOPER_UPSERT ( pID_" +
                "NATOPE TYPE OF COLUMN TB_NAT_OPERACAO.ID_NATOPE , pDESCRICAO TYPE OF COLUMN TB_N" +
                "AT_OPERACAO.DESCRICAO , pRET_PIS_COF_CSLL TYPE OF COLUMN TB_NAT_OPERACAO.RET_PIS" +
                "_COF_CSLL , pRET_INSS TYPE OF COLUMN TB_NAT_OPERACAO.RET_INSS , pRET_IRRF TYPE O" +
                "F COLUMN TB_NAT_OPERACAO.RET_IRRF , pPIS_COFINS TYPE OF COLUMN TB_NAT_OPERACAO.P" +
                "IS_COFINS , pSTATUS TYPE OF COLUMN TB_NAT_OPERACAO.STATUS , pCFOP TYPE OF COLUMN" +
                " TB_NAT_OPERACAO.CFOP , pID_CTI TYPE OF COLUMN TB_NAT_OPERACAO.ID_CTI , pGFR TYP" +
                "E OF COLUMN TB_NAT_OPERACAO.GFR , pOBSERVACAO TYPE OF COLUMN TB_NAT_OPERACAO.OBS" +
                "ERVACAO , pBASE_COMISSAO TYPE OF COLUMN TB_NAT_OPERACAO.BASE_COMISSAO , pCALCULA" +
                "_IPI TYPE OF COLUMN TB_NAT_OPERACAO.CALCULA_IPI , pGRAVA_TOT_TRIBUTOS TYPE OF CO" +
                "LUMN TB_NAT_OPERACAO.GRAVA_TOT_TRIBUTOS ) RETURNS ( rRowsAffected INTEGER ) AS B" +
                "EGIN UPDATE OR INSERT INTO TB_NAT_OPERACAO (ID_NATOPE , DESCRICAO , RET_PIS_COF_" +
                "CSLL , RET_INSS , RET_IRRF , PIS_COFINS , STATUS , CFOP , ID_CTI , GFR , OBSERVA" +
                "CAO , BASE_COMISSAO , CALCULA_IPI , GRAVA_TOT_TRIBUTOS ) VALUES ( :pID_NATOPE , " +
                ":pDESCRICAO , :pRET_PIS_COF_CSLL , :pRET_INSS , :pRET_IRRF , :pPIS_COFINS , :pST" +
                "ATUS , :pCFOP , :pID_CTI , :pGFR , :pOBSERVACAO , :pBASE_COMISSAO , :pCALCULA_IP" +
                "I , :pGRAVA_TOT_TRIBUTOS ) MATCHING (ID_NATOPE ) ; rRowsAffected = ROW_COUNT; EN" +
                "D;\';\r\n\r\nerro = \'sproc SP_TRI_FMAPGTONFCE_UPSERT\';\r\nexecute statement \'CREATE OR " +
                "ALTER PROCEDURE SP_TRI_FMAPGTONFCE_UPSERT ( pID_FMANFCE TYPE OF COLUMN TB_FORMA_" +
                "PAGTO_NFCE.ID_FMANFCE , pSTATUS TYPE OF COLUMN TB_FORMA_PAGTO_NFCE.STATUS , pID_" +
                "NFCE TYPE OF COLUMN TB_FORMA_PAGTO_NFCE.ID_NFCE , pDESCRICAO TYPE OF COLUMN TB_F" +
                "ORMA_PAGTO_NFCE.DESCRICAO ) RETURNS ( rRowsAffected INTEGER ) AS BEGIN UPDATE OR" +
                " INSERT INTO TB_FORMA_PAGTO_NFCE (ID_FMANFCE , STATUS , ID_NFCE , DESCRICAO ) VA" +
                "LUES ( :pID_FMANFCE , :pSTATUS , :pID_NFCE , :pDESCRICAO ) MATCHING (ID_FMANFCE " +
                ") ; rRowsAffected = ROW_COUNT; END;\';\r\n\r\nerro = \'sproc SP_TRI_NFVENDA_GETALL_SYN" +
                "C\';\r\nexecute statement \'CREATE OR ALTER PROCEDURE SP_TRI_NFVENDA_GETALL_SYNC ( T" +
                "IP_QUERY SMALLINT ) RETURNS ( ID_NFVENDA TYPE OF COLUMN TB_NFVENDA.ID_NFVENDA , " +
                "ID_NATOPE TYPE OF COLUMN TB_NFVENDA.ID_NATOPE , ID_VENDEDOR TYPE OF COLUMN TB_NF" +
                "VENDA.ID_VENDEDOR , ID_CLIENTE TYPE OF COLUMN TB_NFVENDA.ID_CLIENTE , NF_NUMERO " +
                "TYPE OF COLUMN TB_NFVENDA.NF_NUMERO , NF_SERIE TYPE OF COLUMN TB_NFVENDA.NF_SERI" +
                "E , NF_MODELO TYPE OF COLUMN TB_NFVENDA.NF_MODELO , DT_EMISSAO TYPE OF COLUMN TB" +
                "_NFVENDA.DT_EMISSAO , DT_SAIDA TYPE OF COLUMN TB_NFVENDA.DT_SAIDA , HR_SAIDA TYP" +
                "E OF COLUMN TB_NFVENDA.HR_SAIDA , ESPECIE TYPE OF COLUMN TB_NFVENDA.ESPECIE , TI" +
                "PO_FRETE TYPE OF COLUMN TB_NFVENDA.TIPO_FRETE , PES_LIQUID TYPE OF COLUMN TB_NFV" +
                "ENDA.PES_LIQUID , PES_BRUTO TYPE OF COLUMN TB_NFVENDA.PES_BRUTO , STATUS TYPE OF" +
                " COLUMN TB_NFVENDA.STATUS , ENT_SAI TYPE OF COLUMN TB_NFVENDA.ENT_SAI , ID_FMAPG" +
                "TO TYPE OF COLUMN TB_NFVENDA.ID_FMAPGTO , ID_PARCELA TYPE OF COLUMN TB_NFVENDA.I" +
                "D_PARCELA , MARCA TYPE OF COLUMN TB_NFVENDA.MARCA , QTD_VOLUM TYPE OF COLUMN TB_" +
                "NFVENDA.QTD_VOLUM , NUM_VOLUM TYPE OF COLUMN TB_NFVENDA.NUM_VOLUM , PROD_REV TYP" +
                "E OF COLUMN TB_NFVENDA.PROD_REV , SOMA_FRETE TYPE OF COLUMN TB_NFVENDA.SOMA_FRET" +
                "E , VLR_TROCO TYPE OF COLUMN TB_NFVENDA.VLR_TROCO , IND_PRES TYPE OF COLUMN TB_N" +
                "FVENDA.IND_PRES , IND_IE_DEST TYPE OF COLUMN TB_NFVENDA.IND_IE_DEST , DESCONTO_C" +
                "ONDICIONAL TYPE OF COLUMN TB_NFVENDA.DESCONTO_CONDICIONAL , INF_COMP_FIXA TYPE O" +
                "F COLUMN TB_NFVENDA.INF_COMP_FIXA , INF_COMP_EDIT TYPE OF COLUMN TB_NFVENDA.INF_" +
                "COMP_EDIT , ENDERECO_ENTREGA TYPE OF COLUMN TB_NFVENDA.ENDERECO_ENTREGA , ENVIO_" +
                "API TYPE OF COLUMN TB_NFVENDA.ENVIO_API , SYNCED SMALLINT , QTD_CTAREC INTEGER )" +
                " AS BEGIN FOR SELECT FIRST 200 COALESCE(a.ID_NFVENDA, 0) , a.ID_NATOPE , a.ID_VE" +
                ...
                "EGA , a.ENVIO_API , a.SYNCED , COUNT(b.ID_CTAREC) QTD_CTAREC FROM TB_NFVENDA a L" +
                "EFT JOIN TB_NFV_CTAREC b ON a.ID_NFVENDA = b.ID_NFVENDA WHERE ( :TIP_QUERY = 0 A" +
                "ND ( ( a.SYNCED IS NULL OR a.SYNCED = 0 ) AND (a.STATUS = \'\'E\'\' OR a.STATUS = \'\'" +
                "I\'\') ) ) OR ( :TIP_QUERY = 1 AND ( a.SYNCED = 1 AND (a.STATUS = \'\'C\'\' OR a.STATU" +
                "S = \'\'X\'\') ) ) GROUP BY a.ID_NFVENDA , a.ID_NATOPE , a.ID_VENDEDOR , a.ID_CLIENT" +
                "E , a.NF_NUMERO , a.NF_SERIE , a.NF_MODELO , a.DT_EMISSAO , a.DT_SAIDA , a.HR_SA" +
                "IDA , a.ESPECIE , a.TIPO_FRETE , a.PES_LIQUID , a.PES_BRUTO , a.STATUS , a.ENT_S" +
                "AI , a.ID_FMAPGTO , a.ID_PARCELA , a.MARCA , a.QTD_VOLUM , a.NUM_VOLUM , a.PROD_" +
                "REV , a.SOMA_FRETE , a.VLR_TROCO , a.IND_PRES , a.IND_IE_DEST , a.DESCONTO_CONDI" +
                "CIONAL , a.INF_COMP_FIXA , a.INF_COMP_EDIT , a.ENDERECO_ENTREGA , a.ENVIO_API , " +
                "a.SYNCED ORDER BY a.DT_EMISSAO , a.DT_SAIDA , a.HR_SAIDA INTO :ID_NFVENDA , :ID_" +
                "NATOPE , :ID_VENDEDOR , :ID_CLIENTE , :NF_NUMERO , :NF_SERIE , :NF_MODELO , :DT_" +
                "EMISSAO , :DT_SAIDA , :HR_SAIDA , :ESPECIE , :TIPO_FRETE , :PES_LIQUID , :PES_BR" +
                "UTO , :STATUS , :ENT_SAI , :ID_FMAPGTO , :ID_PARCELA , :MARCA , :QTD_VOLUM , :NU" +
                "M_VOLUM , :PROD_REV , :SOMA_FRETE , :VLR_TROCO , :IND_PRES , :IND_IE_DEST , :DES" +
                "CONTO_CONDICIONAL , :INF_COMP_FIXA , :INF_COMP_EDIT , :ENDERECO_ENTREGA , :ENVIO" +
                "_API , :SYNCED , :QTD_CTAREC DO BEGIN SUSPEND ; END END;\';\r\n\r\nerro = \'sproc SP_T" +
                "RI_NFVENDA_SYNC_INSERT\';\r\nexecute statement \'CREATE OR ALTER PROCEDURE SP_TRI_NF" +
                "VENDA_SYNC_INSERT ( pID_NATOPE TYPE OF COLUMN TB_NFVENDA.ID_NATOPE , pID_VENDEDO" +
                "R TYPE OF COLUMN TB_NFVENDA.ID_VENDEDOR , pID_CLIENTE TYPE OF COLUMN TB_NFVENDA." +
                "ID_CLIENTE , pNF_NUMERO TYPE OF COLUMN TB_NFVENDA.NF_NUMERO , pNF_SERIE TYPE OF " +
                "COLUMN TB_NFVENDA.NF_SERIE , pNF_MODELO TYPE OF COLUMN TB_NFVENDA.NF_MODELO , pD" +
                "T_EMISSAO TYPE OF COLUMN TB_NFVENDA.DT_EMISSAO , pDT_SAIDA TYPE OF COLUMN TB_NFV" +
                "ENDA.DT_SAIDA , pHR_SAIDA TYPE OF COLUMN TB_NFVENDA.HR_SAIDA , pESPECIE TYPE OF " +
                "COLUMN TB_NFVENDA.ESPECIE , pTIPO_FRETE TYPE OF COLUMN TB_NFVENDA.TIPO_FRETE , p" +
                "PES_LIQUID TYPE OF COLUMN TB_NFVENDA.PES_LIQUID , pPES_BRUTO TYPE OF COLUMN TB_N" +
                "FVENDA.PES_BRUTO , pSTATUS TYPE OF COLUMN TB_NFVENDA.STATUS , pENT_SAI TYPE OF C" +
                "OLUMN TB_NFVENDA.ENT_SAI , pID_FMAPGTO TYPE OF COLUMN TB_NFVENDA.ID_FMAPGTO , pI" +
                "D_PARCELA TYPE OF COLUMN TB_NFVENDA.ID_PARCELA , pMARCA TYPE OF COLUMN TB_NFVEND" +
                "A.MARCA , pQTD_VOLUM TYPE OF COLUMN TB_NFVENDA.QTD_VOLUM , pNUM_VOLUM TYPE OF CO" +
                "LUMN TB_NFVENDA.NUM_VOLUM , pPROD_REV TYPE OF COLUMN TB_NFVENDA.PROD_REV , pSOMA" +
                "_FRETE TYPE OF COLUMN TB_NFVENDA.SOMA_FRETE , pVLR_TROCO TYPE OF COLUMN TB_NFVEN" +
                "DA.VLR_TROCO , pIND_PRES TYPE OF COLUMN TB_NFVENDA.IND_PRES , pIND_IE_DEST TYPE " +
                "OF COLUMN TB_NFVENDA.IND_IE_DEST , pDESCONTO_CONDICIONAL TYPE OF COLUMN TB_NFVEN" +
                "DA.DESCONTO_CONDICIONAL , pINF_COMP_FIXA TYPE OF COLUMN TB_NFVENDA.INF_COMP_FIXA" +
                " , pINF_COMP_EDIT TYPE OF COLUMN TB_NFVENDA.INF_COMP_EDIT , pENDERECO_ENTREGA TY" +
                "PE OF COLUMN TB_NFVENDA.ENDERECO_ENTREGA , pENVIO_API TYPE OF COLUMN TB_NFVENDA." +
                "ENVIO_API , pSYNCED SMALLINT ) RETURNS ( NFVENDAUSAVEL INTEGER ) AS DECLARE VARI" +
                "ABLE ULTIMONFVENDA INTEGER; BEGIN SELECT NEXT VALUE FOR GEN_TB_NFVENDA_ID FROM R" +
                "DB$DATABASE INTO :ULTIMONFVENDA ; IF( :ULTIMONFVENDA IS NULL ) THEN ULTIMONFVEND" +
                "A = 0; BEGIN INSERT INTO TB_NFVENDA ( ID_NFVENDA , ID_NATOPE , ID_VENDEDOR , ID_" +
                "CLIENTE , NF_NUMERO , NF_SERIE , NF_MODELO , DT_EMISSAO , DT_SAIDA , HR_SAIDA , " +
                "ESPECIE , TIPO_FRETE , PES_LIQUID , PES_BRUTO , STATUS , ENT_SAI , ID_FMAPGTO , " +
                "ID_PARCELA , MARCA , QTD_VOLUM , NUM_VOLUM , PROD_REV , SOMA_FRETE , VLR_TROCO ," +
                " IND_PRES , IND_IE_DEST , DESCONTO_CONDICIONAL , INF_COMP_FIXA , INF_COMP_EDIT ," +
                " ENDERECO_ENTREGA , ENVIO_API , SYNCED ) VALUES (:ULTIMONFVENDA , :pID_NATOPE , " +
                ":pID_VENDEDOR , :pID_CLIENTE , :pNF_NUMERO , :pNF_SERIE , :pNF_MODELO , :pDT_EMI" +
                "SSAO , :pDT_SAIDA , :pHR_SAIDA , :pESPECIE , :pTIPO_FRETE , :pPES_LIQUID , :pPES" +
                "_BRUTO , :pSTATUS , :pENT_SAI , :pID_FMAPGTO , :pID_PARCELA , :pMARCA , :pQTD_VO" +
                "LUM , :pNUM_VOLUM , :pPROD_REV , :pSOMA_FRETE , :pVLR_TROCO , :pIND_PRES , :pIND" +
                "_IE_DEST , :pDESCONTO_CONDICIONAL , :pINF_COMP_FIXA , :pINF_COMP_EDIT , :pENDERE" +
                "CO_ENTREGA , :pENVIO_API , :pSYNCED ) ; NFVENDAUSAVEL = ULTIMONFVENDA; END END;\'" +
                ";\r\n\r\nerro = \'sproc SP_TRI_NFV_FMAPAGTO_BY_IDNFV\';\r\nexecute statement \'CREATE OR " +
                "ALTER PROCEDURE SP_TRI_NFV_FMAPAGTO_BY_IDNFV ( pIdNfvenda TYPE OF COLUMN TB_NFVE" +
                "NDA_FMAPAGTO_NFCE.ID_NFVENDA ) RETURNS ( ID_NUMPAG TYPE OF COLUMN TB_NFVENDA_FMA" +
                "PAGTO_NFCE.ID_NUMPAG , VLR_PAGTO TYPE OF COLUMN TB_NFVENDA_FMAPAGTO_NFCE.VLR_PAG" +
                "TO , ID_NFVENDA TYPE OF COLUMN TB_NFVENDA_FMAPAGTO_NFCE.ID_NFVENDA , ID_FMANFCE " +
                "TYPE OF COLUMN TB_NFVENDA_FMAPAGTO_NFCE.ID_FMANFCE , ID_PARCELA TYPE OF COLUMN T" +
                "B_NFVENDA_FMAPAGTO_NFCE.ID_PARCELA) AS BEGIN FOR SELECT a.ID_NUMPAG , a.VLR_PAGT" +
                "O , a.ID_NFVENDA , a.ID_FMANFCE , a.ID_PARCELA FROM TB_NFVENDA_FMAPAGTO_NFCE a W" +
                "HERE a.ID_NFVENDA =:pIdNfvenda INTO :ID_NUMPAG , :VLR_PAGTO , :ID_NFVENDA , :ID_" +
                "FMANFCE , :ID_PARCELA DO SUSPEND ; END;\';\r\n\r\nerro = \'sproc SP_TRI_NFV_FMAPAGT_SY" +
                "NC_INSERT\';\r\nexecute statement \'CREATE OR ALTER PROCEDURE SP_TRI_NFV_FMAPAGT_SYN" +
                "C_INSERT ( pVLR_PAGTO TYPE OF COLUMN TB_NFVENDA_FMAPAGTO_NFCE.VLR_PAGTO , pID_NF" +
                "VENDA TYPE OF COLUMN TB_NFVENDA_FMAPAGTO_NFCE.ID_NFVENDA , pID_FMANFCE TYPE OF C" +
                "OLUMN TB_NFVENDA_FMAPAGTO_NFCE.ID_FMANFCE , pID_PARCELA TYPE OF COLUMN TB_NFVEND" +
                "A_FMAPAGTO_NFCE.ID_PARCELA ) RETURNS ( rNewIdNumpag TYPE OF COLUMN TB_NFVENDA_FM" +
                "APAGTO_NFCE.ID_NUMPAG ) AS DECLARE VARIABLE newIdNumpag TYPE OF COLUMN TB_NFVEND" +
                "A_FMAPAGTO_NFCE.ID_NUMPAG; BEGIN SELECT NEXT VALUE FOR gen_tb_nfvenda_fmapagto_n" +
                "fce_id FROM RDB$DATABASE INTO :newIdNumpag ; IF( :newIdNumpag IS NULL ) THEN new" +
                "IdNumpag = 0; BEGIN INSERT INTO TB_NFVENDA_FMAPAGTO_NFCE (ID_NUMPAG , VLR_PAGTO " +
                ", ID_NFVENDA , ID_FMANFCE , ID_PARCELA ) VALUES ( :newIdNumpag , :pVLR_PAGTO , :" +
                "pID_NFVENDA , :pID_FMANFCE , :pID_PARCELA ) ; rNewIdNumpag = newIdNumpag; END EN" +
                "D;\';\r\n\r\nerro = \'sproc SP_TRI_NFVITEM_GETBY_IDNFVENDA\';\r\nexecute statement \'CREAT" +
                "E OR ALTER PROCEDURE SP_TRI_NFVITEM_GETBY_IDNFVENDA ( pIdNfvenda INTEGER ) RETUR" +
                "NS ( ID_NFVITEM TYPE OF COLUMN TB_NFV_ITEM.ID_NFVITEM , ID_NFVENDA TYPE OF COLUM" +
                "N TB_NFV_ITEM.ID_NFVENDA , ID_IDENTIFICADOR TYPE OF COLUMN TB_NFV_ITEM.ID_IDENTI" +
                "FICADOR , CFOP TYPE OF COLUMN TB_NFV_ITEM.CFOP , NUM_ITEM TYPE OF COLUMN TB_NFV_" +
                "ITEM.NUM_ITEM , QTD_ITEM TYPE OF COLUMN TB_NFV_ITEM.QTD_ITEM , UNI_MEDIDA TYPE O" +
                "F COLUMN TB_NFV_ITEM.UNI_MEDIDA , VLR_TOTAL TYPE OF COLUMN TB_NFV_ITEM.VLR_TOTAL" +
                " , VLR_DESC TYPE OF COLUMN TB_NFV_ITEM.VLR_DESC , VLR_CUSTO TYPE OF COLUMN TB_NF" +
                "V_ITEM.VLR_CUSTO , PRC_LISTA TYPE OF COLUMN TB_NFV_ITEM.PRC_LISTA , CF TYPE OF C" +
                "OLUMN TB_NFV_ITEM.CF , VLR_FRETE TYPE OF COLUMN TB_NFV_ITEM.VLR_FRETE , VLR_SEGU" +
                "RO TYPE OF COLUMN TB_NFV_ITEM.VLR_SEGURO , VLR_DESPESA TYPE OF COLUMN TB_NFV_ITE" +
                "M.VLR_DESPESA , RET_PIS_COF_CSLL TYPE OF COLUMN TB_NFV_ITEM.RET_PIS_COF_CSLL , R" +
                "ET_IRRF TYPE OF COLUMN TB_NFV_ITEM.RET_IRRF , COD_ENQ TYPE OF COLUMN TB_NFV_ITEM" +
                ".COD_ENQ , COD_BASE TYPE OF COLUMN TB_NFV_ITEM.COD_BASE , CSOSN TYPE OF COLUMN T" +
                "B_NFV_ITEM.CSOSN , NPED_COMPRA TYPE OF COLUMN TB_NFV_ITEM.NPED_COMPRA , ITEM_COM" +
                "PRA TYPE OF COLUMN TB_NFV_ITEM.ITEM_COMPRA , VLR_TOTTRIB TYPE OF COLUMN TB_NFV_I" +
                "TEM.VLR_TOTTRIB , FCI TYPE OF COLUMN TB_NFV_ITEM.FCI , VLR_ICM_DESO TYPE OF COLU" +
                "MN TB_NFV_ITEM.VLR_ICM_DESO , ID_MOTIVO_DESO TYPE OF COLUMN TB_NFV_ITEM.ID_MOTIV" +
                "O_DESO , EST_BX TYPE OF COLUMN TB_NFV_ITEM.EST_BX , VLR_TRIB_FED TYPE OF COLUMN " +
                "TB_NFV_ITEM.VLR_TRIB_FED , VLR_TRIB_EST TYPE OF COLUMN TB_NFV_ITEM.VLR_TRIB_EST " +
                ", VLR_TRIB_MUN TYPE OF COLUMN TB_NFV_ITEM.VLR_TRIB_MUN , INCLUIR_FATURA TYPE OF " +
                "COLUMN TB_NFV_ITEM.INCLUIR_FATURA , VLR_UNIT TYPE OF COLUMN TB_NFV_ITEM.VLR_UNIT" +
                " , VLR_RETENCAO TYPE OF COLUMN TB_NFV_ITEM.VLR_RETENCAO , REFERENCIA TYPE OF COL" +
                "UMN TB_NFV_ITEM.REFERENCIA , ID_COMPOSICAO TYPE OF COLUMN TB_EST_COMPOSICAO.ID_C" +
                "OMPOSICAO ) AS BEGIN FOR SELECT COALESCE(a.ID_NFVITEM, 0) ID_NFVITEM , a.ID_NFVE" +
                "NDA , a.ID_IDENTIFICADOR , a.CFOP , a.NUM_ITEM , a.QTD_ITEM , a.UNI_MEDIDA , a.V" +
                "LR_TOTAL , a.VLR_DESC , a.VLR_CUSTO , a.PRC_LISTA , a.CF , a.VLR_FRETE , a.VLR_S" +
                "EGURO , a.VLR_DESPESA , a.RET_PIS_COF_CSLL , a.RET_IRRF , a.COD_ENQ , a.COD_BASE" +
                " , a.CSOSN , a.NPED_COMPRA , a.ITEM_COMPRA , a.VLR_TOTTRIB , a.FCI , a.VLR_ICM_D" +
                "ESO , a.ID_MOTIVO_DESO , a.EST_BX , a.VLR_TRIB_FED , a.VLR_TRIB_EST , a.VLR_TRIB" +
                "_MUN , a.INCLUIR_FATURA , a.VLR_UNIT , a.VLR_RETENCAO , a.REFERENCIA , e.ID_COMP" +
                "OSICAO FROM TB_NFV_ITEM a LEFT JOIN TB_EST_COMPOSICAO e ON e.ID_IDENTIFICADOR = " +
                "a.ID_IDENTIFICADOR WHERE a.ID_NFVENDA =:pIdNfvenda INTO :ID_NFVITEM , :ID_NFVEND" +
                "A , :ID_IDENTIFICADOR , :CFOP , :NUM_ITEM , :QTD_ITEM , :UNI_MEDIDA , :VLR_TOTAL" +
                " , :VLR_DESC , :VLR_CUSTO , :PRC_LISTA , :CF , :VLR_FRETE , :VLR_SEGURO , :VLR_D" +
                "ESPESA , :RET_PIS_COF_CSLL , :RET_IRRF , :COD_ENQ , :COD_BASE , :CSOSN , :NPED_C" +
                "OMPRA , :ITEM_COMPRA , :VLR_TOTTRIB , :FCI , :VLR_ICM_DESO , :ID_MOTIVO_DESO , :" +
                "EST_BX , :VLR_TRIB_FED , :VLR_TRIB_EST , :VLR_TRIB_MUN , :INCLUIR_FATURA , :VLR_" +
                "UNIT , :VLR_RETENCAO , :REFERENCIA , :ID_COMPOSICAO DO SUSPEND ; END;\';\r\n\r\nerro " +
                "= \'sproc SP_TRI_NFVITEM_SYNC_INSERT\';\r\nexecute statement \'CREATE OR ALTER PROCED" +
                "URE SP_TRI_NFVITEM_SYNC_INSERT ( pID_NFVENDA TYPE OF COLUMN TB_NFV_ITEM.ID_NFVEN" +
                "DA , pID_IDENTIFICADOR TYPE OF COLUMN TB_NFV_ITEM.ID_IDENTIFICADOR , pCFOP TYPE " +
                "OF COLUMN TB_NFV_ITEM.CFOP , pNUM_ITEM TYPE OF COLUMN TB_NFV_ITEM.NUM_ITEM , pQT" +
                "D_ITEM TYPE OF COLUMN TB_NFV_ITEM.QTD_ITEM , pUNI_MEDIDA TYPE OF COLUMN TB_NFV_I" +
                "TEM.UNI_MEDIDA , pVLR_TOTAL TYPE OF COLUMN TB_NFV_ITEM.VLR_TOTAL , pVLR_DESC TYP" +
                "E OF COLUMN TB_NFV_ITEM.VLR_DESC , pVLR_CUSTO TYPE OF COLUMN TB_NFV_ITEM.VLR_CUS" +
                "TO , pPRC_LISTA TYPE OF COLUMN TB_NFV_ITEM.PRC_LISTA , pCF TYPE OF COLUMN TB_NFV" +
                "_ITEM.CF , pVLR_FRETE TYPE OF COLUMN TB_NFV_ITEM.VLR_FRETE , pVLR_SEGURO TYPE OF" +
                " COLUMN TB_NFV_ITEM.VLR_SEGURO , pVLR_DESPESA TYPE OF COLUMN TB_NFV_ITEM.VLR_DES" +
                "PESA , pRET_PIS_COF_CSLL TYPE OF COLUMN TB_NFV_ITEM.RET_PIS_COF_CSLL , pRET_IRRF" +
                " TYPE OF COLUMN TB_NFV_ITEM.RET_IRRF , pCOD_ENQ TYPE OF COLUMN TB_NFV_ITEM.COD_E" +
                "NQ , pCOD_BASE TYPE OF COLUMN TB_NFV_ITEM.COD_BASE , pCSOSN TYPE OF COLUMN TB_NF" +
                "V_ITEM.CSOSN , pNPED_COMPRA TYPE OF COLUMN TB_NFV_ITEM.NPED_COMPRA , pITEM_COMPR" +
                "A TYPE OF COLUMN TB_NFV_ITEM.ITEM_COMPRA , pVLR_TOTTRIB TYPE OF COLUMN TB_NFV_IT" +
                "EM.VLR_TOTTRIB , pFCI TYPE OF COLUMN TB_NFV_ITEM.FCI , pVLR_ICM_DESO TYPE OF COL" +
                "UMN TB_NFV_ITEM.VLR_ICM_DESO , pID_MOTIVO_DESO TYPE OF COLUMN TB_NFV_ITEM.ID_MOT" +
                "IVO_DESO , pEST_BX TYPE OF COLUMN TB_NFV_ITEM.EST_BX , pVLR_TRIB_FED TYPE OF COL" +
                "UMN TB_NFV_ITEM.VLR_TRIB_FED , pVLR_TRIB_EST TYPE OF COLUMN TB_NFV_ITEM.VLR_TRIB" +
                "_EST , pVLR_TRIB_MUN TYPE OF COLUMN TB_NFV_ITEM.VLR_TRIB_MUN , pINCLUIR_FATURA T" +
                "YPE OF COLUMN TB_NFV_ITEM.INCLUIR_FATURA , pVLR_UNIT TYPE OF COLUMN TB_NFV_ITEM." +
                "VLR_UNIT , pVLR_RETENCAO TYPE OF COLUMN TB_NFV_ITEM.VLR_RETENCAO , pREFERENCIA T" +
                "YPE OF COLUMN TB_NFV_ITEM.REFERENCIA ) RETURNS ( rnewIdNfvItem INTEGER ) AS DECL" +
                "ARE VARIABLE newIdNfvItem INTEGER; BEGIN SELECT NEXT VALUE FOR GEN_TB_NFV_ITEM_I" +
                "D FROM RDB$DATABASE INTO :newIdNfvItem ;  IF( :newIdNfvItem IS NULL ) THEN newId" +
                "NfvItem = 0; BEGIN INSERT INTO TB_NFV_ITEM ( ID_NFVITEM , ID_NFVENDA , ID_IDENTI" +
                "FICADOR , CFOP , NUM_ITEM , QTD_ITEM , UNI_MEDIDA , VLR_TOTAL , VLR_DESC , VLR_C" +
                "USTO , PRC_LISTA , CF , VLR_FRETE , VLR_SEGURO , VLR_DESPESA , RET_PIS_COF_CSLL " +
                ", RET_IRRF , COD_ENQ , COD_BASE , CSOSN , NPED_COMPRA , ITEM_COMPRA , VLR_TOTTRI" +
                "B , FCI , VLR_ICM_DESO , ID_MOTIVO_DESO , EST_BX , VLR_TRIB_FED , VLR_TRIB_EST ," +
                " VLR_TRIB_MUN , INCLUIR_FATURA , VLR_UNIT , VLR_RETENCAO , REFERENCIA  ) VALUES " +
                "( :newIdNfvItem , :pID_NFVENDA , :pID_IDENTIFICADOR , :pCFOP , :pNUM_ITEM , :pQT" +
                "D_ITEM , :pUNI_MEDIDA , :pVLR_TOTAL , :pVLR_DESC , :pVLR_CUSTO , :pPRC_LISTA , :" +
                "pCF , :pVLR_FRETE , :pVLR_SEGURO , :pVLR_DESPESA , :pRET_PIS_COF_CSLL , :pRET_IR" +
                "RF , :pCOD_ENQ , :pCOD_BASE , :pCSOSN , :pNPED_COMPRA , :pITEM_COMPRA , :pVLR_TO" +
                "TTRIB , :pFCI , :pVLR_ICM_DESO , :pID_MOTIVO_DESO , :pEST_BX , :pVLR_TRIB_FED , " +
                ":pVLR_TRIB_EST , :pVLR_TRIB_MUN , :pINCLUIR_FATURA , :pVLR_UNIT , :pVLR_RETENCAO" +
                " , :pREFERENCIA  ) ;  rnewIdNfvItem = newIdNfvItem; END END;\';\r\n\r\nerro = \'sproc " +
                "SP_TRI_CTAREC_GETBY_IDNFVENDA\';\r\nexecute statement \'CREATE OR ALTER PROCEDURE SP" +
                "_TRI_CTAREC_GETBY_IDNFVENDA ( pIdNfvenda INTEGER ) RETURNS ( ID_CTAREC INTEGER ," +
                " DOCUMENTO VARCHAR(12) , HISTORICO VARCHAR(50) , DT_EMISSAO DATE , DT_VENCTO DAT" +
                "E , VLR_CTAREC TYPE OF COLUMN TRI_PDV_OPER.DIN , TIP_CTAREC CHAR(1) , ID_PORTADO" +
                "R INTEGER , ID_CLIENTE INTEGER , INV_REFERENCIA VARCHAR(18) , DT_VENCTO_ORIG DAT" +
                "E , NSU_CARTAO VARCHAR(32) , ID_NUMPAG TYPE OF COLUMN TB_NFV_CTAREC.ID_NUMPAG ) " +
                "AS BEGIN FOR SELECT COALESCE(a.ID_CTAREC, 0) , a.DOCUMENTO , a.HISTORICO , a.DT_" +
                "EMISSAO , a.DT_VENCTO , a.VLR_CTAREC , a.TIP_CTAREC , a.ID_PORTADOR , a.ID_CLIEN" +
                "TE , a.INV_REFERENCIA , a.DT_VENCTO_ORIG , a.NSU_CARTAO , b.ID_NUMPAG FROM TB_CO" +
                "NTA_RECEBER a JOIN TB_NFV_CTAREC b ON a.ID_CTAREC = b.ID_CTAREC WHERE b.ID_NFVEN" +
                "DA =:pIdNfvenda INTO :ID_CTAREC , :DOCUMENTO , :HISTORICO , :DT_EMISSAO , :DT_VE" +
                "NCTO , :VLR_CTAREC , :TIP_CTAREC , :ID_PORTADOR , :ID_CLIENTE , :INV_REFERENCIA " +
                ", :DT_VENCTO_ORIG , :NSU_CARTAO , :ID_NUMPAG DO SUSPEND ; END;\';\r\n\r\nerro = \'spro" +
                "c SP_TRI_NFVENDA_SETSYNCED\';\r\nexecute statement \'CREATE OR ALTER PROCEDURE SP_TR" +
                "I_NFVENDA_SETSYNCED ( pIdNfvenda INTEGER , pSynced SMALLINT ) AS BEGIN UPDATE TB" +
                "_NFVENDA SET SYNCED = :pSynced WHERE ID_NFVENDA = :pIdNfvenda ; END;\';\r\n\r\nerro =" +
                " \'sproc SP_TRI_CTAREC_GETBY_NFNUMSERIE\';\r\nexecute statement \'CREATE OR ALTER PRO" +
                "CEDURE SP_TRI_CTAREC_GETBY_NFNUMSERIE ( pNF_NUMERO TYPE OF COLUMN TB_NFVENDA.NF_" +
                "NUMERO , pNF_SERIE TYPE OF COLUMN TB_NFVENDA.NF_SERIE ) RETURNS ( ID_CTAREC INTE" +
                "GER , DOCUMENTO VARCHAR(12) , HISTORICO VARCHAR(50) , DT_EMISSAO DATE , DT_VENCT" +
                "O DATE , VLR_CTAREC TYPE OF COLUMN TRI_PDV_OPER.DIN , TIP_CTAREC CHAR(1) , ID_PO" +
                "RTADOR INTEGER , ID_CLIENTE INTEGER , INV_REFERENCIA VARCHAR(18) , DT_VENCTO_ORI" +
                "G DATE , NSU_CARTAO VARCHAR(32) ) AS BEGIN FOR SELECT a.ID_CTAREC , a.DOCUMENTO " +
                ", a.HISTORICO , a.DT_EMISSAO , a.DT_VENCTO , a.VLR_CTAREC , a.TIP_CTAREC , a.ID_" +
                "PORTADOR , a.ID_CLIENTE , a.INV_REFERENCIA , a.DT_VENCTO_ORIG , a.NSU_CARTAO FRO" +
                "M TB_CONTA_RECEBER a JOIN TB_NFV_CTAREC b ON a.ID_CTAREC = b.ID_CTAREC JOIN TB_N" +
                "FVENDA c ON c.ID_NFVENDA = b.ID_NFVENDA WHERE c.NF_NUMERO = :pNF_NUMERO AND c.NF" +
                "_SERIE = :pNF_SERIE INTO :ID_CTAREC , :DOCUMENTO , :HISTORICO , :DT_EMISSAO , :D" +
                "T_VENCTO , :VLR_CTAREC , :TIP_CTAREC , :ID_PORTADOR , :ID_CLIENTE , :INV_REFEREN" +
                "CIA , :DT_VENCTO_ORIG , :NSU_CARTAO DO SUSPEND ; END;\';\r\n\r\nerro = \'sproc SP_TRI_" +
                "NFV_CTAREC_SYNC_DEL\';\r\nexecute statement \'CREATE OR ALTER PROCEDURE SP_TRI_NFV_C" +
                "TAREC_SYNC_DEL ( pNfNumero TYPE OF COLUMN TB_NFVENDA.NF_NUMERO , pNfSerie TYPE O" +
                "F COLUMN TB_NFVENDA.NF_SERIE , pIdCtarec TYPE OF COLUMN TB_NFV_CTAREC.ID_CTAREC " +
                ") AS DECLARE VARIABLE idNfvenda TYPE OF COLUMN TB_NFVENDA.ID_NFVENDA; BEGIN SELE" +
                "CT ID_NFVENDA FROM TB_NFVENDA WHERE NF_NUMERO = :pNfNumero AND NF_SERIE = :pNfSe" +
                "rie AND NF_MODELO = 59 INTO :idNfvenda ; DELETE FROM TB_NFV_CTAREC WHERE ID_NFVE" +
                "NDA = :idNfvenda AND ID_CTAREC= :pIdCtarec ; END;\';\r\n\r\nerro = \'sproc SP_TRI_NFV_" +
                "UPDT_BYNFNUMSERIE\';\r\nexecute statement \'CREATE OR ALTER PROCEDURE SP_TRI_NFV_UPD" +
                "T_BYNFNUMSERIE ( pID_NATOPE TYPE OF COLUMN TB_NFVENDA.ID_NATOPE , pID_VENDEDOR T" +
                "YPE OF COLUMN TB_NFVENDA.ID_VENDEDOR , pID_CLIENTE TYPE OF COLUMN TB_NFVENDA.ID_" +
                "CLIENTE , pNF_NUMERO TYPE OF COLUMN TB_NFVENDA.NF_NUMERO , pNF_SERIE TYPE OF COL" +
                "UMN TB_NFVENDA.NF_SERIE , pNF_MODELO TYPE OF COLUMN TB_NFVENDA.NF_MODELO , pDT_E" +
                "MISSAO TYPE OF COLUMN TB_NFVENDA.DT_EMISSAO , pDT_SAIDA TYPE OF COLUMN TB_NFVEND" +
                "A.DT_SAIDA , pHR_SAIDA TYPE OF COLUMN TB_NFVENDA.HR_SAIDA , pESPECIE TYPE OF COL" +
                "UMN TB_NFVENDA.ESPECIE , pTIPO_FRETE TYPE OF COLUMN TB_NFVENDA.TIPO_FRETE , pPES" +
                "_LIQUID TYPE OF COLUMN TB_NFVENDA.PES_LIQUID , pPES_BRUTO TYPE OF COLUMN TB_NFVE" +
                "NDA.PES_BRUTO , pSTATUS TYPE OF COLUMN TB_NFVENDA.STATUS , pENT_SAI TYPE OF COLU" +
                "MN TB_NFVENDA.ENT_SAI , pID_FMAPGTO TYPE OF COLUMN TB_NFVENDA.ID_FMAPGTO , pID_P" +
                "ARCELA TYPE OF COLUMN TB_NFVENDA.ID_PARCELA , pMARCA TYPE OF COLUMN TB_NFVENDA.M" +
                "ARCA , pQTD_VOLUM TYPE OF COLUMN TB_NFVENDA.QTD_VOLUM , pNUM_VOLUM TYPE OF COLUM" +
                "N TB_NFVENDA.NUM_VOLUM , pPROD_REV TYPE OF COLUMN TB_NFVENDA.PROD_REV , pSOMA_FR" +
                "ETE TYPE OF COLUMN TB_NFVENDA.SOMA_FRETE , pVLR_TROCO TYPE OF COLUMN TB_NFVENDA." +
                "VLR_TROCO , pIND_PRES TYPE OF COLUMN TB_NFVENDA.IND_PRES , pIND_IE_DEST TYPE OF " +
                "COLUMN TB_NFVENDA.IND_IE_DEST , pDESCONTO_CONDICIONAL TYPE OF COLUMN TB_NFVENDA." +
                "DESCONTO_CONDICIONAL , pINF_COMP_FIXA TYPE OF COLUMN TB_NFVENDA.INF_COMP_FIXA , " +
                "pINF_COMP_EDIT TYPE OF COLUMN TB_NFVENDA.INF_COMP_EDIT , pENDERECO_ENTREGA TYPE " +
                "OF COLUMN TB_NFVENDA.ENDERECO_ENTREGA , pENVIO_API TYPE OF COLUMN TB_NFVENDA.ENV" +
                "IO_API , pSYNCED TYPE OF COLUMN TB_NFVENDA.SYNCED ) AS BEGIN UPDATE TB_NFVENDA S" +
                "ET ID_NATOPE =:pID_NATOPE , ID_VENDEDOR =:pID_VENDEDOR , ID_CLIENTE =:pID_CLIENT" +
                "E , NF_MODELO =:pNF_MODELO , DT_EMISSAO =:pDT_EMISSAO , DT_SAIDA =:pDT_SAIDA , H" +
                "R_SAIDA =:pHR_SAIDA , ESPECIE =:pESPECIE , TIPO_FRETE =:pTIPO_FRETE , PES_LIQUID" +
                " =:pPES_LIQUID , PES_BRUTO =:pPES_BRUTO , STATUS =:pSTATUS , ENT_SAI =:pENT_SAI " +
                ", ID_FMAPGTO =:pID_FMAPGTO , ID_PARCELA =:pID_PARCELA , MARCA =:pMARCA , QTD_VOL" +
                "UM =:pQTD_VOLUM , NUM_VOLUM =:pNUM_VOLUM , PROD_REV =:pPROD_REV , SOMA_FRETE =:p" +
                "SOMA_FRETE , VLR_TROCO =:pVLR_TROCO , IND_PRES =:pIND_PRES , IND_IE_DEST =:pIND_" +
                "IE_DEST , DESCONTO_CONDICIONAL=:pDESCONTO_CONDICIONAL , INF_COMP_FIXA =:pINF_COM" +
                "P_FIXA , INF_COMP_EDIT =:pINF_COMP_EDIT , ENDERECO_ENTREGA =:pENDERECO_ENTREGA ," +
                " ENVIO_API =:pENVIO_API , SYNCED =:pSYNCED WHERE NF_NUMERO = :pNF_NUMERO and NF_" +
